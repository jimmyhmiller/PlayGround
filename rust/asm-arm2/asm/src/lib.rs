...
fn get_instruction_comments(instruction_section: Node, xml_file_text: &str) -> Vec<String> {
...
pub fn generate_template() -> Result<(), Box<dyn Error>> {
    ...
    for instruction_section in instruction_sections {
        ...
        let comments = get_instruction_comments(instruction_section, xml_file_text);
    ...
    let mut file = File::create("asm/src/arm2.rs")?;
