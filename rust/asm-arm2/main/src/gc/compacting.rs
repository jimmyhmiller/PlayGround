...
impl Iterator for ObjectIterator {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if self.offset >= space.space[self.segment_index].offset {
            self.segment_index += 1;
            self.offset = 0;
        }
        if self.segment_index == space.space.len() {
            return None;
        }
...
impl Space {
    ...
    fn contains(&self, pointer: *const u8) -> bool {
        for segment in self.space.iter() {
    ...
    fn copy_data_to_offset(&mut self, data: &[u8]) -> isize {
        ...
        let segment = self.space.get_mut(self.segment_offset).unwrap();
    ...
    fn write_object(&mut self, segment_offset: usize, offset: usize, shifted_size: usize) -> usize {
        let memory = &mut self.space[segment_offset].memory;
    ...
    fn increment_current_offset(&mut self, size: usize) {
        self.space[self.segment_offset].offset += size;
        // align to 8 bytes
        self.space[self.segment_offset].offset =
            (self.space[self.segment_offset].offset + 7) & !7;
        debug_assert!(
            self.space[self.segment_offset].offset % 8 == 0,
            "Heap offset is not aligned"
        );
    }

    fn can_allocate(&mut self, size: usize) -> bool {
        let segment = self.space.get(self.segment_offset).unwrap();
        let current_segment = segment.offset + size + 8 < segment.size;
        if current_segment {
            return true;
        }
       while self.segment_offset < self.space.len() {
            let segment = self.space.get(self.segment_offset).unwrap();
        ...
        if self.segment_offset == self.space.len() {
            self.segment_offset = self.space.len() - 1;
        }
        false
    }
    ...
    fn allocate(&mut self, bytes: usize) -> Result<usize, Box<dyn Error>> {
        ...
        if offset + size > segment.size {
            self.segment_offset += 1;
            if self.segment_offset == self.space.len() {
                self.space.push(Segment::new(self.segment_size));
            }
            offset = 0;
        }
    ...
    fn clear(&mut self) {
        for segment in self.space.iter_mut() {
            segment.offset = 0;
        }
