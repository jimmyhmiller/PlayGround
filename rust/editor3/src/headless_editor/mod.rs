...
pub struct ViewTextBuffer<T: TextBuffer> {
    ...
    pub text_views: Vec<(usize, usize)>,
}

impl<T> ViewTextBuffer<T>
where
    T: TextBuffer<Item = u8>,
{
    pub fn text_view_for_selector(&mut self, selector: &Selector) -> TextView {
        match selector {
            Selector::Lines(start, end) => {
                ...
                let index = self.text_views.len();
                self.text_views.push((*start, *end));
                TextView(index)
            }
            Selector::Indentifer(_) => todo!(),
        }
    }

    pub fn get_lines_for_text_view(&self, text_view: &TextView) -> impl Iterator<Item = &[u8]> {
    ...
    T: TextBuffer<Item = u8>,
{
    ...
    fn line_length(&self, line: usize) -> usize {
        self.text_buffer.line_length(line)
    }

    fn line_start(&self, line: usize) -> usize {
        self.text_buffer.line_start(line)
    }
    ...
    fn insert_bytes(&mut self, line: usize, column: usize, text: &[Self::Item]) {
        self.text_buffer.insert_bytes(line, column, text);

        if text == b"\n" {
            for (start, end) in self.text_views.iter_mut() {
                // TODO: This isn't quite right
                // We need real editing to test this out better
    ...
    fn byte_at_pos(&self, line: usize, column: usize) -> Option<&Self::Item> {
        self.text_buffer.byte_at_pos(line, column)
    }

    fn delete_char(&mut self, line: usize, column: usize) {
        self.text_buffer.delete_char(line, column);
    }
    ...
    fn get_line(&self, index: usize) -> Option<&[Self::Item]> {
        self.text_buffer.get_line(index)
    }

    fn set_contents(&mut self, contents: &[Self::Item]) {
        self.text_buffer.set_contents(contents);
    }

    fn contents(&self) -> &[Self::Item] {
        self.text_buffer.contents()
    }
}
