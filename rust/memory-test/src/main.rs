...
use libc::{mmap, mprotect, PROT_EXEC, PROT_READ, PROT_WRITE, MAP_ANONYMOUS, MAP_PRIVATE, sysconf, _SC_PAGESIZE};
use mach2::exception_types::{exception_behavior_t, exception_mask_t, EXCEPTION_DEFAULT, EXC_MASK_BAD_ACCESS};
...
extern "C" fn handle_sigbus(signal: libc::c_int, info: *mut libc::siginfo_t, context: *mut libc::c_void) {
    unsafe {
...
fn main() {
    ...
    let endless_loop: Vec<ArmAsm> = vec![
        mov_imm(X0, 42),
        jump(-1),
        ret(),
    ];
    ...
    unsafe {
        ...
        mprotect(memory as *mut core::ffi::c_void, page_size, PROT_READ | PROT_EXEC);
        __clear_cache(memory as *mut core::ffi::c_void, (memory as usize + page_size) as *mut core::ffi::c_void);
    }
    ...
    unsafe {
        mprotect(memory as *mut libc::c_void, page_size, PROT_READ | PROT_WRITE);
    }
