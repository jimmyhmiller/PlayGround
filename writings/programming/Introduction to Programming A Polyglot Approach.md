...
This is is a long line that will wrap around and so even though it is multiple lines in this book it is only one line in our program.
Some more code
```

While we certainly could type out these line numbers manually, as code listings get long and we want to edit them, it will be useful to automate this. So let's make a programming langauge designed for this purpose. Our programming language will be very simple  consisting of just one instruction `LINE`. What does line do? It prints out the line we are on. So to produce the output above we would have typed:
