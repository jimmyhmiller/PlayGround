try {
	...
	const { head, emptyList, prepend, tail, isEmpty, map, filter, list, not, myExample, myFunc, zero, one, inc, dec, toNum, add, sub, fromNum, mult, nth, countDown, length, isTrue, last, showAll } = require('./list8.js')
	...
	Array.from({length: 5}, (x,i) => i).forEach(j => {
		setTimeout(function() {
			...
			const contents = fs.readFileSync('list8.js', 'utf8');
			printDivider()
