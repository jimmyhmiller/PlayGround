...
fn editing_a_promoted_function_is_picked_up() {
    ...
    assert_eq!(s.call("main", vec![]).unwrap(), Value::I64(50)); // f(49)=50
    assert!(s.engine.is_hot(f, Version(1)), "f must have been promoted");
    ...
    s.engine.install_function(f_v2).unwrap();

    // Re-run: the edit is live (f(49) = 149), even though the old f was JITted.
    assert_eq!(s.call("main", vec![]).unwrap(), Value::I64(149));
}
...
fn breaking_edit_traps_a_jit_caller_then_repair_resumes() {
    ...
    assert_eq!(s.engine.run_call(main, vec![]), Outcome::Complete(Value::I64(7)));
...
fn data_migration_is_transparent_to_a_jit_reader() {
    ...
    assert_eq!(s.call("main", vec![]).unwrap(), Value::I64(5));
