...
static int32_t parse_map_elements(Parser* p, Value* map_vec, int32_t index) {
    return ({ Token tok = g_user.peek_token(p); ((tok.type == TokenType_RightBrace) ? ({ int32_t _ = g_user.advance_parser(p); index; }) : ({ Value* elem = (Value*)g_user.parse_value(p); g_user.vector_set(map_vec, index, elem); g_user.parse_map_elements(p, map_vec, (index + 1)); })); });
}
...
static int32_t print_value_ptr(Value* v) {
    return ({ ValueTag tag = v->tag; ((tag == ValueTag_Nil) ? printf("nil") : ((tag == ValueTag_Number) ? printf("%lld", v->num_val) : ((tag == ValueTag_Symbol) ? printf("%s", v->str_val) : ((tag == ValueTag_String) ? printf("\"%s\"", v->str_val) : ((tag == ValueTag_Keyword) ? printf("%s", v->str_val) : ((tag == ValueTag_List) ? ({ int32_t _ = printf("("); g_user.print_list_contents(v); }) : ((tag == ValueTag_Vector) ? ({ int32_t _ = printf("["); g_user.print_vector_contents(v, 0); }) : ((tag == ValueTag_Map) ? ({ int32_t _ = printf("{"); g_user.print_map_contents(v, 0); }) : printf("<unknown>"))))))))); });
}
