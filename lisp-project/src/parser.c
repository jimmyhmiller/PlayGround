...
static Value* parse_list(Parser* p) {
    return ({ Token tok = g_user.peek_token(p); ((tok.type == TokenType_RightParen) ? ({ g_user.advance_parser(p); ({ Value* __tmp_ptr = malloc(sizeof(Value)); *__tmp_ptr = g_user.make_nil(); __tmp_ptr; }); }) : ({ Value* first = (Value*)g_user.parse_value(p); Value* rest = (Value*)g_user.parse_list(p); g_user.make_cons(first, rest); })); });
}
...
static int32_t parse_vector_elements(Parser* p, Value* vec, int32_t index) {
    return ({ Token tok = g_user.peek_token(p); ((tok.type == TokenType_RightBracket) ? ({ g_user.advance_parser(p); index; }) : ({ Value* elem = (Value*)g_user.parse_value(p); g_user.vector_set(vec, index, elem); g_user.parse_vector_elements(p, vec, (index + 1)); })); });
}
...
static int32_t parse_map_elements(Parser* p, Value* map_vec, int32_t index) {
    return ({ Token tok = g_user.peek_token(p); ((tok.type == TokenType_RightBrace) ? ({ g_user.advance_parser(p); index; }) : ({ Value* elem = (Value*)g_user.parse_value(p); g_user.vector_set(map_vec, index, elem); g_user.parse_map_elements(p, map_vec, (index + 1)); })); });
}
static Value* parse_map(Parser* p) {
    return ({ Value* map_vec = (Value*)g_user.make_vector_with_capacity(32, 32); int32_t final_count = g_user.parse_map_elements(p, map_vec, 0); ({ uint8_t* vec_ptr = (uint8_t*)map_vec->vec_val; Vector* vec_struct = (Vector*)((Vector*)vec_ptr); vec_struct->count = final_count; ({ Value* map_val = (Value*)({ Value* __tmp_ptr = malloc(sizeof(Value)); *__tmp_ptr = g_user.make_nil(); __tmp_ptr; }); map_val->tag = ValueTag_Map; map_val->vec_val = vec_ptr; map_val; }); }); });
}
static Value* parse_value(Parser* p) {
    return ({ Token tok = g_user.peek_token(p); ((tok.type == TokenType_LeftParen) ? ({ g_user.advance_parser(p); g_user.parse_list(p); }) : ((tok.type == TokenType_LeftBracket) ? ({ g_user.advance_parser(p); g_user.parse_vector(p); }) : ((tok.type == TokenType_LeftBrace) ? ({ g_user.advance_parser(p); g_user.parse_map(p); }) : ((tok.type == TokenType_Symbol) ? ({ g_user.advance_parser(p); ((g_user.is_number_token(tok) != 0) ? ({ uint8_t* str = (uint8_t*)g_user.copy_string(tok.text, tok.length); int64_t num = atoll(str); printf("[DEBUG] Parsed number: %lld\n", num); ({ Value* __tmp_ptr = malloc(sizeof(Value)); *__tmp_ptr = g_user.make_number(num); __tmp_ptr; }); }) : ({ uint8_t* str = (uint8_t*)g_user.copy_string(tok.text, tok.length); printf("[DEBUG] Copied symbol: '%s'\n", str); ({ Value* __tmp_ptr = malloc(sizeof(Value)); *__tmp_ptr = g_user.make_symbol(str); __tmp_ptr; }); })); }) : ((tok.type == TokenType_String) ? ({ g_user.advance_parser(p); ({ uint8_t* str_start = (uint8_t*)((uint8_t*)(((long long)tok.text) + 1)); int32_t str_len = (tok.length - 2); uint8_t* str = (uint8_t*)g_user.copy_string(str_start, str_len); printf("[DEBUG] Copied string: \"%s\"\n", str); ({ Value* __tmp_ptr = malloc(sizeof(Value)); *__tmp_ptr = (Value){ValueTag_String, 0, str, NULL, NULL}; __tmp_ptr; }); }); }) : ((tok.type == TokenType_Keyword) ? ({ g_user.advance_parser(p); ({ uint8_t* str = (uint8_t*)g_user.copy_string(tok.text, tok.length); printf("[DEBUG] Copied keyword: %s\n", str); ({ Value* __tmp_ptr = malloc(sizeof(Value)); *__tmp_ptr = (Value){ValueTag_Keyword, 0, str, NULL, NULL}; __tmp_ptr; }); }); }) : ({ Value* __tmp_ptr = malloc(sizeof(Value)); *__tmp_ptr = g_user.make_nil(); __tmp_ptr; }))))))); });
}
static int32_t print_vector_contents(Value* vec_val, int32_t index) {
    return ({ uint8_t* vec_ptr = (uint8_t*)vec_val->vec_val; Vector* vec = (Vector*)((Vector*)vec_ptr); int32_t count = vec->count; ((index >= count) ? printf("]") : ({ uint8_t* data = (uint8_t*)vec->data; uint8_t* elem_loc = (uint8_t*)((uint8_t*)(((long long)data) + (index * 8))); Value** elem_ptr_ptr = (Value**)((Value**)elem_loc); Value* elem = (Value*)(*elem_ptr_ptr); g_user.print_value_ptr(elem); (((index + 1) < count) ? ({ printf(" "); g_user.print_vector_contents(vec_val, (index + 1)); }) : printf("]")); })); });
}
static int32_t print_map_contents(Value* map_val, int32_t index) {
    return ({ uint8_t* vec_ptr = (uint8_t*)map_val->vec_val; Vector* vec = (Vector*)((Vector*)vec_ptr); int32_t count = vec->count; ((index >= count) ? printf("}") : ({ uint8_t* data = (uint8_t*)vec->data; uint8_t* elem_loc = (uint8_t*)((uint8_t*)(((long long)data) + (index * 8))); Value** elem_ptr_ptr = (Value**)((Value**)elem_loc); Value* elem = (Value*)(*elem_ptr_ptr); g_user.print_value_ptr(elem); (((index + 1) < count) ? ({ printf(" "); g_user.print_map_contents(map_val, (index + 1)); }) : printf("}")); })); });
}
static int32_t print_value_ptr(Value* v) {
    return ({ ValueTag tag = v->tag; ((tag == ValueTag_Nil) ? printf("nil") : ((tag == ValueTag_Number) ? printf("%lld", v->num_val) : ((tag == ValueTag_Symbol) ? printf("%s", v->str_val) : ((tag == ValueTag_String) ? printf("\"%s\"", v->str_val) : ((tag == ValueTag_Keyword) ? printf("%s", v->str_val) : ((tag == ValueTag_List) ? ({ printf("("); g_user.print_list_contents(v); }) : ((tag == ValueTag_Vector) ? ({ printf("["); g_user.print_vector_contents(v, 0); }) : ((tag == ValueTag_Map) ? ({ printf("{"); g_user.print_map_contents(v, 0); }) : printf("<unknown>"))))))))); });
}
static int32_t print_list_contents(Value* v) {
    return ({ ValueTag tag = v->tag; ((tag == ValueTag_Nil) ? printf(")") : ((tag == ValueTag_List) ? ({ uint8_t* cons_ptr = (uint8_t*)v->cons_val; Cons* cons_cell = (Cons*)((Cons*)cons_ptr); Value* car_ptr = (Value*)((Value*)cons_cell->car); Value* cdr_ptr = (Value*)((Value*)cons_cell->cdr); g_user.print_value_ptr(car_ptr); ({ ValueTag cdr_tag = cdr_ptr->tag; ((cdr_tag == ValueTag_Nil) ? printf(")") : ({ printf(" "); g_user.print_list_contents(cdr_ptr); })); }); }) : ({ printf(". "); g_user.print_value_ptr(v); printf(")"); }))); });
}
static int32_t main_fn() {
    ...
    printf("Test 7: [:name :age :email]\n");
    ({ Token* tokens7 = (Token*)((Token*)malloc(256)); Token* t0 = (Token*)tokens7; t0->type = TokenType_LeftBracket; t0->text = "["; t0->length = 1; ({ Token* t1 = (Token*)((Token*)(((long long)tokens7) + 24)); t1->type = TokenType_Keyword; t1->text = ":name"; t1->length = 5; ({ Token* t2 = (Token*)((Token*)(((long long)tokens7) + 48)); t2->type = TokenType_Keyword; t2->text = ":age"; t2->length = 4; ({ Token* t3 = (Token*)((Token*)(((long long)tokens7) + 72)); t3->type = TokenType_Keyword; t3->text = ":email"; t3->length = 6; ({ Token* t4 = (Token*)((Token*)(((long long)tokens7) + 96)); t4->type = TokenType_RightBracket; t4->text = "]"; t4->length = 1; ({ Parser* parser = (Parser*)g_user.make_parser(tokens7, 5); Value* result = (Value*)g_user.parse_value(parser); printf("  Result: "); g_user.print_value_ptr(result); printf("\n\n"); }); }); }); }); }); });
    printf("Test 8: {:name \"John\" :age 30}\n");
    ({ Token* tokens8 = (Token*)((Token*)malloc(256)); Token* t0 = (Token*)tokens8; t0->type = TokenType_LeftBrace; t0->text = "{"; t0->length = 1; ({ Token* t1 = (Token*)((Token*)(((long long)tokens8) + 24)); t1->type = TokenType_Keyword; t1->text = ":name"; t1->length = 5; ({ Token* t2 = (Token*)((Token*)(((long long)tokens8) + 48)); t2->type = TokenType_String; t2->text = "\"John\""; t2->length = 6; ({ Token* t3 = (Token*)((Token*)(((long long)tokens8) + 72)); t3->type = TokenType_Keyword; t3->text = ":age"; t3->length = 4; ({ Token* t4 = (Token*)((Token*)(((long long)tokens8) + 96)); t4->type = TokenType_Symbol; t4->text = "30"; t4->length = 2; ({ Token* t5 = (Token*)((Token*)(((long long)tokens8) + 120)); t5->type = TokenType_RightBrace; t5->text = "}"; t5->length = 1; ({ Parser* parser = (Parser*)g_user.make_parser(tokens8, 6); Value* result = (Value*)g_user.parse_value(parser); printf("  Result: "); g_user.print_value_ptr(result); printf("\n"); }); }); }); }); }); }); });
    return 0;
}
int main() {
