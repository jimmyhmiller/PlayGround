...
static Token peek_token(Parser* p) {
    return ({ int32_t pos = p->position; int32_t count = p->count; ({ Token __if_result; if ((pos >= count)) { __if_result = (Token){TokenType_EOF, NULL, 0}; } else { __if_result = ({ Token* tokens = (Token*)p->tokens; ({ Token* token_ptr = (Token*)((Token*)(((long long)tokens) + (((long long)pos) * 24))); TokenType ttype = token_ptr->type; uint8_t* ttext = (uint8_t*)token_ptr->text; int32_t tlen = token_ptr->length; (Token){ttype, ttext, tlen}; }); }); } __if_result; }); });
}
static int32_t advance_parser(Parser* p) {
    return ({ int32_t pos = p->position; p->position = (pos + 1); 0; });
}
static Value* parse_list(Parser* p) {
    return ({ Token tok = g_parser.peek_token(p); ({ Value* __if_result; if ((tok.type == TokenType_RightParen)) { __if_result = ({ g_parser.advance_parser(p); ({ Value* __tmp_ptr = malloc(sizeof(Value)); *__tmp_ptr = g_types.make_nil(); __tmp_ptr; }); }); } else { __if_result = ({ Value* first = (Value*)g_parser.parse_value(p); Value* rest = (Value*)g_parser.parse_list(p); g_types.make_cons(first, rest); }); } __if_result; }); });
}
static int32_t parse_vector_elements(Parser* p, Value* vec, int32_t index) {
    return ({ Token tok = g_parser.peek_token(p); ({ int32_t __if_result; if ((tok.type == TokenType_RightBracket)) { __if_result = ({ g_parser.advance_parser(p); index; }); } else { __if_result = ({ Value* elem = (Value*)g_parser.parse_value(p); g_types.vector_set(vec, index, elem); g_parser.parse_vector_elements(p, vec, (index + 1)); }); } __if_result; }); });
}
...
static int32_t parse_map_elements(Parser* p, Value* map_vec, int32_t index) {
    return ({ Token tok = g_parser.peek_token(p); ({ int32_t __if_result; if ((tok.type == TokenType_RightBrace)) { __if_result = ({ g_parser.advance_parser(p); index; }); } else { __if_result = ({ Value* elem = (Value*)g_parser.parse_value(p); g_types.vector_set(map_vec, index, elem); g_parser.parse_map_elements(p, map_vec, (index + 1)); }); } __if_result; }); });
}
...
static Value* parse_value(Parser* p) {
    return ({ Token tok = g_parser.peek_token(p); ({ Value* __if_result; if ((tok.type == TokenType_LeftParen)) { __if_result = ({ g_parser.advance_parser(p); g_parser.parse_list(p); }); } else { __if_result = ({ Value* __if_result; if ((tok.type == TokenType_LeftBracket)) { __if_result = ({ g_parser.advance_parser(p); g_parser.parse_vector(p); }); } else { __if_result = ({ Value* __if_result; if ((tok.type == TokenType_LeftBrace)) { __if_result = ({ g_parser.advance_parser(p); g_parser.parse_map(p); }); } else { __if_result = ({ Value* __if_result; if ((tok.type == TokenType_Symbol)) { __if_result = ({ g_parser.advance_parser(p); ({ Value* __if_result; if ((g_types.is_number_token(tok) != 0)) { __if_result = ({ uint8_t* str = (uint8_t*)g_types.copy_string(tok.text, tok.length); int64_t num = atoll(str); ({ Value* __tmp_ptr = malloc(sizeof(Value)); *__tmp_ptr = g_types.make_number(num); __tmp_ptr; }); }); } else { __if_result = ({ uint8_t* str = (uint8_t*)g_types.copy_string(tok.text, tok.length); ({ Value* __tmp_ptr = malloc(sizeof(Value)); *__tmp_ptr = g_types.make_symbol(str); __tmp_ptr; }); }); } __if_result; }); }); } else { __if_result = ({ Value* __if_result; if ((tok.type == TokenType_String)) { __if_result = ({ g_parser.advance_parser(p); ({ uint8_t* str_start = (uint8_t*)((uint8_t*)(((long long)tok.text) + 1)); int32_t str_len = (tok.length - 2); uint8_t* str = (uint8_t*)g_types.copy_string(str_start, str_len); ({ Value* __tmp_ptr = malloc(sizeof(Value)); *__tmp_ptr = (Value){ValueTag_String, 0, str, NULL, NULL}; __tmp_ptr; }); }); }); } else { __if_result = ({ Value* __if_result; if ((tok.type == TokenType_Keyword)) { __if_result = ({ g_parser.advance_parser(p); ({ uint8_t* str = (uint8_t*)g_types.copy_string(tok.text, tok.length); ({ Value* __tmp_ptr = malloc(sizeof(Value)); *__tmp_ptr = (Value){ValueTag_Keyword, 0, str, NULL, NULL}; __tmp_ptr; }); }); }); } else { __if_result = ({ g_parser.advance_parser(p); ({ Value* __tmp_ptr = malloc(sizeof(Value)); *__tmp_ptr = g_types.make_nil(); __tmp_ptr; }); }); } __if_result; }); } __if_result; }); } __if_result; }); } __if_result; }); } __if_result; }); } __if_result; }); });
}
static int32_t print_vector_contents(Value* vec_val, int32_t index) {
    return ({ uint8_t* vec_ptr = (uint8_t*)vec_val->vec_val; Vector* vec = (Vector*)((Vector*)vec_ptr); int32_t count = vec->count; ({ int32_t __if_result; if ((index >= count)) { __if_result = printf("]"); } else { __if_result = ({ uint8_t* data = (uint8_t*)vec->data; uint8_t* elem_loc = (uint8_t*)((uint8_t*)(((long long)data) + (index * 8))); Value** elem_ptr_ptr = (Value**)((Value**)elem_loc); Value* elem = (Value*)(*elem_ptr_ptr); g_parser.print_value_ptr(elem); ({ int32_t __if_result; if (((index + 1) < count)) { __if_result = ({ printf(" "); g_parser.print_vector_contents(vec_val, (index + 1)); }); } else { __if_result = printf("]"); } __if_result; }); }); } __if_result; }); });
}
static int32_t print_map_contents(Value* map_val, int32_t index) {
    return ({ uint8_t* vec_ptr = (uint8_t*)map_val->vec_val; Vector* vec = (Vector*)((Vector*)vec_ptr); int32_t count = vec->count; ({ int32_t __if_result; if ((index >= count)) { __if_result = printf("}"); } else { __if_result = ({ uint8_t* data = (uint8_t*)vec->data; uint8_t* elem_loc = (uint8_t*)((uint8_t*)(((long long)data) + (index * 8))); Value** elem_ptr_ptr = (Value**)((Value**)elem_loc); Value* elem = (Value*)(*elem_ptr_ptr); g_parser.print_value_ptr(elem); ({ int32_t __if_result; if (((index + 1) < count)) { __if_result = ({ printf(" "); g_parser.print_map_contents(map_val, (index + 1)); }); } else { __if_result = printf("}"); } __if_result; }); }); } __if_result; }); });
}
static int32_t print_value_ptr(Value* v) {
    return ({ ValueTag tag = v->tag; ((tag == ValueTag_Nil) ? printf("nil") : ((tag == ValueTag_Number) ? printf("%lld", v->num_val) : ((tag == ValueTag_Symbol) ? printf("%s", v->str_val) : ((tag == ValueTag_String) ? printf("\"%s\"", v->str_val) : ((tag == ValueTag_Keyword) ? printf("%s", v->str_val) : ({ int32_t __if_result; if ((tag == ValueTag_List)) { __if_result = ({ printf("("); g_parser.print_list_contents(v); }); } else { __if_result = ({ int32_t __if_result; if ((tag == ValueTag_Vector)) { __if_result = ({ printf("["); g_parser.print_vector_contents(v, 0); }); } else { __if_result = ({ int32_t __if_result; if ((tag == ValueTag_Map)) { __if_result = ({ printf("{"); g_parser.print_map_contents(v, 0); }); } else { __if_result = printf("<unknown>"); } __if_result; }); } __if_result; }); } __if_result; })))))); });
}
static int32_t print_list_contents(Value* v) {
    return ({ ValueTag tag = v->tag; ((tag == ValueTag_Nil) ? printf(")") : ({ int32_t __if_result; if ((tag == ValueTag_List)) { __if_result = ({ uint8_t* cons_ptr = (uint8_t*)v->cons_val; Cons* cons_cell = (Cons*)((Cons*)cons_ptr); Value* car_ptr = (Value*)((Value*)cons_cell->car); Value* cdr_ptr = (Value*)((Value*)cons_cell->cdr); g_parser.print_value_ptr(car_ptr); ({ ValueTag cdr_tag = cdr_ptr->tag; ({ int32_t __if_result; if ((cdr_tag == ValueTag_Nil)) { __if_result = printf(")"); } else { __if_result = ({ printf(" "); g_parser.print_list_contents(cdr_ptr); }); } __if_result; }); }); }); } else { __if_result = ({ printf(". "); g_parser.print_value_ptr(v); printf(")"); }); } __if_result; })); });
}
static int32_t main_fn() {
