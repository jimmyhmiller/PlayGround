...
static int32_t main_fn() {
    return ({ MLIRBuilderContext* builder = (MLIRBuilderContext*)g_mlir_builder.mlir_builder_init(); ({ int32_t compile_res = g_mlir_builder.compile_file(builder, "tests/fib.lisp"); ({ int32_t __if_result; if ((compile_res == 0)) { __if_result = ({ int32_t lower_res = g_mlir_builder.run_lowering_passes(builder); ({ int32_t __if_result; if ((lower_res == 0)) { __if_result = ({ int32_t jit_res = g_mlir_builder.jit_run_function_i32(builder, "fib", 10); g_mlir_builder.mlir_builder_destroy(builder); printf("fib(10) = %d\n", jit_res); ((jit_res == ((int32_t)-1)) ? ((int32_t)1) : 0); }); } else { __if_result = ({ g_mlir_builder.mlir_builder_destroy(builder); lower_res; }); } __if_result; }); }); } else { __if_result = ({ g_mlir_builder.mlir_builder_destroy(builder); compile_res; }); } __if_result; }); }); });
}
int main() {
