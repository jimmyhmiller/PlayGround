...
static Token read_string(Tokenizer* tok) {
    return ({ int32_t start_pos = tok->position; uint8_t* input = (uint8_t*)tok->input; uint8_t* start_ptr = (uint8_t*)((uint8_t*)(((int64_t)input) + ((int64_t)start_pos))); ({ g_user.advance(tok); int32_t len = 1; ({ while (((g_user.peek_char(tok) != 0) && (g_user.peek_char(tok) != 34))) { g_user.advance(tok); len = (len + 1); } }); ({ g_user.advance(tok); g_user.make_token(TokenType_String, start_ptr, (len + 1)); }); }); });
}
