...
(def peek-char (: (-> [(Pointer Tokenizer)] I32))
  (fn [tok]
    (let [pos (: I32) (pointer-field-read tok position)]
      (let [len (: I32) (pointer-field-read tok length)]
        (if (>= pos len)
            (cast I32 0)  ; null char means EOF
            (let [input (: (Pointer U8)) (pointer-field-read tok input)]
              (let [char-loc (: I64) (+ (cast I64 input) (cast I64 pos))]
                (let [char-ptr (: (Pointer U8)) (cast (Pointer U8) char-loc)]
                  (let [byte-val (: U8) (dereference char-ptr)]
                    (cast I32 byte-val))))))))))
      ...
      (let [input (: (Pointer U8)) (pointer-field-read tok input)]
        (let [start-ptr (: (Pointer U8)) (cast (Pointer U8) (+ (cast I64 input) (cast I64 start-pos)))]
          ...
          (let [_ (: I32) (advance tok)]
            (let [len (: I32) 1]  ; Start with 1 for opening quote
              ;; Read until closing quote
              (while (and (!= (peek-char tok) 0)
                         (!= (peek-char tok) 34))  ; 34 is "
                (advance tok)
                (set! len (+ len 1)))
              ;; Advance past closing quote and include it in length
              (let [_ (: I32) (advance tok)]
