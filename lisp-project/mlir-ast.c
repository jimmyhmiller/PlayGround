...
typedef struct {
    ...
    int32_t (*main_fn)();
} Namespace_src_mlir_ast;

Namespace_src_mlir_ast g_src_mlir_ast;
...
void init_namespace_src_mlir_ast(Namespace_src_mlir_ast* ns) {
...
static int32_t is_op(Value* v) {
    return ({ ValueTag tag = v->tag; ((tag == ValueTag_List) ? g_src_mlir_ast.is_symbol_op(g_types.car(v)) : 0); });
}
static int32_t is_block(Value* v) {
    return ({ ValueTag tag = v->tag; ((tag == ValueTag_List) ? g_src_mlir_ast.is_symbol_block(g_types.car(v)) : 0); });
}
static Value* get_op_name(Value* op_form) {
    return ((g_src_mlir_ast.is_op(op_form) == 1) ? ({ Value* rest = (Value*)g_types.cdr(op_form); ValueTag rest_tag = rest->tag; ((rest_tag == ValueTag_List) ? g_types.car(rest) : ({ Value* __tmp_ptr = malloc(sizeof(Value)); *__tmp_ptr = g_types.make_nil(); __tmp_ptr; })); }) : ({ Value* __tmp_ptr = malloc(sizeof(Value)); *__tmp_ptr = g_types.make_nil(); __tmp_ptr; }));
}
static Value* get_op_result_types(Value* op_form) {
    return ((g_src_mlir_ast.is_op(op_form) == 1) ? ({ Value* rest = (Value*)g_types.cdr(op_form); ValueTag rest_tag = rest->tag; ((rest_tag == ValueTag_List) ? ({ Value* rest2 = (Value*)g_types.cdr(rest); ValueTag rest2_tag = rest2->tag; ((rest2_tag == ValueTag_List) ? g_types.car(rest2) : ({ Value* __tmp_ptr = malloc(sizeof(Value)); *__tmp_ptr = g_types.make_nil(); __tmp_ptr; })); }) : ({ Value* __tmp_ptr = malloc(sizeof(Value)); *__tmp_ptr = g_types.make_nil(); __tmp_ptr; })); }) : ({ Value* __tmp_ptr = malloc(sizeof(Value)); *__tmp_ptr = g_types.make_nil(); __tmp_ptr; }));
}
...
static Value* get_block_args(Value* block_form) {
    return ((g_src_mlir_ast.is_block(block_form) == 1) ? ({ Value* rest = (Value*)g_types.cdr(block_form); ValueTag rest_tag = rest->tag; ((rest_tag == ValueTag_List) ? g_types.car(rest) : ({ Value* __tmp_ptr = malloc(sizeof(Value)); *__tmp_ptr = g_types.make_nil(); __tmp_ptr; })); }) : ({ Value* __tmp_ptr = malloc(sizeof(Value)); *__tmp_ptr = g_types.make_nil(); __tmp_ptr; }));
}
static Value* get_block_operations(Value* block_form) {
    return ((g_src_mlir_ast.is_block(block_form) == 1) ? ({ Value* rest = (Value*)g_types.cdr(block_form); ValueTag rest_tag = rest->tag; ((rest_tag == ValueTag_List) ? ({ Value* rest2 = (Value*)g_types.cdr(rest); ValueTag rest2_tag = rest2->tag; ((rest2_tag == ValueTag_List) ? g_types.car(rest2) : ({ Value* __tmp_ptr = malloc(sizeof(Value)); *__tmp_ptr = g_types.make_nil(); __tmp_ptr; })); }) : ({ Value* __tmp_ptr = malloc(sizeof(Value)); *__tmp_ptr = g_types.make_nil(); __tmp_ptr; })); }) : ({ Value* __tmp_ptr = malloc(sizeof(Value)); *__tmp_ptr = g_types.make_nil(); __tmp_ptr; }));
}
static OpNode* parse_op(Value* op_form) {
    return ((g_src_mlir_ast.is_op(op_form) == 1) ? ({ Value* name_val = (Value*)g_src_mlir_ast.get_op_name(op_form); ValueTag name_tag = name_val->tag; ((name_tag == ValueTag_String) ? ({ uint8_t* name = (uint8_t*)name_val->str_val; Value* result_types = (Value*)g_src_mlir_ast.get_op_result_types(op_form); Value* operands = (Value*)g_src_mlir_ast.get_op_operands(op_form); Value* attributes = (Value*)g_src_mlir_ast.get_op_attributes(op_form); Value* regions = (Value*)g_src_mlir_ast.get_op_regions(op_form); OpNode* node = (OpNode*)((OpNode*)malloc(40)); node->name = name; node->result_types = result_types; node->operands = operands; node->attributes = attributes; node->regions = regions; node; }) : ((OpNode*)0)); }) : ((OpNode*)0));
}
static BlockNode* parse_block(Value* block_form) {
    return ((g_src_mlir_ast.is_block(block_form) == 1) ? ({ Value* args = (Value*)g_src_mlir_ast.get_block_args(block_form); Value* operations = (Value*)g_src_mlir_ast.get_block_operations(block_form); BlockNode* node = (BlockNode*)((BlockNode*)malloc(16)); node->args = args; node->operations = operations; node; }) : ((BlockNode*)0));
}
static int32_t main_fn() {
    printf("Testing is-symbol-op:\n");
    ({ Value* op_sym = (Value*)({ Value* __tmp_ptr = malloc(sizeof(Value)); *__tmp_ptr = g_types.make_symbol("op"); __tmp_ptr; }); int32_t result1 = g_src_mlir_ast.is_symbol_op(op_sym); printf("  symbol 'op': %d (expected 1)\n", result1); });
    ({ Value* block_sym = (Value*)({ Value* __tmp_ptr = malloc(sizeof(Value)); *__tmp_ptr = g_types.make_symbol("block"); __tmp_ptr; }); int32_t result2 = g_src_mlir_ast.is_symbol_op(block_sym); printf("  symbol 'block': %d (expected 0)\n", result2); });
    ({ Value* nil_val = (Value*)({ Value* __tmp_ptr = malloc(sizeof(Value)); *__tmp_ptr = g_types.make_nil(); __tmp_ptr; }); int32_t result3 = g_src_mlir_ast.is_symbol_op(nil_val); printf("  nil: %d (expected 0)\n", result3); });
    printf("\nTesting is-symbol-block:\n");
    ({ Value* block_sym = (Value*)({ Value* __tmp_ptr = malloc(sizeof(Value)); *__tmp_ptr = g_types.make_symbol("block"); __tmp_ptr; }); int32_t result4 = g_src_mlir_ast.is_symbol_block(block_sym); printf("  symbol 'block': %d (expected 1)\n", result4); });
    ({ Value* op_sym = (Value*)({ Value* __tmp_ptr = malloc(sizeof(Value)); *__tmp_ptr = g_types.make_symbol("op"); __tmp_ptr; }); int32_t result5 = g_src_mlir_ast.is_symbol_block(op_sym); printf("  symbol 'op': %d (expected 0)\n", result5); });
    printf("\nTesting is-op predicate:\n");
    ({ Value* nil_val = (Value*)({ Value* __tmp_ptr = malloc(sizeof(Value)); *__tmp_ptr = g_types.make_nil(); __tmp_ptr; }); Value* op_sym = (Value*)({ Value* __tmp_ptr = malloc(sizeof(Value)); *__tmp_ptr = g_types.make_symbol("op"); __tmp_ptr; }); Value* op_list = (Value*)g_types.make_cons(op_sym, nil_val); int32_t result6 = g_src_mlir_ast.is_op(op_list); printf("  (op): %d (expected 1)\n", result6); });
    ({ Value* nil_val = (Value*)({ Value* __tmp_ptr = malloc(sizeof(Value)); *__tmp_ptr = g_types.make_nil(); __tmp_ptr; }); Value* block_sym = (Value*)({ Value* __tmp_ptr = malloc(sizeof(Value)); *__tmp_ptr = g_types.make_symbol("block"); __tmp_ptr; }); Value* block_list = (Value*)g_types.make_cons(block_sym, nil_val); int32_t result7 = g_src_mlir_ast.is_op(block_list); printf("  (block): %d (expected 0)\n", result7); });
    printf("\nTesting is-block predicate:\n");
    ({ Value* nil_val = (Value*)({ Value* __tmp_ptr = malloc(sizeof(Value)); *__tmp_ptr = g_types.make_nil(); __tmp_ptr; }); Value* block_sym = (Value*)({ Value* __tmp_ptr = malloc(sizeof(Value)); *__tmp_ptr = g_types.make_symbol("block"); __tmp_ptr; }); Value* block_list = (Value*)g_types.make_cons(block_sym, nil_val); int32_t result8 = g_src_mlir_ast.is_block(block_list); printf("  (block): %d (expected 1)\n", result8); });
    ({ Value* nil_val = (Value*)({ Value* __tmp_ptr = malloc(sizeof(Value)); *__tmp_ptr = g_types.make_nil(); __tmp_ptr; }); Value* op_sym = (Value*)({ Value* __tmp_ptr = malloc(sizeof(Value)); *__tmp_ptr = g_types.make_symbol("op"); __tmp_ptr; }); Value* op_list = (Value*)g_types.make_cons(op_sym, nil_val); int32_t result9 = g_src_mlir_ast.is_block(op_list); printf("  (op): %d (expected 0)\n", result9); });
    printf("\nAll tests passed!\n");
    return 0;
}
void lisp_main(void) {
    init_namespace_types(&g_types);
    init_namespace_src_mlir_ast(&g_src_mlir_ast);
}
