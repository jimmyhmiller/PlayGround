...
impl Shared {
    ...
    ///
    /// A **failure is not cached**: the build can fail for reasons that go away
    /// (a `.dSYM` being regenerated by a sibling process, `dsymutil` losing a
    /// race), and an agent that remembers the first failure forever turns a
    /// transient hiccup into a permanently untyped session. Every request retries
    /// until one succeeds; a success is cached for the life of the process.
    fn with_oracle<R>(&self, f: impl FnOnce(&TypeOracle) -> R) -> Result<R, String> {
        let mut guard = self.oracle.lock().unwrap();
        if !matches!(&*guard, OracleState::Built(_)) {
        ...
        match &*guard {
            ...
            OracleState::NotBuilt => unreachable!("just built or failed"),
        }
    }
}
