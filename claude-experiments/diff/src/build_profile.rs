...
//! graph, but a production build spends real time OUTSIDE the graph too — evaluating
//! `next.config` under node, scanning and re-encoding `public/` images, copying static
//! files, running the Tailwind delegate, spawning the prerender workers. Attributing an
//! 87-second build needs a stage-level breakdown that spans all of it.
//!
...
pub fn report(label: &str, elapsed_ms: f64) {
    ...
    for (name, total) in rows {
        ...
        eprintln!("  {name:<width$}  {ms:>9.1} ms  {share:>5.1}%  x{}", total.calls);
    }
}
