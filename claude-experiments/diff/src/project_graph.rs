...
fn resolve(
    ...
    if crate::bundler::is_external_specifier(specifier) {
        return Resolution::External;
    }
    // The loader query/fragment (`./a.css?url`) is not a filesystem concern.
    let path_specifier = crate::resource_id::ResourceId::parse(specifier).path;
...
mod tests {
    ...
    fn reachability_follows_static_dynamic_and_require_edges() {
        ...
        let a = write(&root, "lib/a.ts", "const C = require(\"./c\");\nexport default C;\n");
        ...
        for expected in [&page, &a, &b, &c] {
            assert!(reachable.contains(expected), "{} not reachable", expected.display());
        }
        assert!(!reachable.contains(&orphan), "an unimported module is not reachable");
    ...
    fn an_unresolvable_specifier_is_reported_through_the_whole_chain() {
        ...
        let mid = write(&root, "lib/mid.ts", "export { deep as mid } from \"./missing\";\n");
        let graph = ProjectImportGraph::build(std::slice::from_ref(&entry), &HashMap::new(), &[]);
        assert_eq!(
            graph.first_unresolved_from(&entry),
            Some(UnresolvedImport { file: mid, specifier: "./missing".to_string() })
        );
        std::fs::remove_dir_all(&root).ok();
    }
    ...
    fn a_tsconfig_paths_alias_resolves() {
        ...
        let client = write(&root, "lib/client-layout.tsx", "export default function C() { return null; }\n");
        ...
        assert!(
            graph.reachable_from(std::slice::from_ref(&layout)).contains(&client),
    ...
    fn an_alias_resolves_exactly_as_the_bundler_resolves_it() {
        ...
        let shim = write(&root, "shims/link.tsx", "export default function Link() { return null; }\n");
        let module = write(&root, "lib/nav.tsx", "import Link from \"next/link\";\nexport default Link;\n");
        let aliases = vec![("next/link".to_string(), shim.to_string_lossy().into_owned())];
        let graph = ProjectImportGraph::build(std::slice::from_ref(&module), &HashMap::new(), &aliases);
        assert_eq!(graph.first_unresolved_from(&module), None);
