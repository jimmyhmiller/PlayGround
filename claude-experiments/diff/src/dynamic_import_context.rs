...
impl ContextCollector<'_> {
    ...
    fn rewrite(
        ...
        for request in &matches {
            arms.push_str(&format!("\n    case {0}: return import({0});", json(request)));
        }
        let message = format!(
...
mod tests {
    ...
    fn a_two_variable_pattern_expands_to_every_matching_file() {
        ...
        for expected in [
            ...
            assert!(output.contains(expected), "missing arm {expected} in {output}");
        }
        assert!(
    ...
    fn a_use_client_directive_stays_first() {
        ...
        assert!(output.contains("function __diffpack_dynimport_0"), "{output}");
    }
    ...
    fn several_patterns_get_distinct_helpers() {
        ...
        let output = transform(&importer, source).expect("rewritten");
        assert!(output.contains("function __diffpack_dynimport_0"), "{output}");
        assert!(output.contains("function __diffpack_dynimport_1"), "{output}");
    }
