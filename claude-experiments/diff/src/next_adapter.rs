...
"##;
    ...
    fn next_cache_shim_emits_revalidate_and_unstable_cache() {
        ...
        assert!(shim.contains("export function cacheTag("), "cacheTag exported");
        assert!(shim.contains("export function cacheLife("), "cacheLife exported");
        assert!(shim.contains("export function __diffpackUseCache("), "cache boundary helper exported");
        assert!(shim.contains("scope.tags.add"), "cacheTag records into the cache scope");
        assert!(
...
"#;
    ...
    fn use_cache_directive_detected_and_wrapped_in_cache_boundary() {
        ...
        // The react-server transform succeeds (no diagnostic) and emits a wrapped export.
        let result = crate::transform::transform_module(path, src, Target::ReactServer);
        assert!(
            result.diagnostics.is_empty(),
            "\"use cache\" module transforms cleanly: {:?}",
            result.diagnostics
        );
        ...
        assert!(
            result.code.contains("#data") && result.code.contains("data"),
            "the cached export is wrapped + re-exported under its original name: {}",
            result.code
        );
        // The original body is kept VERBATIM (it must run on the server, unlike use-client).
        assert!(result.code.contains("return 1"), "cached body is preserved: {}", result.code);
    }
