...
/// What the command line asked for, before the app's own framework config gets a
/// say.
///
/// `Auto` is the interesting one: it means "whatever this app's framework would
/// do", which is the only way a diffpack build of a Next app can be compared with
/// a `next build` of the same app on equal terms. Next always emits SERVER source
/// maps and gates BROWSER maps behind `productionBrowserSourceMaps`; diffpack
/// resolves `Auto` to exactly that (see `next_adapter::configure` and
/// `next_pages::configure`). `--sourcemap` / `--no-sourcemap` force the answer
/// either way, for both graphs, whatever the config says.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
pub enum SourceMapChoice {
    /// No flag: the framework's own policy decides, per graph.
    #[default]
    Auto,
    /// `--sourcemap`: emit maps for every graph.
    On,
    /// `--no-sourcemap`: emit none.
    Off,
}

impl SourceMapChoice {
    /// Read the choice out of a command line. Asking for both at once is a hard
    /// error rather than a silent precedence rule — a build that was told two
    /// incompatible things must not quietly pick one.
    pub fn from_flags<'a>(flags: impl IntoIterator<Item = &'a str>) -> Result<Self, String> {
        let mut on = false;
        let mut off = false;
        for flag in flags {
            match flag {
        ...
        match (on, off) {
            (true, true) => Err(
                "--sourcemap and --no-sourcemap were both given; pass one or neither (with \
                 neither, a Next app follows its own next.config: server maps always, browser \
                 maps when `productionBrowserSourceMaps` is set)"
                    .to_string(),
            ),
    ...
    /// Apply the choice to the default the framework config produced for one graph.
    pub fn resolve(self, framework_default: bool) -> bool {
        match self {
            Self::Auto => framework_default,
            Self::On => true,
            Self::Off => false,
        }
    }

    /// The flag that reproduces this choice, for a child process that has to be
    /// told explicitly (the instrumentation bundle, whose own `bundle` subcommand
    /// has no framework config to consult). `Auto` has no flag — it is the absence
    /// of one — so the caller must resolve it first.
    pub fn forced_flag(self) -> Option<&'static str> {
        match self {
            Self::Auto => None,
            Self::On => Some("--sourcemap"),
            Self::Off => Some("--no-sourcemap"),
        }
    }
}
...
///
/// Several stages hand the parser a source that is not the bytes on disk: the MDX
/// compile, the RSC `"use client"`/`"use server"`/`"use cache"` rewrites, the
/// TanStack route split, the `next/font` macro, `createServerFn`, and Vite's
/// `import.meta.env`/`import.meta.glob`/`define` replacements. For those modules
...
impl ModuleSourceMap {
    ...
    /// generated (a compiled component, a Vite `define`/`import.meta` rewrite),
...
mod tests {
    ...
    fn a_token_inside_rewritten_text_is_dropped_and_a_later_one_is_shifted() {
        ...
        let map = map_of(
            vec![token(0, 0, 5), token(0, 4, 5), token(0, 8, 5), token(0, 20, 5)],
            1,
        );
    ...
    fn several_replacements_on_one_line_all_land_in_the_map_s_own_coordinates() {
        ...
        let map = map_of(
            vec![token(0, 12, 9), token(0, 35, 9), token(0, 57, 9)],
            1,
        );
        let mut tokens = Vec::new();
        track.project(&map, 0, &mut tokens);
    ...
    #[test]
    fn no_flag_defers_to_the_framework_and_a_flag_overrides_it_in_both_directions() {
        let auto = SourceMapChoice::from_flags(["--minify", "production"]).unwrap();
        assert_eq!(auto, SourceMapChoice::Auto);
        assert!(
            auto.resolve(true) && !auto.resolve(false),
            "with no flag the framework's own per-graph default is what stands — this is \
             what makes a diffpack build of a Next app comparable with `next build`"
        );

        let on = SourceMapChoice::from_flags(["--sourcemap"]).unwrap();
        assert!(on.resolve(false), "--sourcemap must win over a `false` default");
        assert_eq!(on.forced_flag(), Some("--sourcemap"));

        let off = SourceMapChoice::from_flags(["--no-sourcemap"]).unwrap();
        assert!(!off.resolve(true), "--no-sourcemap must win over a `true` default");
        assert_eq!(off.forced_flag(), Some("--no-sourcemap"));

        assert_eq!(
            SourceMapChoice::Auto.forced_flag(),
            None,
            "`Auto` is the ABSENCE of a flag; a child process that cannot consult the \
             framework config must be handed a resolved answer, not this"
        );
    }

    #[test]
    fn asking_for_both_source_map_flags_is_refused_rather_than_silently_ranked() {
        let error = SourceMapChoice::from_flags(["--sourcemap", "--no-sourcemap"])
            .expect_err("a build told two incompatible things must not quietly pick one");
        assert!(
            error.contains("--sourcemap") && error.contains("--no-sourcemap"),
            "the refusal must name both flags, got: {error}"
        );
    }
