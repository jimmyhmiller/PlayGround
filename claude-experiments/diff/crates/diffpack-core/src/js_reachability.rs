...
//!
//! Integration transforms use this when pruning a module to a live subset while
//! retaining every transitive module-level dependency. One implementation keeps
//! integrations from drifting on what "reachable" means.
...
fn is_pure_expression(expression: &Expression<'_>) -> bool {
    match expression {
        ...
        Expression::TemplateLiteral(template) => {
            template.expressions.iter().all(is_pure_expression)
        }
        ...
        Expression::ArrayExpression(array) => array.elements.iter().all(|element| match element {
            ...
            other => other.as_expression().is_some_and(is_pure_expression),
        }),
