...
//! ```
//!
//! A host source policy can turn the test into a comparison of two literals; this
...
//! the generic compiler pipeline, alongside other compile-time rewrites and
//! BEFORE the module is parsed for dependencies,
...
impl<'a> Visit<'a> for BranchCollector<'_> {
    ...
    ///
    /// ```js
    /// var Devtools = process.env.NODE_ENV !== "development" ? function () { return null; } : RealDevtools;
    /// ```
    ///
    ...
    fn visit_conditional_expression(
        ...
        if let Some(test) = evaluate(&conditional.test) {
            ...
            self.edits.push((conditional.span, format!("({text})")));
            return;
        }
...
fn literal_value(expression: &Expression<'_>) -> Option<LiteralValue> {
    match expression {
        Expression::StringLiteral(literal) => Some(LiteralValue::String(literal.value.to_string())),
...
mod tests {
    ...
    fn a_dead_branch_without_an_else_becomes_nothing() {
        let out =
            run("before(); if (\"production\" !== 'production') { dev(); } after();").unwrap();
    ...
    /// module: for example `if (!process.browser) { var fs =
    ...
    fn every_name_a_destructuring_var_binds_is_kept() {
        ...
        assert!(
            out.contains("var a,b,c;") || out.contains("var a,c;"),
            "{out}"
        );
        assert!(!out.contains("require("), "{out}");
    }
    ...
    fn a_function_declaration_in_the_dead_branch_is_kept_as_a_declaration() {
        let out =
            run("if (\"a\" === 'b') { function hoisted() { require('heavy'); } }\nuse(hoisted);")
    ...
    fn a_var_inside_a_nested_function_does_not_block_elimination() {
        ...
        let out =
            run("if (\"a\" === 'b') { call(() => { var local = 1; return local; }); }").unwrap();
    ...
    fn an_else_if_that_folds_to_nothing_removes_the_dangling_else() {
        let out =
            run("if (x) { a(); } else if (\"production\" !== 'production') { b(); }").unwrap();
        ...
        assert!(
            out.contains("else {c();}") || out.contains("else { c(); }"),
            "{out}"
        );
    }
    ...
    fn an_else_if_chain_keeps_the_surviving_arm() {
        let out = run(
            "if (\"a\" === 'b') { one(); } else if (\"c\" === 'c') { two(); } else { three(); }",
        )
        .unwrap();
