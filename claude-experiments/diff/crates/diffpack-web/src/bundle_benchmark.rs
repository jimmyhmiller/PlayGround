...
use diffpack_core::frontend_profile;
use diffpack_default_loader::driver::{Bundler, EmitOptions, partition_diagnostics};
...
fn discover_direct(
    entry: &std::path::Path,
) -> Result<(Bundler, diffpack_core::BuildUpdate), String> {
    crate::compiler::discover(entry, &BuildConfig::default())
}
...
pub fn write_live_scale_visualization(
    ...
    let (mut bundler, update) = discover_direct(&entry)?;
...
fn run_bundle_scale_inner_impl(
    ...
    let (mut bundler, initial) = discover_direct(&entry)?;
...
pub fn run_bundle_scale_memory(
    ...
    diffpack_core::memory::enable_accounting();
    // Baseline after generation, before any graph exists.
    let baseline = diffpack_core::memory::snapshot().live_bytes;
    diffpack_core::memory::reset_peak();
    ...
    let after_build = diffpack_core::memory::snapshot();
    ...
    for edit in 0..edits {
        ...
        rendered_chunks_per_edit_max = rendered_chunks_per_edit_max.max(emit_stats.rendered_chunks);
    }

    let render_cache_entries = bundler.render_cache_len();
    let live_after_edits = diffpack_core::memory::snapshot().live_bytes;
    let retained_growth_over_edits_bytes = live_after_edits.saturating_sub(live_after_build);

    drop(reachable);
    drop(reachability);
    drop(bundler);
    let retained_after_drop_bytes = diffpack_core::memory::snapshot()
        .live_bytes
