...
pub fn parse(source: &str, origin: &str) -> Result<HtmlEntry, String> {
    ...
    while let Some(offset) = find_ci(source, cursor, "<") {
        ...
        if source[offset..].starts_with("<!--") {
            let close = find_ci(source, offset + 4, "-->")
                .map(|at| at + 3)
                .ok_or_else(|| {
                    format!(
                        "{origin}:{}: unterminated HTML comment",
        ...
        module_scripts.push(ModuleScript {
            src,
            start: offset,
            end: close,
        });
...
impl HtmlEntry {
    ...
    pub fn rewrite(&self, origin: &str, injection: &HeadInjection) -> Result<String, String> {
        ...
        let line_start = output[..head_close]
            .rfind('\n')
            .map(|at| at + 1)
            .unwrap_or(0);
...
<script type="module" src="//cdn.example.com/y.js"></script>
    ...
    fn a_commented_out_module_script_is_ignored() {
        let source =
            "<head></head><body><!-- <script type=\"module\" src=\"/gone.js\"></script> --></body>";
        let entry = parse(source, "index.html").unwrap();
        assert!(entry.module_scripts.is_empty());
    }
    ...
    fn rewrite_moves_the_entry_into_head_and_removes_the_original() {
        ...
        assert!(
            !built.contains("/src/main.tsx"),
            "original tag removed: {built}"
        );
        let head_close = built.find("</head>").unwrap();
        assert!(
            built.find("/index.js").unwrap() < head_close,
            "script is inside head: {built}"
        );
        assert!(
            !built.contains("\n\n\n"),
    ...
    fn a_document_without_head_is_a_hard_error() {
        ...
        .unwrap();
        let error = entry
            .rewrite("x.html", &HeadInjection::default())
            .unwrap_err();
...
pub fn apply_base(html: &str, base: &str) -> String {
    ...
    for attribute in ["src=\"/", "href=\"/"] {
        ...
        while let Some(found) = out[cursor..].find(attribute) {
            ...
            if out[value_start..].starts_with('/') || out[value_start..].starts_with(&base[1..]) {
...
mod base_tests {
    ...
    fn public_rooted_references_gain_the_base_and_protocol_relative_do_not() {
        ...
        assert!(
            rebased.contains("src=\"//cdn.example.com/x.js\""),
            "{rebased}"
        );
