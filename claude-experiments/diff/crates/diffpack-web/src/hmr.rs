...
use diffpack_core::parser::{self, JsxExtensions};
...
/// All web-owned substitutions used by the generic registry renderer in an HMR
/// build. Keeping this decision together prevents core rendering from knowing
/// the HMR protocol or its global names.
pub struct RegistryRenderPolicy {
    pub require_dynamic: &'static str,
    pub hot_install: &'static str,
    pub methods: String,
    pub runtime_return: String,
    pub reimport_guard: &'static str,
    pub server_control: &'static str,
}

pub fn registry_render_policy(
    entry_runtime_id: usize,
    any_async: bool,
    node_esm: bool,
) -> RegistryRenderPolicy {
    let pending_lookup = if any_async {
        "function __hmrPending(id){return __pending[id];}"
    } else {
        "function __hmrPending(){return undefined;}"
    };
    let methods = format!("const __entryId={entry_runtime_id};{pending_lookup}{RUNTIME_METHODS}");
    let require_async_export = if any_async {
        "requireAsync:__requireAsync,"
    } else {
        ""
    };
    let runtime_return = format!(
        ...
        serde_json::to_string(RUNTIME_GLOBAL).expect("static HMR runtime name is valid JSON")
    );
    RegistryRenderPolicy {
        require_dynamic: if any_async {
            REQUIRE_DYNAMIC_ESM_HMR_ASYNC
        } else {
            REQUIRE_DYNAMIC_ESM_HMR
        },
        hot_install: "module.hot=__makeHot(id);",
        methods,
        runtime_return,
        reimport_guard: REIMPORT_GUARD,
        server_control: if node_esm { SERVER_CONTROL } else { "" },
    }
}

/// Re-bases a compiler map across `import.meta.hot` rewriting and the injected
/// Fast Refresh prelude. Appended refresh footers do not move existing lines.
pub fn rebase_refresh_map(
    map: &mut Option<diffpack_core::source_map::ModuleSourceMap>,
    before: &str,
    preamble_lines: u32,
    hot_rewritten: bool,
) {
    use diffpack_core::source_map::LineTrack;
    ...
    if hot_rewritten {
        ...
        let _ = diffpack_core::source_map::replace_tracked(
            before,
            "import.meta.hot",
            "module.hot",
            &mut edits,
...
pub fn stylesheet_fingerprint(output_root: &std::path::Path, file: &str) -> Option<u64> {
    use std::hash::{Hash, Hasher};

    let bytes = std::fs::read(output_root.join(file)).ok()?;
    let mut hasher = std::collections::hash_map::DefaultHasher::new();
    bytes.hash(&mut hasher);
    Some(hasher.finish())
}

pub fn is_stylesheet_module(id: &str) -> bool {
...
pub fn push_css_update(hrefs: &[String], hub: &crate::websocket::HmrHub) {
    let list = hrefs
        .iter()
        .map(|href| serde_json::to_string(href).expect("stylesheet URL is valid JSON"))
        .collect::<Vec<_>>()
        .join(",");
...
pub fn is_refresh_boundary(
    ...
    if !parser::source_type_for(real_path, jsx).is_jsx() {
        return false;
    }
    ...
    exports.iter().all(|name| {
        name == "default" || name.chars().next().is_some_and(|c| c.is_ascii_uppercase())
    })
}
...
pub fn rewrite_import_meta_hot(code: &str) -> String {
...
mod tests {
    ...
    fn the_refresh_runtime_is_resolved_up_the_node_modules_chain() {
        ...
        let source =
            find_refresh_runtime(&app).expect("the hoisted install one level up must be found");
        assert!(
    ...
    fn next_own_fast_refresh_runtime_is_composed_and_answers_the_boundary_check() {
        ...
        let core = std::fs::read_to_string(
            node_modules
                .join("next/dist/compiled/react-refresh/cjs/react-refresh-runtime.development.js"),
        )
        .unwrap();
    ...
    fn a_missing_refresh_runtime_names_every_layout_and_directory_searched() {
        ...
        assert!(
            error.contains("@vitejs/plugin-react/dist/refresh-runtime.js"),
            "{error}"
        );
        assert!(
            error.contains("@vitejs/plugin-react/dist/refreshUtils.js"),
            "{error}"
        );
        ...
        assert!(
            error.contains(&directory.path().join("node_modules").display().to_string()),
    ...
    fn footer_passes_flattened_exports_to_the_boundary_check() {
        ...
        assert!(
            footer.contains("__flat"),
            "footer must define a flatten helper: {footer}"
        );
        assert!(
        ...
        assert!(
            footer.contains(
                "validateRefreshBoundaryAndEnqueueUpdate({key},__flat("
    ...
    fn overlay_script_installs_listeners_and_a_vlq_consumer() {
        ...
        assert!(
            script.contains("window.__diffpackOverlay"),
            "must publish the overlay: {script}"
        );
        assert!(
        ...
        assert!(
            script.contains("function decodeVlq"),
            "must define a VLQ decoder: {script}"
        );
        assert!(
        ...
        assert!(
            script.contains("function mapStack"),
            "must map stacks to original source: {script}"
        );
    ...
    fn client_script_dispatches_build_error_and_build_ok() {
        ...
        assert!(
            script.contains("msg.type===\"build-ok\"")
                && script.contains("__diffpackOverlay.clear()"),
    ...
    fn prune_registers_callbacks_and_the_runtime_can_fire_them() {
        ...
        let m = RUNTIME_METHODS;
        assert!(
            m.contains("prune(cb){__hmrEntry(id).pruners.push(cb);}"),
            "prune must register: {m}"
        );
        let body = m
            ...
            .unwrap_or("");
        assert!(
            body.contains("vite:beforePrune"),
            "prune must emit vite:beforePrune: {body}"
        );
        for cleared in [
            "delete __cache[id]",
            "delete __hmrEntries[id]",
            ...
            assert!(
                ...
                "prune must forget the module ({cleared}): {body}"
            );
        }
    }
    ...
    fn apply_emits_before_and_after_update_events() {
        ...
        let m = RUNTIME_METHODS;
        assert!(
            m.contains("vite:beforeUpdate"),
            "apply must emit vite:beforeUpdate: {m}"
        );
        assert!(
            m.contains("vite:afterUpdate"),
            "apply must emit vite:afterUpdate: {m}"
        );
        assert!(
...
"#;
        ...
        assert_eq!(
            got["applied"], true,
            "the walk must reach the accepting boundary: {got}"
        );
    ...
    fn a_single_named_component_export_is_a_refresh_boundary() {
        ...
        assert!(
            boundary,
            "a lone uppercase named component export must be a boundary"
        );
    }
    ...
    fn two_modules_with_the_same_component_name_register_under_different_family_ids() {
        ...
        assert_eq!(
            ids.len(),
            2,
            "both modules must register their component: {ids:?}"
        );
        assert_ne!(
            ids[0], ids[1],
    ...
    #[test]
    fn registry_policy_keeps_hmr_protocol_details_in_web() {
        let policy = registry_render_policy(7, true, true);
        assert!(policy.require_dynamic.contains("__requireAsync"));
        assert!(policy.methods.contains("const __entryId=7"));
        assert!(policy.runtime_return.contains(RUNTIME_GLOBAL));
        assert!(policy.server_control.contains("DIFFPACK_HMR_CONTROL_PORT"));
    }
}
