...
/// supplied per environment by the caller.
...
impl ImportMetaEnv {
    ...
    fn value_for(&self, key: &str, ssr: bool) -> String {
        match key {
            "MODE" => json(&self.mode),
...
pub fn transform(path: &Path, source: &str, options: &ImportMetaEnv, ssr: bool) -> Option<String> {
...
impl<'a> Visit<'a> for EnvCollector<'_> {
    fn visit_static_member_expression(
        ...
        if is_import_meta_env(&member.object) {
            let value = self
                .options
                .value_for(member.property.name.as_str(), self.ssr);
        ...
        if member.property.name == "env" && is_import_meta(&member.object) {
            self.edits
                .push((member.span, self.options.object_literal(self.ssr)));
            return;
        }
...
mod tests {
    ...
    fn run(source: &str, ssr: bool) -> String {
        transform(Path::new("m.tsx"), source, &env(), ssr).unwrap_or_else(|| source.to_string())
    }

    #[test]
    fn known_keys_fold_to_literals_for_dce() {
        let out = run(
            "const a = import.meta.env.PROD; const b = import.meta.env.DEV; const c = import.meta.env.MODE;",
            false,
        );
    ...
    fn ssr_depends_on_target() {
        assert!(run("x(import.meta.env.SSR)", true).contains("x(true)"));
        assert!(run("x(import.meta.env.SSR)", false).contains("x(false)"));
    }

    #[test]
    fn vite_var_uses_its_value_and_unknown_is_undefined() {
        let out = run(
            "const u = import.meta.env.VITE_API; const m = import.meta.env.VITE_MISSING || '';",
            false,
        );
        assert!(
            out.contains("const u = \"https://api.example.com\""),
            "{out}"
        );
    ...
    fn bare_reference_becomes_the_object_so_access_never_throws() {
        ...
        let out = run(
            "const e = import.meta.env; const url = e.VITE_MISSING || '';",
            false,
        );
        assert!(out.contains("\"MODE\":\"production\""), "{out}");
        assert!(out.contains("\"PROD\":true"), "{out}");
        assert!(
            out.contains("\"VITE_API\":\"https://api.example.com\""),
            "{out}"
        );
    }
    ...
    fn computed_access_replaces_the_receiver() {
        let out = run("const v = import.meta.env[key];", false);
        assert!(out.contains("[key]"), "{out}");
        assert!(
            out.contains("\"BASE_URL\":\"/\""),
            "receiver became the object: {out}"
        );
    }
    ...
    fn a_module_without_import_meta_env_is_untouched() {
        ...
        assert!(transform(Path::new("m.ts"), "export const x = 1", &env(), true).is_none());
    }
}
