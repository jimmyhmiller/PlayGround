...
impl GlobCollector<'_> {
    ...
    fn rewrite(&mut self, call: &CallExpression<'_>) -> Result<String, String> {
        ...
        let first = arguments.next().ok_or_else(|| {
            format!(
                "{}: import.meta.glob requires a pattern argument",
                self.path.display()
            )
        })?;
        ...
        for (index, (key, specifier)) in entries.iter().enumerate() {
            ...
            let value = if options.eager {
                ...
                self.prelude.push(import_statement(
                    &identifier,
                    options.import.as_deref(),
                    &specifier,
                ));
                identifier
            } else {
...
mod tests {
    ...
    fn fixture() -> (tempfile::TempDir, PathBuf, ImportMetaGlob) {
        ...
        std::fs::write(
            root.join("src/widgets/alpha.ts"),
    ...
    fn eager_glob_hoists_static_imports_after_the_directive_prologue() {
        ...
        assert!(
            out.contains("\"./widgets/alpha.ts\": __diffpack_glob_0_0"),
            "{out}"
        );
    }
    ...
    fn import_option_selects_the_export_in_both_modes() {
        ...
        .unwrap();
        assert!(
            eager.contains("import __diffpack_glob_0_0 from \"./widgets/alpha.ts\";"),
            "{eager}"
        );
        let lazy =
    ...
    fn bare_specifier_pattern_is_a_hard_error_naming_the_file() {
        ...
        assert!(error.contains("entry.ts"), "{error}");
        assert!(
            error.contains("must start with './', '../', or '/'"),
            "{error}"
        );
    }
    ...
    fn unsupported_options_are_hard_errors_naming_the_file() {
        ...
        assert!(error.contains("entry.ts"), "{error}");
        assert!(
            error.contains("unknown import.meta.glob option `unknown`"),
            "{error}"
        );
        ...
        assert!(
            error.contains("`eager` must be a boolean literal"),
            "{error}"
        );
