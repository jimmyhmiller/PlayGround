...
use diffpack_default_loader::loader::{self, LoaderKind};
...
use diffpack_core::js_reachability::{
    collect_references, item_of, local_symbol, push_unique, top_level_items, within,
};

/// Returns the route id attributed to a `?tsr-split=*` chunk, or `None` for a
/// chunk not produced by TanStack route splitting.
pub fn chunk_route_id(id: &str) -> Result<Option<String>, String> {
    let resource = ResourceId::parse(id);
    if loader::kind(&resource) != Some(LoaderKind::TsrSplit) {
        return Ok(None);
    }
    let path = Path::new(&resource.path);
    let source = fs::read_to_string(path)
        .map_err(|error| format!("cannot read route file {}: {error}", path.display()))?;
    route_id(path, &source).map(Some).ok_or_else(|| {
        format!(
...
/// Decodes and loads a `?tsr-split=<property>` virtual module request.
pub fn load_split_module(resource: &ResourceId) -> Result<String, String> {
    let target = resource
        .query
        .as_deref()
        .and_then(|query| query.strip_prefix("tsr-split="))
        .unwrap_or("");
    let path = Path::new(&resource.path);
    let source = fs::read_to_string(path)
        .map_err(|error| format!("cannot read route file {}: {error}", path.display()))?;
    build_split_module(path, &source, target)
}

/// Loads and compiles the TanStack-owned split query into a graph module.
pub fn compile_split_module(
    resource: &ResourceId,
    compile: impl FnOnce(&Path, &str) -> TransformResult,
) -> Result<SpecialModule, String> {
    let source = load_split_module(resource)?;
    Ok(diffpack_default_loader::module::virtual_module_at(
...
pub fn split_reference_route(path: &Path, source: &str) -> Option<String> {
    ...
    for (import_span, specifier_symbols) in import_statements(program) {
        if !specifier_symbols.is_empty()
            && specifier_symbols
                .iter()
                .all(|symbol| !used.contains(symbol))
    ...
    for statement in &program.body {
        ...
        let used_elsewhere = refs.iter().any(|(reference_span, referenced)| {
            *referenced == symbol
                && !value_spans
                    .iter()
                    .any(|span| within(*reference_span, *span))
        });
        if used_elsewhere {
            return None;
        }
...
mod tests {
    ...
    fn splits_a_local_component_out_of_the_reference_file() {
        ...
        assert!(rewritten.contains(
            "const $$splitComponentImporter = () => import('./index.tsx?tsr-split=component');"
        ));
    ...
    fn virtual_module_exports_the_component() {
        let source = "import { createFileRoute } from '@tanstack/react-router'\n\
             ...
             function Home() {\n  return <div>hi</div>\n}\n";
        let module =
            build_split_module(Path::new("/app/src/routes/index.tsx"), source, "component")
    ...
    fn virtual_module_keeps_the_route_when_the_component_uses_it() {
        ...
        let module =
            build_split_module(Path::new("/app/src/routes/posts.tsx"), source, "component")
                .unwrap();
        ...
        assert!(
            module.contains("export const Route = createFileRoute('/posts')({})"),
            "{module}"
        );
        assert!(!module.contains("fetchPosts"), "{module}");
        assert!(module.contains("Link"), "{module}");
        assert!(
            module.contains("export { PostsComponent as component };"),
            "{module}"
        );
    }
    ...
    fn non_route_files_are_left_alone() {
        assert!(
            split_reference_route(Path::new("/app/src/utils/posts.ts"), "export const x = 1")
                .is_none()
        );
    ...
    fn splits_every_target_property_into_its_own_lazy_chunk() {
        ...
        assert!(
            rewritten
                .contains("import { lazyFn, lazyRouteComponent } from '@tanstack/react-router';"),
            "{rewritten}"
        );
        assert!(
            rewritten.contains("lazyFn($$splitLoaderImporter, 'loader')"),
            "{rewritten}"
        );
        assert!(
            rewritten.contains("lazyRouteComponent($$splitComponentImporter, 'component')"),
            "{rewritten}"
        );
        assert!(
            rewritten
                .contains("lazyRouteComponent($$splitErrorComponentImporter, 'errorComponent')"),
            "{rewritten}"
        );
        assert!(
            rewritten.contains("import('./posts.$postId.tsx?tsr-split=loader')"),
            "{rewritten}"
        );
        assert!(
            rewritten.contains("import('./posts.$postId.tsx?tsr-split=errorComponent')"),
            "{rewritten}"
        );
        // Every split-out definition (and its only-used-here import) leaves the
        // reference file.
        assert!(
            !rewritten.contains("function PostComponent()"),
            "{rewritten}"
        );
        ...
        .unwrap();
        assert!(
            error.contains("export { PostError as errorComponent };"),
            "{error}"
        );
    }
    ...
    fn extracts_the_route_id_from_the_factory_argument() {
        ...
        assert_eq!(
            route_id(Path::new("/app/src/utils/x.ts"), "export const x = 1"),
            None
        );
    }
    ...
    fn an_unrecognized_split_target_is_a_hard_error() {
        let source = "import { createFileRoute } from '@tanstack/react-router'\n\
             ...
             function Home() {\n  return <div />\n}\n";
        let error =
            build_split_module(Path::new("/app/src/routes/index.tsx"), source, "beforeLoad")
