...
use oxc_ast::ast::{Declaration, Expression, ImportDeclarationSpecifier, Program, Statement};
use oxc_parser::Parser;
use oxc_semantic::SemanticBuilder;
use oxc_span::{GetSpan, Span};

pub(crate) use diffpack_core::text_edit::{apply_edits, quote};

use diffpack_core::js_reachability::dead_declaration_spans;
...
pub fn transform_server_fns(
    ...
    for server_fn in &found {
        ...
        if client {
            ...
            edits.push((
                server_fn.handler_arg,
                format!("createClientRpc({})", quote(&id)),
            ));
        } else {
    ...
    if client {
        let scoping = SemanticBuilder::new()
            ...
            .semantic
            .into_scoping();
        let removed: Vec<Span> = found
            .iter()
            .map(|server_fn| server_fn.handler_arg)
            .collect();
    ...
    let import = if client {
        format!(
            "import {{ createClientRpc }} from {};\n",
            quote(CLIENT_RPC_SPECIFIER)
        )
    } else {
        format!(
            "import {{ createServerRpc }} from {};\n",
...
pub fn generate_resolver_module(entries: &[ServerFnEntry]) -> String {
    ...
    source.push_str(
        "// Generated natively by Diffpack from the app's createServerFn declarations.\n",
    );
...
mod tests {
    ...
    fn function_id_is_deterministic_and_opaque() {
        ...
        assert_ne!(
            a,
            function_id(Path::new("/app/src/utils/users.tsx"), "fetchPosts")
        );
    ...
    fn client_build_replaces_the_handler_with_an_rpc_stub() {
        ...
        let rewritten = transform_server_fns(Path::new("/app/src/utils/posts.tsx"), source, true)
            .unwrap()
        ...
        assert!(
            rewritten.contains(&format!("createClientRpc(\"{id}\")")),
            "{rewritten}"
        );
    ...
    fn client_build_sweeps_server_only_code_the_handler_orphaned() {
        ...
        let client = transform_server_fns(path, source, true)
            .unwrap()
            .expect("the module has server fns");

        // The server-only import, helper, and constant are gone from the client.
        assert!(
            !client.contains("node:fs"),
            "node:fs must not ship to the client:\n{client}"
        );
        assert!(
            !client.contains("readCount"),
            "the server helper must be swept:\n{client}"
        );
        assert!(
            !client.contains("count.txt"),
            "the server constant must be swept:\n{client}"
        );
        assert!(
            !client.contains("writeFile"),
            "the handler body must be gone:\n{client}"
        );

        // The client RPC surface and the live route code are kept intact.
        assert!(client.contains("createClientRpc"), "{client}");
        assert!(
            client.contains("createServerFn"),
            "the client server-fn wrapper stays:\n{client}"
        );
        assert!(
            client.contains(".validator("),
            "the validator runs client-side:\n{client}"
        );
        assert!(client.contains("createFileRoute"), "{client}");
        assert!(
            client.contains("useRouter"),
            "the live component's imports stay:\n{client}"
        );
    ...
    fn client_dce_keeps_server_only_code_still_used_by_client_code() {
        ...
        assert!(
            client.contains("import { format }"),
            "shared import must stay:\n{client}"
        );
    ...
    fn client_dce_keeps_a_side_effect_import() {
        ...
        let client = transform_server_fns(Path::new("/app/src/routes/y.tsx"), source, true)
            .unwrap()
            .expect("the module has a server fn");
        assert!(
            client.contains("import './styles.css'"),
            "side-effect import must stay:\n{client}"
        );
    }
    ...
    fn server_build_does_not_sweep_server_only_code() {
        ...
        let server = transform_server_fns(Path::new("/app/src/routes/index.tsx"), source, false)
            .unwrap()
            .expect("the module has a server fn");
        assert!(
            server.contains("node:fs"),
            "server keeps its imports:\n{server}"
        );
        assert!(
            server.contains("count.txt"),
            "server keeps its constants:\n{server}"
        );
    }
    ...
    fn server_build_keeps_the_handler_and_registers_it() {
        ...
        let path = Path::new("/app/src/utils/posts.tsx");
        let rewritten = transform_server_fns(path, source, false)
            .unwrap()
    ...
    fn a_local_same_named_create_server_fn_is_not_rewritten() {
        ...
        assert!(
            transform_server_fns(Path::new("/app/src/x.ts"), source, true)
                .unwrap()
                .is_none(),
    ...
    fn a_module_without_server_fns_is_left_alone() {
        assert!(
            transform_server_fns(Path::new("/app/src/x.ts"), "export const x = 1", true)
                .unwrap()
                .is_none()
        );
    }
    ...
    fn client_stub_and_server_registration_agree_on_the_id() {
        ...
        let client = transform_server_fns(path, source, true).unwrap().unwrap();
        ...
        assert!(
            client.contains(&format!("createClientRpc(\"{id}\")")),
            "{client}"
        );

        let server = transform_server_fns(path, source, false).unwrap().unwrap();
