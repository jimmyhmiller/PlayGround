...
pub fn from_bundle_graph(
    graph: &diffpack_core::bundle::IntegrationManifestGraph,
    base: &str,
) -> Result<ClientRouteManifest, String> {
    let mut routes = BTreeMap::from([(ROOT_ROUTE_ID.to_string(), vec![graph.entry_file.clone()])]);
    for chunk in &graph.chunks {
        for root in &chunk.roots {
            let Some(route_id) = crate::route_split::chunk_route_id(root)? else {
                continue;
            };
            let preloads = routes.entry(route_id).or_default();
            for file in &chunk.load_order {
                if !preloads.contains(file) {
                    preloads.push(file.clone());
                }
            }
        }
    }
    Ok(ClientRouteManifest {
        base: base.to_string(),
        entry: graph.entry_file.clone(),
        routes,
    })
}
            ...
            let files = chunks
                ...
                .map(|chunk| {
                    chunk.as_str().map(str::to_string).ok_or_else(|| {
                        format!(
                            "{}: route `{route_id}` has a non-string chunk",
                            path.display()
                        )
                    })
                })
...
mod tests {
    ...
    fn injected_head_scripts_module_exports_the_guarded_name() {
        ...
        assert!(
            source.contains("export const injectedHeadScripts = undefined;"),
            "{source}"
        );
    ...
    fn generates_the_tsr_start_manifest_contract() {
        ...
        assert!(
            source.contains("\"/\": { preloads: [\"/client.chunk-3.js\"] }"),
            "{source}"
        );
        // A route with several split chunks lists them all.
        assert!(
            source.contains(
                "\"/posts/$postId\": { preloads: [\"/client.chunk-7.js\", \"/client.chunk-8.js\"] }"
            ),
            "{source}"
        );
    }
