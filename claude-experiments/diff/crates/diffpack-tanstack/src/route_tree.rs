...
pub fn generate_for_project(project_root: &Path) -> Result<Option<usize>, String> {
    ...
    let routes_name =
        diffpack_vite_compat::vite_config::config_string(project_root, "routesDirectory")
    ...
    let ignore = RouteIgnore::parse(
        diffpack_vite_compat::vite_config::config_string(project_root, "routeFileIgnorePattern")
            .as_deref(),
    )?;
...
fn classify_filename(relative: &str) -> Result<(Vec<String>, bool), String> {
    ...
    while let Some(character) = chars.next() {
        match character {
            ...
            '[' => {
                let escaped = chars
                    .next()
                    .ok_or_else(|| "unterminated `[` escape (expected `[<char>]`)".to_string())?;
                match chars.next() {
    ...
    for segment in &segments {
        if segment.is_empty() {
            return Err("empty path segment (a stray `.`/`/` or a doubled separator)".to_string());
        }
    }
...
mod tests {
    ...
    fn generated_tree_is_semantically_equivalent_to_the_reference() {
        let tree =
            generate_from_routes_dir(&fixture_routes_dir()).expect("fixture routes must classify");
    ...
    fn generated_tree_covers_every_reference_route_id_path_and_parent() {
        ...
        let by_id: HashMap<&str, &Route> = tree
            .routes
            .iter()
        ...
        let cases: &[(&str, Option<&str>, &str)] = &[
            ...
            ("/posts_/$postId/deep", Some("/posts/$postId/deep"), ROOT_ID),
            // Escaped literal dot.
    ...
    fn route_ignore_parses_literal_alternation_and_rejects_regex() {
        ...
        assert!(
            RouteIgnore::parse(Some("(_components|api)"))
                .unwrap()
    ...
    fn custom_routes_directory_drives_the_import_prefix_and_ignore() {
        ...
        fs::write(
            app.join("settings/configuration.tsx"),
            "export const Route = {}\n",
        )
        .unwrap();
        fs::write(app.join("favicon.ico"), "not a route").unwrap();
        fs::write(
            app.join("settings/-components/Panel.tsx"),
            "export default 1\n",
        )
        .unwrap();
        ...
        let ids: Vec<&str> = tree
            .routes
            .iter()
            .map(|route| route.full_id.as_str())
            .collect();
        assert_eq!(tree.routes.len(), 2, "got routes: {ids:?}");
        assert!(ids.contains(&"/"), "{ids:?}");
        assert!(
            ids.iter().any(|id| id.contains("settings/configuration")),
            "{ids:?}"
        );
        ...
        assert!(
            emitted.contains("from './app/settings/configuration'"),
            "{emitted}"
        );
        assert!(
            !emitted.contains("./routes/"),
            "no hardcoded routes dir: {emitted}"
        );
        assert!(
            !emitted.contains("_components")
                && !emitted.contains("/api/")
                && !emitted.contains("-components"),
            "{emitted}"
        );
    }
}
