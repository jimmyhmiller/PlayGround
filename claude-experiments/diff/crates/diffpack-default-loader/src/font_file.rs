...
fn average_width(tables: &Tables<'_>, path: &Path) -> Result<Option<f64>, String> {
    ...
    let advances = HorizontalMetrics {
        hmtx,
        num_h_metrics,
        num_glyphs,
        transformed: tables.hmtx_transformed,
    };
...
impl<'a> Tables<'a> {
    ...
    fn parse_woff2(bytes: &'a [u8], path: &Path) -> Result<Self, String> {
        ...
        for _ in 0..num_tables {
            let flags = *bytes
                .get(cursor)
                .ok_or_else(|| truncated(path, "woff2 table directory"))?;
            cursor += 1;
            ...
            let tag = if known == 63 {
                ...
                *KNOWN_TABLE_TAGS.get(usize::from(known)).ok_or_else(|| {
                    format!(
                        "{}: woff2 known-table index {known} is out of range",
                        path.display()
                    )
                })?
            };
            ...
            let length = if transformed {
                read_base128(bytes, &mut cursor, path)?
            } else {
                orig_length
            };
        ...
        brotli_decompressor::BrotliDecompress(&mut &compressed[..], &mut decompressed).map_err(
            |error| {
                format!(
                    "{}: the woff2 Brotli stream will not decompress: {error}",
                    path.display()
                )
            },
        )?;
        ...
        for (tag, length, transformed) in directory {
            let end = offset.checked_add(length).ok_or_else(|| {
                format!(
                    "{}: woff2 table `{}` overflows the stream",
                    path.display(),
                    tag_name(&tag)
                )
            })?;
...
fn read_base128(bytes: &[u8], cursor: &mut usize, path: &Path) -> Result<usize, String> {
    ...
    for index in 0..5 {
        let byte = *bytes
            .get(*cursor)
            .ok_or_else(|| truncated(path, "woff2 UIntBase128"))?;
        *cursor += 1;
        if index == 0 && byte == 0x80 {
            return Err(format!(
                "{}: woff2 UIntBase128 has a leading zero",
                path.display()
            ));
        }
        value = value
    ...
    Err(format!(
        "{}: woff2 UIntBase128 is longer than 5 bytes",
        path.display()
    ))
}
...
impl<'a> CmapLookup<'a> {
    ...
    fn parse(cmap: &'a [u8], path: &Path) -> Result<Self, String> {
        ...
        let subtable = cmap.get(chosen.2..).ok_or_else(|| {
            format!(
                "{}: cmap subtable offset {} is past the table",
                path.display(),
                chosen.2
            )
        })?;
    ...
    fn glyph(&self, code: u32) -> u16 {
        match self {
            Self::Byte(data) => {
                ...
                data.get(6 + code as usize)
                    .map(|b| u16::from(*b))
                    .unwrap_or(0)
            }
            Self::Segment(data) => segment_lookup(data, code),
            Self::Trimmed(data) => {
                let Ok(first) = be_u16(data, 6, b"cmap", Path::new("")) else {
                    return 0;
                };
                let Ok(count) = be_u16(data, 8, b"cmap", Path::new("")) else {
                    return 0;
                };
            ...
            Self::Group(data) => {
                let Ok(groups) = be_u32(data, 12, b"cmap", Path::new("")) else {
                    return 0;
                };
...
fn segment_lookup(data: &[u8], code: u32) -> u16 {
    ...
    let quiet = Path::new("");
    let Ok(seg_count_x2) = be_u16(data, 6, b"cmap", quiet) else {
        return 0;
    };
    ...
    for segment in 0..seg_count {
        let Ok(end) = be_u16(data, end_codes + segment * 2, b"cmap", quiet) else {
            return 0;
        };
        if code > end {
            continue;
        }
        let Ok(start) = be_u16(data, start_codes + segment * 2, b"cmap", quiet) else {
            return 0;
        };
        if code < start {
            return 0;
        }
        let Ok(delta) = be_u16(data, id_deltas + segment * 2, b"cmap", quiet) else {
            return 0;
        };
        ...
        let Ok(glyph) = be_u16(data, index, b"cmap", quiet) else {
            return 0;
        };
        if glyph == 0 {
            return 0;
...
fn be_u16(bytes: &[u8], offset: usize, table: &[u8; 4], path: &Path) -> Result<u16, String> {
    let slice = bytes
        .get(offset..offset + 2)
        .ok_or_else(|| short(path, table, offset))?;
...
#[doc(hidden)]
pub fn synthetic_font(units_per_em: u16, advance: u16) -> Vec<u8> {
...
#[doc(hidden)]
pub fn synthetic_font_covering(units_per_em: u16, advance: u16, start_code: u16) -> Vec<u8> {
    {
...
mod tests {
    ...
    fn stored_brotli(data: &[u8]) -> Vec<u8> {
        assert!(
            !data.is_empty() && data.len() <= 0x10000,
            "one meta-block's worth"
        );
    ...
    fn a_truncated_file_is_a_hard_error_naming_it() {
        let error =
            read_metrics_from_bytes(&[0, 1, 0, 0, 0, 3], Path::new("broken.ttf")).unwrap_err();
