...
use diffpack_core::diagnostic::{Diagnostic, DiagnosticKind};

/// Classifies a module conservatively, turning unsupported package glob policy
/// into a build diagnostic while keeping the module in the graph.
pub fn droppable_with_diagnostics(path: &Path, diagnostics: &mut Vec<Diagnostic>) -> bool {
    match is_droppable(path) {
...
mod tests {
    ...
    fn glob_array_keeps_only_matching_files() {
        ...
        assert!(
            !is_droppable(&css).unwrap(),
            "a matched CSS file has side effects"
        );

        let polyfill = root.join("src/polyfill.js");
        std::fs::write(&polyfill, "").unwrap();
        assert!(
            !is_droppable(&polyfill).unwrap(),
            "an exact-path match has side effects"
        );

        let plain = root.join("src/util.js");
        std::fs::write(&plain, "").unwrap();
        assert!(
            is_droppable(&plain).unwrap(),
            "an unmatched file is droppable"
        );
    }
}
