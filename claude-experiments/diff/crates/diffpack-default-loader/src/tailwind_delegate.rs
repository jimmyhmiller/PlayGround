...
//! Tailwind — the same shape used for MDX plugins,
...
const RUNNER: &str = include_str!("tailwind/runner.mjs");
...
/// [`diffpack_core::CancelToken::never`] and always run to completion.
pub fn compile(
    entry: &Path,
    css: &str,
    candidates: &BTreeSet<String>,
    gap: &crate::tailwind::NativeGap,
    cancel: diffpack_core::CancelToken<'_>,
) -> Result<Option<DelegatedSheet>, String> {
    let _stage = diffpack_core::build_profile::stage("css/tailwind-delegate");
    let context = format!(
...
mod tests {
    ...
    fn probe_candidates() -> BTreeSet<String> {
        ["diffpack-probe".to_string(), "flex".to_string()]
            .into_iter()
            .collect()
    }
