//! Scope-aware compile-time replacement of global identifiers with a resolved
//! value (e.g. `__OC_WEB_VERSION__` -> `"1.1.4"`).
//!
//! A define replaces a *free* identifier reference only: a local binding of the
//! same name shadows it, so the rewrite is scope-aware rather than a blind text
//! substitution. Framework profiles own the table and its precedence.
//!
...
//! literal comparison, which `diffpack_core::dead_branch` then resolves — the mechanism
...
mod tests {
    ...
    fn a_local_binding_shadows_the_define() {
        ...
        assert!(
            out.contains("function f(__DEBUG__) { return __DEBUG__; }"),
            "{out}"
        );
    ...
    fn a_property_access_is_not_a_define() {
        ...
        let out = transform(
            Path::new("m.ts"),
            "const v = obj.__OC_WEB_VERSION__;",
            &defines(),
        );
        assert!(
            out.is_none(),
            "property access must not be replaced: {out:?}"
        );
    }
    ...
    fn a_dotted_key_does_not_match_a_longer_chain_it_is_only_a_suffix_of() {
        ...
        assert!(
            out.is_none(),
            "suffix of a longer chain must not match: {out:?}"
        );
    }
