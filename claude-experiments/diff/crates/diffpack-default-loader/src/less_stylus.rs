...
pub fn compile_stylus(
    file: &Path,
    source: &str,
    root: Option<&Path>,
) -> Result<CompiledCss, String> {
    compile(Tool::Stylus, file, source, root)
}

/// Reads and compiles either a Less or Stylus file according to its extension.
pub fn compile_file(path: &Path, root: Option<&Path>) -> Result<CompiledCss, String> {
    let source = std::fs::read_to_string(path)
        .map_err(|error| format!("cannot read {}: {error}", path.display()))?;
    if is_less_path(path) {
        compile_less(path, &source, root)
    } else {
        compile_stylus(path, &source, root)
    }
}
...
fn compile(
...

    {
        ...
        stdin.write_all(source.as_bytes()).map_err(|error| {
            format!("cannot pipe source to the {} runner: {error}", tool.name())
        })?;
    }
...
mod tests {
    ...
    fn parses_runner_json_and_filters_self() {
        ...
        assert_eq!(
            ...
            vec![PathBuf::from("/proj/_vars.less")]
        );
    }
