//! Neutral `.env` syntax parsing and file loading.
//!
//! This module deliberately owns only syntax and ordered merging. Framework
//! layers choose filenames, precedence, process overlays, and client exposure.
//!
...
/// Loads an ordered list of env files, with later files overriding earlier ones.
/// The caller owns filename precedence, process-environment overlay, and exposure.
pub fn load_files(root: &Path, files: &[String]) -> Result<Vec<(String, String)>, String> {
    let mut merged: Vec<(String, String)> = Vec::new();
    let mut set = |name: String, value: String| match merged
        .iter_mut()
...
pub fn parse(text: &str, origin: &str) -> Result<Vec<(String, String)>, String> {
...
mod tests {
    ...
    fn later_files_override_earlier() {
        ...
        let vars = load_files(
            dir.path(),
            &[".env".into(), ".env.local".into(), ".env.production".into()],
        )
        .unwrap();
        assert!(
            vars.contains(&("VITE_A".to_string(), "production".to_string())),
            "{vars:?}"
        );
        assert!(
            vars.contains(&("VITE_B".to_string(), "local".to_string())),
            "{vars:?}"
        );
    ...
    fn mode_local_wins_over_mode_and_other_modes_are_ignored() {
        ...
        write(
            ...
            "VITE_X=production-local\n",
        );
        write(dir.path(), ".env.development", "VITE_X=development\n");
        let vars = load_files(
            dir.path(),
            &[".env.production".into(), ".env.production.local".into()],
        )
        .unwrap();
        assert!(
