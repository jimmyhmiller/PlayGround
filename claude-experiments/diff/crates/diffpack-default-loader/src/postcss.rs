...
/// Runs optional PostCSS while borrowing the input when no configuration exists.
pub fn process_optional<'a>(
    text: &'a str,
    path: &Path,
    postcss: Option<&Postcss>,
...
/// Adds the active PostCSS configuration to a module's watched inputs.
pub fn record_config(files: &mut Vec<PathBuf>, postcss: Option<&Postcss>) {
...
impl Postcss {
    ...
    fn run_node(&self, css: &str, from: &Path) -> Result<String, String> {
        ...
        String::from_utf8(output.stdout).map_err(|error| {
            format!(
                "PostCSS produced non-UTF-8 CSS for {}: {error}",
                from.display()
            )
        })
    }
