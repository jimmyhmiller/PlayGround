...
use diffpack_core::transform::SourceLanguage;

use crate::module::{ComponentSideEffects, PrecompiledComponent};
...
/// Compiles an SFC and delegates its optional CSS to the ordinary stylesheet
/// pipeline supplied by the caller.
pub fn precompile(
    path: &Path,
    source: &str,
    root: Option<&Path>,
    process_styles: impl FnOnce(&str) -> Result<ComponentSideEffects, String>,
) -> Result<Option<PrecompiledComponent>, String> {
    let Some(framework) = framework_for(path) else {
        return Ok(None);
    };
    let compiled = compile(framework, path, source, root)?;
    let language = match compiled.language {
        OutputLanguage::TypeScript => SourceLanguage::TypeScript,
        OutputLanguage::JavaScript => SourceLanguage::JavaScript,
    };
    let side_effects = match compiled.css.as_deref() {
        Some(css) => process_styles(css)?,
...
mod tests {
    ...
    fn blank_css_is_no_css() {
        ...
        let compiled =
            parse_output(Framework::Svelte, Path::new("/p/A.svelte"), json).expect("parse");
    ...
    fn a_missing_code_field_is_an_error() {
        ...
        let error = parse_output(Framework::Svelte, Path::new("/p/A.svelte"), json)
