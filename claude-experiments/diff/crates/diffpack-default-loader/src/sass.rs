...
//! in the same spirit as the default loader's Tailwind and CSS support.
//!
...
/// Reads and compiles an SCSS file, preserving every loaded partial as an input.
pub fn compile_file(path: &Path, options: &ScssOptions) -> Result<CompiledScss, String> {
    let source = std::fs::read_to_string(path)
        .map_err(|error| format!("cannot read {}: {error}", path.display()))?;
    compile_scss(path, &source, options)
}
    ...
    while cursor < bytes.len() {
        match bytes[cursor] {
            ...
            b'#' if bytes.get(cursor + 1) == Some(&b'{') => {
                return Err(format!("{file}: interpolation `#{{...}}` is not supported"));
            }
...
fn parse_var_decl(text: &str, bytes: &[u8], pos: &mut usize, file: &str) -> Result<Item, String> {
    ...
    if bytes.get(cursor) != Some(&b':') {
        return Err(format!("{file}: expected `:` after variable `${name}`"));
    }
    cursor += 1;
    ...
    loop {
        if let Some(stripped) = value.strip_suffix("!default") {
            ...
            return Err(format!("{file}: `!global` on `${name}` is not supported"));
        } else {
            break;
        }
    }
...
fn parse_at_rule(
    ...
    match name.as_str() {
        ...
        "mixin" => {
            ...
            if bytes.get(cursor) != Some(&b'{') {
                return Err(format!("{file}: expected `{{` after `@mixin {mixin_name}`"));
            }
            *pos = cursor + 1;
...
fn parse_use(body: &str, file: &str) -> Result<Item, String> {
    ...
    let namespace = if rest.is_empty() {
        ...
        if alias == "*" {
            ...
            return Err(format!("{file}: cannot parse `@use {path:?} as {alias}`"));
        }
    } else {
    ...
    fn prepare_include(
        ...
        let definition = match target.split_once('.') {
            ...
            None => ctx
                ...
                .get(target)
                .cloned()
                .ok_or_else(|| format!("{}: mixin `{target}` is not defined", ctx.display()))?,
        };
                ...
                for item in body {
                    match item {
                        Item::Rule { selector, body } => {
                            ...
                            ctx.scopes.push(HashMap::new());
                            let result =
                                self.eval_keyframe_body(body, ctx, &mut frames, frame_index);
                            ctx.scopes.pop();
                            result?;
                        }
                        Item::VarDecl {
    ...
    fn eval_prelude(&self, prelude: &str, ctx: &FileCtx) -> Result<String, String> {
        ...
        while index < bytes.len() {
            match bytes[index] {
                ...
                b'(' => {
                    let close = find_matching(bytes, index, b'(', b')', &ctx.display())?;
        ...
        while index < bytes.len() {
            match bytes[index] {
                ...
                b'$' => {
                    ...
                    let namespace = out.strip_suffix('.').and_then(|head| {
                        let ns_start = head
                    ...
                    let value = match namespace {
                        Some((keep, namespace)) => {
                            let value = self.lookup_namespaced_var(&namespace, name, ctx)?;
                            out.truncate(keep);
                            value
                        }
                        None => ctx.lookup(name).map(str::to_string).ok_or_else(|| {
                            format!(
...
fn resolve_selectors(selector: &str, parent: &[String], file: &str) -> Result<Vec<String>, String> {
    ...
    while index < bytes.len() {
        ...
        match byte {
            ...
            b'$' => {
                let end = ident_end(bytes, index + 1);
                if end == index + 1 {
                    return Err(format!("{file}: `$` without a variable name in {value:?}"));
                }
                tokens.push(SpacedTok {
                    spaced_before,
                    tok: Tok::Var {
            ...
            b'+' | b'-' | b'*' | b'/' | b'%' => {
                ...
                if byte == b'-' || byte == b'+' {
                    let follows_operand = next
                        .is_some_and(|next_byte| next_byte.is_ascii_digit() || next_byte == b'.');
...
impl ValueParser<'_> {
    ...
    fn parse_primary(&mut self) -> Result<Val, String> {
        ...
        match token.tok {
            ...
            Tok::Var { namespace, name } => {
                let raw =
                    match namespace {
                        ...
                        None => self.ctx.lookup(&name).map(str::to_string).ok_or_else(|| {
            ...
            Tok::Comma | Tok::Op { .. } | Tok::UnaryMinus => {
                Err(format!("{}: unexpected operator in value", self.file()))
            }
        }
    }
        ...
        match token.tok {
            ...
            Tok::Var { namespace, name } => {
                let raw = match namespace {
                    Some(namespace) => self.value.compiler.lookup_namespaced_var(
                        &namespace,
                        &name,
                        self.value.ctx,
                    )?,
                    None => self
                        ...
                        .lookup(&name)
                        .map(str::to_string)
                        .ok_or_else(|| format!("{}: undefined variable `${name}`", self.file()))?,
                };
...
mod tests {
    ...
    fn media_with_nested_rule_inside_rule() {
        let css = compile(".wrap { @media (min-width: 10px) { .inner { color: red; } } }\n");
        assert_eq!(
            flat(&css),
    ...
    fn mixin_without_arguments() {
        ...
        assert_eq!(flat(&css), "h1 { color: red; font-size: 5rem; margin: 0; }");
    }
    ...
    fn mixin_local_variable() {
        ...
        assert_eq!(flat(&css), "body { background-size: 12rem 12rem; }");
    }
    ...
    fn keyword_arguments_are_rejected() {
        let error =
            compile_error("@mixin m($a: 1px) { width: $a; }\n.x { @include m($a: 2px); }\n");
    ...
    fn data_uri_with_slashes_survives() {
        let css =
            compile(".a { cursor: url(\"data:image/svg+xml;base64,PHN2//Zz4K+A==\"), auto; }\n");
        assert_eq!(
            flat(&css),
    ...
    fn use_with_namespace_variables_and_mixins() {
        ...
        let compiled = compile_scss(&root, &source, &ScssOptions::default()).expect("compiles");
        assert_eq!(
            flat(&compiled.css),
    ...
    fn additional_data_prepends_to_root() {
        ...
        let source = fs::read_to_string(&root).unwrap();
        let options = ScssOptions {
            additional_data: Some("@use \"/src/global/theme.scss\" as theme;".to_string()),
            root: Some(dir.path().to_path_buf()),
        };
        let compiled = compile_scss(&root, &source, &options).expect("compiles");
    ...
    fn css_import_passes_through() {
        ...
        assert_eq!(flat(&css), "@import './plain.css'; .a { color: red; }");
    }
    ...
    fn module_loop_is_an_error() {
        ...
        let source = fs::read_to_string(&root).unwrap();
        let error =
            compile_scss(&root, &source, &ScssOptions::default()).expect_err("loop should fail");
