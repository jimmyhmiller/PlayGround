...
use diffpack_core::transform::{DecoratorConfig, JsxConfig, JsxRuntime, ProjectConfig};

/// Resolves the JSX/decorator compilation contract for one source file. An
/// integration-generated file can opt out of the application's project config
/// while still receiving explicit build overrides.
pub fn project_config(
    ...
    if generated {
        return Ok(ProjectConfig {
            jsx: JsxConfig::default().overridden_by(overrides),
            decorators: DecoratorConfig::default(),
        });
    }
    let tsconfig = owning_config(resolver, path)?;
    let mut resolved = JsxConfig::default();
    let mut decorators = DecoratorConfig::default();
    if let Some(tsconfig) = tsconfig {
        ...
        resolved.runtime = match compiler_options.jsx.as_deref() {
            None => None,
            Some("react-jsx" | "react-jsxdev") => Some(JsxRuntime::Automatic),
            Some("react") => Some(JsxRuntime::Classic),
            Some("preserve" | "react-native") => Some(JsxRuntime::Automatic),
            Some(other) => {
                return Err(format!(
        ...
        resolved
            .import_source
            .clone_from(&compiler_options.jsx_import_source);
    ...
    while let Some(current) = directory {
        for name in CONFIG_FILE_NAMES {
            ...
            let config = resolver.resolve_tsconfig(&config_path).map_err(|error| {
                format!(
                    "cannot read {} (it configures how {}'s JSX is lowered): {error}",
                    config_path.display(),
                    path.display()
                )
            })?;
...
fn covers(config: &TsConfig, path: &Path) -> bool {
    ...
    !config.exclude.as_ref().is_some_and(|patterns| {
        patterns
            .iter()
            .any(|pattern| pattern_covers(pattern, &candidates))
    })
