...
use crate::asset::{asset_public_name, content_hash};
...
fn flush_own(own: &mut Vec<u8>, file: &Path, assets: &mut Vec<CssAsset>) -> Result<String, String> {
    let text = String::from_utf8(std::mem::take(own))
...
fn parse_import_statement(
    ...
    if body.is_empty() {
        return Err(format!(
            "@import in {display} has no target ({statement:?})"
        ));
    }
...
fn rewrite_urls(css: &str, file: &Path, assets: &mut Vec<CssAsset>) -> Result<String, String> {
    ...
    while index < bytes.len() {
        match bytes[index] {
            ...
            {
                let close =
                    find_matching(bytes, index + 3, b'(', b')', &file.display().to_string())?;
...
impl ModuleParser<'_> {
    ...
    fn copy_trivia(&mut self) {
        loop {
            ...
            self.out.extend_from_slice(&self.bytes[self.pos..advanced]);
            self.pos = advanced;
    ...
    fn parse_items(&mut self, top: bool) -> Result<(), String> {
        loop {
            ...
            match self.bytes[self.pos] {
                b'}' => {
                    if top {
                        return Err(format!("unmatched `}}` in CSS module {}", self.display));
                    }
                    return Ok(());
                }
    ...
    fn parse_keyframes(&mut self, at: usize, name_start: usize) -> Result<(), String> {
        ...
        if starts_with_ci(self.bytes, cursor, ":global(") {
            ...
            self.out.extend_from_slice(self.scoped(&name).as_bytes());
            cursor = end;
        }
    ...
    fn parse_style_rule(&mut self) -> Result<(), String> {
        ...
        let brace = loop {
            ...
            match self.bytes[cursor] {
                ...
                b'(' | b'[' => {
                    let close = if self.bytes[cursor] == b'(' {
                        b')'
                    } else {
                        b']'
                    };
...
mod tests {
    ...
    fn scopes_classes_across_combinators_pseudos_and_comma_lists() {
        ...
        assert!(
            processed.css.contains(&format!(".{foo}:hover")),
            "{}",
            processed.css
        );
        let bar = scoped_of(&processed, "bar");
        assert!(
            processed.css.contains(&format!(".{bar}::before")),
            "{}",
            processed.css
        );
        assert!(
        ...
        assert!(
            processed.css.contains("[data-x=\".raw\"]"),
            "{}",
            processed.css
        );
        assert!(
            !processed.css.contains(".foo"),
            "no unscoped local: {}",
            processed.css
        );
    ...
    fn global_leaves_contents_unscoped_and_local_scopes_explicitly() {
        ...
        assert!(
            processed.css.contains(&format!(".raw .{item}")),
            "{}",
            processed.css
        );
        assert!(processed.css.contains(".a .b"), "{}", processed.css);
        assert!(
            processed.css.contains(&format!(".{mine}")),
            "{}",
            processed.css
        );
    ...
    fn classes_inside_selector_pseudo_functions_are_scoped() {
        ...
        assert!(
            processed.css.contains(&format!(":not(.{bar})")),
            "{}",
            processed.css
        );
        assert!(
            processed.css.contains(&format!(":is(.{baz})")),
            "{}",
            processed.css
        );
    }
    ...
    fn composes_from_a_file_and_from_global_produce_the_right_segments() {
        ...
        assert_eq!(
            processed.compose_imports,
            vec!["./other.module.css".to_string()]
        );
        let (_, segments) = processed
            .mapping
            .iter()
        ...
        assert!(
            matches!(&segments[1], MappingSegment::Foreign { import: 0, name } if name == "bar")
        );
        assert!(
            matches!(&segments[2], MappingSegment::Foreign { import: 0, name } if name == "baz")
        );
    ...
    fn keyframes_names_and_animation_references_are_scoped_together() {
        ...
        assert!(
            processed.css.contains(&format!("@keyframes {spin}")),
            "{}",
            processed.css
        );
        assert!(
            processed.css.contains(&format!("animation: {spin} 1s")),
            "{}",
            processed.css
        );
        assert!(
        ...
        assert!(
            processed.css.contains("@keyframes shared"),
            "{}",
            processed.css
        );
    }
    ...
    fn conditional_group_rules_scope_their_nested_style_rules() {
        ...
        assert!(
            processed.css.contains("@media (min-width: 600px)"),
            "{}",
            processed.css
        );
        assert!(
            processed.css.contains(&format!(".{wide}")),
            "{}",
            processed.css
        );
    ...
    fn unsupported_constructs_are_hard_errors_naming_the_file_and_construct() {
        ...
        assert!(
            unknown_at_rule.contains("unsupported at-rule `@tailwind`"),
            "{unknown_at_rule}"
        );
        assert!(
            unknown_at_rule.contains("button.module.css"),
            "{unknown_at_rule}"
        );

        let nested = module_error(".foo { .bar { color: red; } }");
        assert!(
            nested.contains("nested rules are not supported"),
            "{nested}"
        );
    ...
    fn import_statements_parse_string_and_url_forms_with_media() {
        ...
        assert!(
            processed.css.contains(".app { color: red; }"),
            "{}",
            processed.css
        );
    }
    ...
    fn an_import_layer_or_supports_condition_is_a_hard_error() {
        ...
        assert!(
            layer.contains("layer(...) condition is not supported"),
            "{layer}"
        );
        ...
        assert!(
            supports.contains("supports(...) condition is not supported"),
            "{supports}"
        );
    }
    ...
    fn public_rooted_urls_become_stylesheet_relative_for_any_base() {
        ...
        .unwrap();
        assert!(
            processed.css.contains("url(\"fonts/x.ttf\")"),
            "{}",
            processed.css
        );
    ...
    fn url_rewriting_skips_absolute_data_fragment_and_public_urls() {
        ...
        for original in [
            ...
            assert!(
                processed.css.contains(original),
