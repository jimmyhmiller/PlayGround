...
//! ever touches the filesystem, and answers from the adjacent helper sources — files embedded
...
pub fn helper_source(specifier: &str) -> Option<&'static str> {
    match helper_name(specifier)? {
        "decorate" => Some(include_str!("runtime_helpers/decorate.js")),
        "decorateMetadata" => Some(include_str!("runtime_helpers/decorateMetadata.js")),
        "decorateParam" => Some(include_str!("runtime_helpers/decorateParam.js")),
        _ => None,
    }
}
...
pub fn unknown_helper_error(specifier: &str) -> String {
    ...
    format!(
        "diffpack does not carry the oxc runtime helper `{name}` (requested as \
         ...
         oxc_transformer version into crates/diffpack-default-loader/src/runtime_helpers/ and \
         register it in runtime_helpers.rs."
    )
}
...
mod tests {
    ...
    fn an_unknown_helper_is_still_recognized_as_a_helper_and_refused_by_name() {
        ...
        assert!(error.contains("crates/diffpack-default-loader"), "{error}");
    }
