...
pub type PagesAppConfig = diffpack_default_loader::driver_config::EnvironmentConfig;

pub type PagesBuildConfig = diffpack_default_loader::driver_config::BuildConfig;
...
pub fn is_pages_router(root: &Path) -> bool {
    ...
    if !crate::shared_config::is_next_project(&canonical) {
        return false;
    }
...
fn discover(dir: &Path) -> Result<Discovery, String> {
    ...
    let key = |route: &Route| -> (usize, usize, i32) {
        let dynamics = route
            ...
            .count();
        let has_catchall = route
            .segments
            .iter()
            .any(|s| matches!(s, Segment::CatchAll(_) | Segment::OptionalCatchAll(_)));
        (
            has_catchall as usize,
            dynamics,
            -(route.segments.len() as i32),
        )
    };
    discovery
        .pages
...
const RUNTIME_FILES: &[(&str, &str)] = &[
    (
        "pages-url.js",
        include_str!("../../../scripts/pages/pages-url.js"),
    ),
    (
        "pages-runtime.jsx",
        include_str!("../../../scripts/pages/pages-runtime.jsx"),
    ),
    (
        "next-router.jsx",
        include_str!("../../../scripts/pages/next-router.jsx"),
    ),
    (
        "pages-head-manager.jsx",
        include_str!("../../../scripts/pages/pages-head-manager.jsx"),
    ),
    (
        "next-head.jsx",
        include_str!("../../../scripts/pages/next-head.jsx"),
    ),
    (
        "next-link.jsx",
        include_str!("../../../scripts/pages/next-link.jsx"),
    ),
    (
        "route-announcer.jsx",
        include_str!("../../../scripts/pages/route-announcer.jsx"),
    ),
    (
        "next-document.jsx",
        include_str!("../../../scripts/pages/next-document.jsx"),
    ),
    (
        "default-app.jsx",
        include_str!("../../../scripts/pages/default-app.jsx"),
    ),
    (
        "default-document.jsx",
        include_str!("../../../scripts/pages/default-document.jsx"),
    ),
    (
        "default-error.jsx",
        include_str!("../../../scripts/pages/default-error.jsx"),
    ),
    (
        "pages-client-entry.jsx",
        include_str!("../../../scripts/pages/pages-client-entry.jsx"),
    ),
    (
        "pages-server-entry.jsx",
        include_str!("../../../scripts/pages/pages-server-entry.jsx"),
    ),
];
...
pub fn configure(
    ...
    environment: &str,
    dev: bool,
) -> Result<Option<PagesAppConfig>, String> {
    let root = root
        .canonicalize()
        .map_err(|error| format!("cannot open project root {}: {error}", root.display()))?;
    if !crate::shared_config::is_next_project(&root) {
        return Ok(None);
    }
    ...
    let next_config = crate::next_config::run_next_config_eval(&root);
    ...
    crate::shared_config::report_mdx_config(next_config.as_ref());
    ...
    let alias = |spec: &str, file: &str| {
        (
            spec.to_string(),
            adapter_dir.join(file).to_string_lossy().into_owned(),
        )
    };
    let mut aliases = vec![
    ...
    if is_client {
        aliases.extend(crate::shared_config::next_browser_polyfill_aliases(&root));
    }
    ...
    let defines = vec![
        ...
        (
            ...
            crate::shared_config::next_runtime_define(target).to_string(),
        ),
        (
            "process.browser".to_string(),
            crate::shared_config::process_browser_define(target).to_string(),
        ),
    ];

    Ok(Some(PagesAppConfig {
        environment: environment.to_string(),
        build: PagesBuildConfig {
            base: "/".to_string(),
            ...
            source_maps: crate::shared_config::default_source_maps(
                target,
                dev,
                next_config.as_ref(),
            ),
            ...
            server_external_packages: crate::shared_config::server_external_packages(
                next_config.as_ref(),
            ),
        },
        entry: Some(entry),
...
mod tests {
    ...
    fn i18n_locales_and_default_are_read() {
        ...
        let config = i18n_from_eval(Some(&eval))
            .unwrap()
            .expect("i18n configured");
    ...
    fn unsupported_i18n_features_are_hard_errors_not_silent_gaps() {
        ...
        assert!(
            i18n_from_eval(Some(&eval))
                .unwrap_err()
                .contains("i18n.locales")
        );
        let eval = eval_with_i18n(serde_json::json!({
            "locales": ["en"],
            "defaultLocale": "fr",
    ...
    fn manifests_carry_the_i18n_table() {
        ...
        assert!(
            client_manifest_module(&discovery, &adapter, None)
                .contains("export const i18n = null;")
        );
        assert!(
            server_manifest_module(&discovery, &adapter, None)
                .contains("export const i18n = null;")
        );
        ...
        let expected = r#"export const i18n = { locales: ["en", "fr", "nl"], defaultLocale: "en", localeDetection: true };"#;
    ...
    fn entries_implement_locale_routing_end_to_end() {
        ...
        assert!(
            server.contains("locale: i18n ? locale : null,"),
            "<html lang> needs the locale"
        );
        // SSG prerender covers every locale, under locale-prefixed URLs.
        assert!(
            server.contains("addLocale(urlFor(page, params), locale, i18n && i18n.defaultLocale)")
        );

        let document = include_str!("../../../scripts/pages/next-document.jsx");
        assert!(
            document.contains("ctx && ctx.locale"),
            "<Html> must fill lang from the locale"
        );
    ...
    fn server_entry_api_request_surface_is_complete() {
        ...
        assert!(
            entry.contains("cookies: parseCookies(headers)"),
    ...
    fn discover_classifies_api_and_dynamic_api_routes() {
        ...
        let dir =
            std::env::temp_dir().join(format!("diffpack-api-discover-{}", std::process::id()));
    ...
    fn scratch_pages_project(name: &str, pages_rel: &str, page_file: &str) -> PathBuf {
        let root =
            std::env::temp_dir().join(format!("diffpack-next-pages-{name}-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
        std::fs::create_dir_all(root.join(pages_rel)).unwrap();
        std::fs::write(
            root.join("package.json"),
            "{\"name\":\"app\",\"dependencies\":{\"next\":\"16.2.11\"}}\n",
        )
        .unwrap();
        std::fs::write(
            root.join(pages_rel).join(page_file),
            "export default function P(){return null}\n",
        )
        .unwrap();
        root
    ...
    fn hybrid_project_matches_both_routers() {
        ...
        .unwrap();
        assert!(crate::shared_config::is_app_router(&root));
        assert!(is_pages_router(&root));
        let _ = std::fs::remove_dir_all(&root);
    }
    ...
    fn client_entry_renders_the_route_announcer() {
        ...
        assert!(
            RUNTIME_FILES
                .iter()
                .any(|(name, _)| *name == "route-announcer.jsx"),
