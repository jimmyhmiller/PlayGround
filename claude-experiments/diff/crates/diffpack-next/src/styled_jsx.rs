        ...
        if is_style {
            ...
            if has_jsx && has_global {
                let children = element
                    .closing_element
                    .as_ref()
                    .map(|closing| Span::new(element.opening_element.span.end, closing.span.start));
...
mod tests {
    ...
    fn global_style_jsx_compiles_to_the_styled_jsx_component() {
        ...
        assert!(
            output.starts_with(
                "\"use client\";\nimport __diffpack_JSXStyle from \"styled-jsx/style\";"
            ),
            "{output}"
        );
        ...
        assert!(
            ...
            "the raw style element must be gone: {output}"
        );
    ...
    fn identical_css_hashes_identically_and_different_css_does_not() {
        let a = transform_styled_jsx(
            &tsx(),
            "export const A = () => <style jsx global>{`.x{}`}</style>;\n",
        )
        .unwrap()
        .unwrap();
        let b = transform_styled_jsx(
            &tsx(),
            "export const B = () => <style jsx global>{`.x{}`}</style>;\n",
        )
        .unwrap()
        .unwrap();
        let c = transform_styled_jsx(
            &tsx(),
            "export const C = () => <style jsx global>{`.y{}`}</style>;\n",
        )
        .unwrap()
        .unwrap();
        let id = |s: &str| {
