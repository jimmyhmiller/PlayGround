...
use diffpack_core::text_edit::apply_edits;

/// Builds the webpack-compatible browser globals consumed by
/// `react-server-dom-webpack/client.browser`.
///
/// Diffpack's registry remains the implementation; this is strictly the Next/RSC
/// compatibility adapter that translates webpack chunk ids into diffpack chunk
/// URLs and forwards module requests to the registry.
pub fn webpack_runtime_seam(
    runtime_entry_id: &str,
    public_base: &str,
    chunk_files: &[String],
) -> String {
    let base = public_base.trim_end_matches('/');
    let mut chunk_filenames = BTreeMap::new();
    ...
    let chunk_filenames_json =
        serde_json::to_string(&chunk_filenames).unwrap_or_else(|_| "{}".to_string());
    let runtime_key =
        diffpack_core::text_edit::quote(&format!("__diffpack_runtime:{runtime_entry_id}"));
    format!(
...
/// Detects whether a rendered browser graph contains a client boundary and, if
/// so, emits the complete Next/RSC compatibility seam.
pub fn webpack_runtime_seam_for_modules<'a>(
    ...
    modules
        .into_iter()
        .any(|(id, source)| detect_directive(Path::new(id), source) == Some(RscDirective::Client))
        .then(|| webpack_runtime_seam(runtime_entry_id, public_base, chunk_files))
}
...
pub fn transform_use_server_server(path: &Path, source: &str) -> Result<Option<String>, String> {
    ...
    if let Some(span) = default_value {
        ...
        edits.push((
            span,
            format!("__rsr({value}, {}, \"default\")", json_string(&module_id)),
        ));
    }
...
pub fn transform_use_server_client(path: &Path, source: &str) -> Option<String> {
    ...
    let module_id = module_reference_id(path);
    let mut out = String::new();
    out.push_str("import { createServerReference } from \"react-server-dom-webpack/client\";\n");
    out.push_str(&format!(
...
fn walk_project_modules_inner<F>(
    root: &Path,
...
pub fn generate_action_resolver_module(entries: &[ActionEntry]) -> String {
    ...
    source.push_str(
        "// Generated natively by Diffpack from the app's \"use server\" action exports.\n",
    );
...
pub fn is_stylesheet_specifier(specifier: &str) -> bool {
    let path = diffpack_core::resource_id::ResourceId::parse(specifier).path;
    matches!(
...
mod ssr_module_map_tests {
    ...
    fn run_join(client: &str, flight: &str, ssr: &str, script: &str) -> (String, String, bool) {
...
pub fn client_references_from_bundle_graph(
    graph: &diffpack_core::bundle::IntegrationManifestGraph,
) -> ClientReferencesManifest {
    let mut entries = BTreeMap::new();
    for module in &graph.modules {
        let path = Path::new(&module.id);
        if detect_directive(path, &module.source) != Some(RscDirective::Client) {
            continue;
        }
        let chunks = module
            .chunk
            .as_ref()
            .map_or_else(Vec::new, |file| vec![file.clone(), file.clone()]);
        entries.insert(
    ...
    ClientReferencesManifest { entries }
}
...
mod tests {
    ...
    fn use_client_server_transform_emits_client_module_proxy_reexports() {
        ...
        assert!(
            ...
            "server build must not ship client code: {out}"
        );
        // Each export is re-exported off react-server-dom's client module proxy.
        assert!(
            out.contains("createClientModuleProxy(\"/app/src/Counter.tsx\")"),
            "{out}"
        );
        assert!(
            out.contains("export const Counter = __diffpack_client[\"Counter\"];"),
            "{out}"
        );
        assert!(
            out.contains("export const Label = __diffpack_client[\"Label\"];"),
            "{out}"
        );
        assert!(
            out.contains("export default __diffpack_client.default;"),
            "{out}"
        );
        assert!(
    ...
    fn the_proxy_keeps_every_stylesheet_form_and_every_stylesheet_language() {
        ...
        let out = transform_use_client_server(Path::new("/app/src/w.tsx"), source)
            .unwrap()
            .unwrap();
    ...
    fn a_commonjs_use_client_module_keeps_its_export_surface() {
        ...
        let out = transform_use_client_server(
            Path::new("/app/node_modules/next/dist/client/script.js"),
            source,
        )
    ...
    fn a_use_client_module_with_no_enumerable_exports_is_a_hard_error() {
        ...
        assert!(
            error.contains("/app/node_modules/weird/index.js"),
    ...
    fn ssr_consumer_manifest_module_exports_the_manifest() {
        ...
        assert!(
            module.contains("export const serverConsumerManifest ="),
            "{module}"
        );
        assert!(
            module.contains("export default serverConsumerManifest;"),
            "{module}"
        );
    ...
    fn a_missing_client_references_manifest_is_a_specific_error() {
        let dir = tempfile::tempdir().unwrap();
        let error = ClientReferencesManifest::read(&dir.path().join("nope.json")).unwrap_err();
    ...
    fn use_cache_transform_wraps_named_exports_keeping_bodies() {
        ...
        assert!(
            out.contains("return id + 1"),
            "cached body preserved: {out}"
        );
        ...
        assert!(
            !out.contains("export async function getData"),
            "export stripped off decl: {out}"
        );
        // Each export is wrapped and re-exported under its original name, keyed by id.
        assert!(
            out.contains("= __diffpack_uc(getData, \"/app/src/data.ts#getData\");"),
            "{out}"
        );
        assert!(
            out.contains("= __diffpack_uc(load, \"/app/src/data.ts#load\");"),
            "{out}"
        );
    ...
    fn use_cache_transform_wraps_the_default_export() {
        ...
        assert!(
            out.contains("const __diffpack_uc_default = (async function Page(){ return null });"),
            "{out}"
        );
        assert!(
            out.contains("= __diffpack_uc(__diffpack_uc_default, \"/app/src/page.tsx#default\");"),
            "{out}"
        );
    ...
    fn use_cache_transform_wraps_renamed_specifiers_by_exported_name() {
        ...
        assert!(
            out.contains("= __diffpack_uc(impl, \"/app/src/data.ts#run\");"),
            "{out}"
        );
        assert!(out.contains("as run };"), "{out}");
        // The original specifier-only export statement is removed (no duplicate export).
        assert!(
            !out.contains("export { impl as run };"),
            "original specifier export removed: {out}"
        );
    }
    ...
    fn use_cache_transform_hard_errors_on_reexport() {
        ...
        .unwrap_err();
        assert!(
            err.contains("export ... from") && err.contains("data.ts"),
            "{err}"
        );
    ...
    fn use_server_server_transform_registers_each_export_keeping_bodies() {
        ...
        assert!(
            out.contains("return n + 1"),
            "server keeps the real body: {out}"
        );
    ...
    fn use_server_server_transform_registers_renamed_specifiers_by_exported_name() {
        ...
        assert!(
            out.contains("__rsr(impl, \"/app/src/act.ts\", \"run\");"),
            "{out}"
        );
    }
    ...
    fn use_server_client_transform_drops_bodies_and_emits_stubs() {
        ...
        assert!(
            !out.contains("node:fs"),
            "server-only import must not ship: {out}"
        );
        assert!(
            !out.contains("return n + 1"),
            "server body must not ship: {out}"
        );
        ...
        assert!(
            out.contains("import { callServer } from \"#diffpack-call-server\";"),
            "{out}"
        );
        assert!(
    ...
    fn client_stub_and_server_registration_and_resolver_agree_on_the_action_id() {
        ...
        assert!(
            client.contains(&format!("createServerReference(\"{id}\", callServer)")),
            "{client}"
        );
        ...
        assert!(
            server
                .contains(&"__rsr(increment, \"/app/src/actions.ts\", \"increment\");".to_string()),
            "{server}"
        );
        ...
        assert!(
            resolver.contains("no server action registered"),
            "{resolver}"
        );
        assert!(
            resolver.contains("export { getServerActionById }"),
            "{resolver}"
        );
    }
    ...
    #[test]
    fn webpack_runtime_seam_maps_chunk_ids_to_public_urls() {
        let seam = webpack_runtime_seam(
            "/app/src/client.tsx",
            "/assets/",
            &["./client-a.js".to_string(), "shared.js".to_string()],
        );
        assert!(
            seam.contains("\"client-a.js\":\"/assets/client-a.js\""),
            "{seam}"
        );
        assert!(
            seam.contains("\"shared.js\":\"/assets/shared.js\""),
            "{seam}"
        );
        assert!(
            seam.contains("__diffpack_runtime:/app/src/client.tsx"),
            "{seam}"
        );
        assert!(seam.contains("unknown chunk id"), "{seam}");

        assert!(
            webpack_runtime_seam_for_modules(
                [("/app/plain.ts", "export const value = 1;")],
                "/app/plain.ts",
                "/",
                &[],
            )
        ...
        assert!(
            webpack_runtime_seam_for_modules(
                [("/app/client.tsx", "\"use client\"; export default 1;")],
                "/app/client.tsx",
                "/",
                &[],
            )
