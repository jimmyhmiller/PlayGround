...
//! * local — the font binary itself, read by [`diffpack_default_loader::font_file`], which is what
...
use diffpack_core::text_edit::{apply_edits, quote};
            ...
            "src" => match &prop.value {
                ...
                Expression::ArrayExpression(array) => {
                    ...
                    for element in &array.elements {
                        ...
                        let mut src = LocalSrc {
                            path: String::new(),
                            weight: None,
                            style: None,
                        };
                        for field in &entry.properties {
                            let ObjectPropertyKind::ObjectProperty(field) = field else {
                                continue;
                            };
                ...
                if let Expression::ArrayExpression(array) = &prop.value {
                    for element in &array.elements {
                        ...
                        for field in &entry.properties {
                            ...
                            match name {
                                "prop" => {
                                    declaration.0 = string_of(&field.value).unwrap_or_default()
                                }
                                "value" => {
                                    declaration.1 =
...
fn resolve_module(path: &Path, source: &str) -> Result<Option<ResolvedModule>, String> {
    ...
    for (name, call) in &collector.calls {
        let Some(import) = imports.iter().find(|i| &i.local == name) else {
            continue;
        };
...
fn resolve_call(
    ...
    let src = match (&options.src_string, &options.src_array) {
        ...
        (None, Some(_)) => {
            return Err(format!(
                "{}: `next/font/local` got an empty `src` array.",
                path.display()
            ));
        }
        (None, None) => {
    ...
    for usage in usages {
        let rules = match &usage.local {
            None => {
                ...
                vec![
                    table
                        .size_adjust_values(&usage.family)?
                        .font_face(&usage.family),
                ]
            }
            Some(local) => {
...
fn local_font_faces(
    ...
    let mut faces = LocalFaces {
        rules: Vec::new(),
        preloads: Vec::new(),
    };
    // fontkit metadata per src entry, kept so the fallback face can be built from the
    // file Next would pick.
    let mut read: Vec<(&LocalSrc, diffpack_default_loader::font_file::FontMetrics)> = Vec::new();
    ...
    for entry in &local.src {
        ...
        let bytes = std::fs::read(&source)
            .map_err(|error| format!("cannot read font file {}: {error}", source.display()))?;
        let metrics = diffpack_default_loader::font_file::read_metrics_from_bytes(&bytes, &source)
            .map_err(|error| {
                format!(
        ...
        let asset = FontAsset {
            source: source.clone(),
            file: format!("{FONT_ASSET_DIR}/{file}"),
        };
...
fn asset_file_name(source: &Path, bytes: &[u8]) -> Result<String, String> {
    ...
    Ok(format!(
        "{stem}-{:016x}.{}",
        diffpack_default_loader::asset::content_hash(bytes),
...
fn pick_fallback_source(
    read: &[(&LocalSrc, diffpack_default_loader::font_file::FontMetrics)],
) -> usize {
    let mut used = 0usize;
...
fn distance_from_normal_weight(weight: Option<&str>) -> f64 {
    ...
    if first_distance.abs() < second_distance.abs() {
        first_distance
    } else {
        second_distance
    }
...
impl SizeAdjustValues {
    ...
    fn from_font_metrics(
        metrics: &diffpack_default_loader::font_file::FontMetrics,
        adjust: LocalAdjust,
    ) -> Self {
...
impl MetricsTable {
    ...
    fn size_adjust_values(&self, family: &str) -> Result<SizeAdjustValues, String> {
        ...
        let fallback_font = if category == "serif" {
            "Times New Roman"
        } else {
            "Arial"
        };
        ...
        let size_adjust = if x_width_avg != 0.0 {
            (x_width_avg / units_per_em) / fallback_avg
        } else {
            1.0
        };
...
pub fn emit_font_assets(root: &Path, out_public: &Path) -> Result<usize, String> {
    let manifest = root.join(crate::APP_ADAPTER_DIR).join(FONT_MANIFEST_FILE);
...
mod tests {
    ...
    fn rewrites_google_font_calls_and_drops_the_import() {
        ...
        assert!(
            !out.contains("next/font/google"),
            "import must be removed: {out}"
        );
        assert!(
            !out.contains("Geist("),
            "the throwing call must be gone: {out}"
        );
    ...
    fn generates_google_import_and_variable_classes() {
        ...
        assert!(
            css.contains("fonts.googleapis.com/css2?family=Geist:wght"),
            "{css}"
        );
        assert!(css.contains("family=Geist+Mono:wght"), "{css}");
        assert!(
            css.contains(".__df_fontvar_geist { --font-geist-sans: 'Geist'"),
            "{css}"
        );
        assert!(
            css.contains(".__df_font_geist { font-family: 'Geist'"),
            "{css}"
        );
    }
    ...
    fn emits_the_metric_matched_fallback_face_next_generates() {
        ...
        assert!(
            out.contains("fontFamily: \"'Inter', 'Inter Fallback'\""),
            "{out}"
        );
    }
    ...
    fn honors_adjust_font_fallback_false_and_the_fallback_list() {
        ...
        assert_eq!(
            usages[0].fallbacks,
            vec!["Helvetica".to_string(), "sans-serif".to_string()]
        );
        let css = generate_css(&root, &usages).unwrap();
        assert!(
            !css.contains("@font-face"),
            "opted out, so no fallback face: {css}"
        );
        assert!(
    ...
    fn an_unknown_family_is_a_hard_error_naming_it() {
        ...
        assert!(
            error.contains("notAReal"),
            "the table key must be named too: {error}"
        );
    ...
    // average width of 462.7906976744186, which is exactly what the font metrics loader
    ...
    fn scaffold_local(name: &str, source: &str, units_per_em: u16, advance: u16) -> PathBuf {
        ...
        std::fs::write(
            ...
            diffpack_default_loader::font_file::synthetic_font(units_per_em, advance),
        )
        .unwrap();
        root
    }
    ...
    fn rewrites_a_local_font_call_using_the_const_name_as_the_family() {
        ...
        let out = transform_next_font(Path::new("app/layout.tsx"), source)
            .unwrap()
            .unwrap();
        assert!(
            !out.contains("next/font/local"),
            "the import must be removed: {out}"
        );
        assert!(
            !out.contains("localFont("),
            "the throwing call must be gone: {out}"
        );
    ...
    fn emits_the_face_asset_and_fallback_next_generates_for_a_local_font() {
        ...
        assert!(
            css.contains(
                "@font-face { font-family: calFont; src: url(/_diffpack-font/CalSans-SemiBold-"
            ),
            "{css}"
        );
        assert!(
            css.contains(") format('truetype'); font-display: block; font-weight: 600; }"),
            "{css}"
        );
        ...
        assert_eq!(
            output.assets[0].source,
            root.join("fonts/CalSans-SemiBold.ttf")
        );
        assert!(
            output.assets[0]
                .file
                .starts_with("_diffpack-font/CalSans-SemiBold-"),
            "{:?}",
            output.assets
        );
        assert!(
            css.contains(&output.assets[0].file),
            "the face must name the emitted file: {css}"
        );
        assert_eq!(output.preloads, vec![format!("/{}", output.assets[0].file)]);

        // And the emit copies exactly what the manifest recorded.
        let adapter = root.join(crate::APP_ADAPTER_DIR);
    ...
    fn the_font_url_carries_the_asset_prefix() {
        ...
        assert!(
            output
                .css
                .contains("src: url(/docs/_diffpack-font/CalSans-SemiBold-"),
            "{}",
            output.css
        );
        assert!(
            output.preloads[0].starts_with("/docs/_diffpack-font/"),
            "{:?}",
            output.preloads
        );
    }
    ...
    fn an_src_array_emits_one_face_per_file_and_picks_the_fallback_source() {
        ...
        std::fs::write(
            ...
            diffpack_default_loader::font_file::synthetic_font(1000, 900),
        )
        .unwrap();
        std::fs::write(
            root.join("fonts/Regular.ttf"),
            diffpack_default_loader::font_file::synthetic_font(1000, 500),
        )
        .unwrap();
        let usages = scan_at(&root.join("app/layout.tsx"));
        let output = generate(&root, &usages, "").unwrap();
        let css = &output.css;
        assert_eq!(
            css.matches("src: url(").count(),
            2,
            "one face per src entry: {css}"
        );
        assert!(
            css.contains("font-weight: 700; font-style: normal; }"),
            "{css}"
        );
        assert!(
            css.contains("font-weight: 400; font-style: normal; }"),
            "{css}"
        );
    ...
    fn honors_the_local_adjust_font_fallback_option() {
        ...
        let css = generate(&root, &scan_at(&root.join("app/layout.tsx")), "")
            .unwrap()
            .css;
        assert!(
            css.contains(
        ...
        assert!(
            !css.contains("Fallback"),
            "opted out, so no generated face: {css}"
        );
    ...
    fn a_missing_src_file_is_a_hard_error_naming_the_module_and_the_path() {
        ...
        assert!(
            error.contains("fonts/Nope.woff2"),
            "the resolved path too: {error}"
        );
    }
    ...
    fn invalid_local_font_options_are_hard_errors() {
        ...
        .unwrap_err();
        assert!(
            missing_src.contains("missing the required `src`"),
            "{missing_src}"
        );
    ...
    fn a_module_can_mix_google_and_local_fonts() {
        ...
        assert!(
            output.css.contains("font-family: calFont;"),
            "{}",
            output.css
        );
    ...
    fn repeated_usages_collapse_but_differing_ones_do_not() {
        ...
        assert_eq!(
            css.matches("'calFont Fallback'; src: local(\"Arial\")")
                .count(),
            1,
    ...
    fn the_font_manifest_is_rewritten_even_when_empty() {
        ...
        write_font_manifest(
            &adapter,
            &[FontAsset {
                source: root.join("nope.woff2"),
                file: "_diffpack-font/nope.woff2".into(),
            }],
        )
        .unwrap();
        write_font_manifest(&adapter, &[]).unwrap();
        assert_eq!(
            emit_font_assets(&root, &root.join("out/public")).unwrap(),
            0
        );
    }
