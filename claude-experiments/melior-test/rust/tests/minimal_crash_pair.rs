//! Minimal test with just the two tests that cause 100% crash rate
//!
//! Based on systematic testing, these are the minimal set of tests that reproduce the crash
...
fn create_safe_tensor_function(
    ...
    let location = Location::unknown(context);
    let i32_type = IntegerType::new(context, 32);
    let tensor_type = melior::ir::r#type::RankedTensorType::new(&[2], i32_type.into(), None);
    ...
    let const1 = OperationBuilder::new("arith.constant", location)
        .add_attributes(&[(
            ...
            melior::ir::attribute::IntegerAttribute::new(i32_type.into(), 1).into(),
        )])
