...
// C-compatible tool wrapper that implements FoundationModels.Tool
@objc public final class CToolWrapper: NSObject, Tool {
    ...
    // Simple arguments schema for C tools (just takes a JSON string)
    @Generable
    ...
    public func call(arguments: Arguments) async throws -> ToolOutput {
        let result = callFunction(arguments.arguments)
...
@objc public class FoundationModelsWrapper: NSObject {
    ...
    @objc public func addTool(_ tool: CToolWrapper) {
        ...
        if let currentSession = session {
            ...
            let instructions = currentSession.transcript.description.isEmpty ? nil : "Continue the conversation"
            self.session = LanguageModelSession(
                tools: cTools,
                instructions: instructions
            )
