...
// A SOURCE VALUE is represented with NO wrapper, to minimize per-iteration heap
// churn (so the reduce loop's state normalizes and memo-ties): a STATIC source
// value is a bare number; a DYNAMIC one is its code AST (["rlit"|"rin"|"radd"|...]).
function toCode(v) { if (typeof v === "number") { return ["rlit", v]; } return v; }
...
function pevalArith(op, a, b) {
  ...
  if (op === "+") { return ["radd", ca, cb]; }
  if (op === "-") { return ["rsub", ca, cb]; }
  return ["rmul", ca, cb];
}
...
function evalExpr(funcs, expr, env) {
  ...
  if (tag === "call") {
    ...
    if (callee[0] === "dot" && callee[2] === "reduce") {
      ...
      var acc = ["rlit", evalExpr(funcs, args[1], env)]; // init lifted to dynamic code (uniform)
...
function jsmix(funcs, entryName, prog) {
  ...
  var env = envExtend(envExtend(emptyEnv(), fn[2][0], prog), fn[2][1], ["rin"]);
  return toCode(evalStmts(funcs, fn[3], env));
}

function main(prog){var INT=[["fundecl","step",["instr","acc","input"],[["return",["cond",["bin","===",["idx",["var","instr"],["lit",0]],["lit","addlit"]],["bin","+",["var","acc"],["idx",["var","instr"],["lit",1]]],["cond",["bin","===",["idx",["var","instr"],["lit",0]],["lit","mullit"]],["bin","*",["var","acc"],["idx",["var","instr"],["lit",1]]],["cond",["bin","===",["idx",["var","instr"],["lit",0]],["lit","addin"]],["bin","+",["var","acc"],["var","input"]],["cond",["bin","===",["idx",["var","instr"],["lit",0]],["lit","subin"]],["bin","-",["var","acc"],["var","input"]],["var","acc"]]]]]]]],["fundecl","interp",["prog","input"],[["return",["call",["dot",["var","prog"],"reduce"],[["fun",["acc","instr"],[["return",["call",["var","step"],[["var","instr"],["var","acc"],["var","input"]]]]]],["lit",0]]]]]]];return jsmix(INT,"interp",prog);}
