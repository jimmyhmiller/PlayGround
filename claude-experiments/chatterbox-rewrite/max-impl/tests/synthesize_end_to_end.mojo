...
This is the production path:
  1. Load voice/text conditioning (prompt_token, prompt_feat, embedding,
     cond_emb, text_emb, bos_emb, spks). These are non-RNG preprocessing
     outputs (BPE tokenization + voice encoding); not yet ported to Mojo,
     so they come from the conditioning dump. Listed as `voice + text inputs`.
  2. Mojo T3 generate (with CFG) → speech_tokens.
  3. Concat[prompt_token, speech_tokens] → tok_buf (B, T_TOTAL_TOKEN).
  4. Mojo flow encoder → mu (B, 80, T_TOTAL_MEL).
  5. Build cond (zeros) and slot prompt_feat into [:, :, :T_PROMPT_MEL].
  6. Mojo gaussian_noise_fill → x (B, 80, T_TOTAL_MEL).
  7. Mojo cfm_solve_euler (cosine schedule, CFG) → x (=mel) over the
     full T_TOTAL_MEL range.
  8. Trim prompt prefix: mel_out = x[:, :, T_PROMPT_MEL:].
  9. Mojo HiFT: mel → f0 → LCG source module → STFT → trunk → iSTFT → audio.

NO upstream RNG dumps (cfm_noise_z, sg_phase_vec, sg_noise) are loaded.
NO upstream speech_tokens are loaded — they're generated by Mojo T3.
"""
