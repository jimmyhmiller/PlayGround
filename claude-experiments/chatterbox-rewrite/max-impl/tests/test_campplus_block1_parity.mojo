"""block1 parity vs upstream tdnn_out → block1_out."""
...
def test_block1() raises:
    ...
    var x = upload_fp32(ctx, "weights/campplus_parity/tdnn_out.bin")
    var expected = load_fp32("weights/campplus_parity/block1_out.bin")

    var out = ctx.enqueue_create_buffer[DType.float32](B * OUT_CH * T)
    camdense_tdnn_block_forward(
        ctx, cp.xvector.block1, x, out, B, IN_CH, T, 3, 1, 1,
    )
    ctx.synchronize()
    ...
    print("[block1] max-abs =", max_abs, " rel_l2 =", rel_l2)
