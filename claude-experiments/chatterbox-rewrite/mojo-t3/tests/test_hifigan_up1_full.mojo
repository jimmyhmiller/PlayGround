"""
HiFiGAN upsample stage 1 in Mojo (i=1 in the decode() loop).

Input:  stage_after_up0.bin   (1, 256, 256)
Output: stage_after_up1.bin   (1, 128, 1280)

Mirrors test_hifigan_up0_full.mojo at i=1 sizes:
  ups[1]: 256 -> 128 channels, K=11, stride=5, padding=3, T 256 -> 1280
  source_downs[1]: 18 -> 128, K=6, stride=3, padding=1, T 3841 -> 1280 (with center)
  source_resblocks[1]: K=7, dilations=[1,3,5] at C=128
  resblocks.3 (i=1, j=0): K=3 at C=128
  resblocks.4 (i=1, j=1): K=7 at C=128
  resblocks.5 (i=1, j=2): K=11 at C=128
"""
...
comptime OUT_T = 1280
comptime UP_K = 11
comptime UP_STRIDE = 5
comptime UP_PAD = 3
comptime S_STFT_C = 18
comptime S_STFT_T = 3841
comptime SRC_DOWN_K = 6
comptime SRC_DOWN_STRIDE = 3
comptime SRC_DOWN_PAD = 1
...
def test_up1_full_fp32() raises:
    comptime assert has_accelerator(), "Requires GPU"

    var fix = "tests/fixtures/hifigan/"
    var x_in = load_fp32(fix + "stage_after_up0.bin")
    var s_stft = load_fp32(fix + "stage_s_stft_cat.bin")
    var exp = load_fp32(fix + "stage_after_up1.bin")
    ...
    var x_up_buf = ctx.enqueue_create_buffer[DType.float32](n_x_out)
    ...
    var up_w = load_fp32(fix + "weights/ups__1__weight.bin")
    var up_b = load_fp32(fix + "weights/ups__1__bias.bin")
    var src_down_w = load_fp32(fix + "weights/source_downs__1__weight.bin")
    var src_down_b = load_fp32(fix + "weights/source_downs__1__bias.bin")
    ...
    _run_resblock_chain[11](ctx, rb_x_buf, rb_next_buf, rb_xt_buf, rb_xt2_buf,
                             fix + "weights/resblocks__5__", 1, 3, 5)
    ctx.enqueue_function[add_out_k, add_out_k](
        rb_out_acc2_flat, rb_out_acc_flat, rb_x_flat, n_x_out,
        grid_dim=ceildiv(n_x_out, POINTWISE_BLOCK), block_dim=POINTWISE_BLOCK,
    )
    ...
    with rb_out_acc_buf.map_to_host() as h:
        for i in range(n_x_out):
            ...
            sum_abs += Float64(d)
            assert_almost_equal(v, exp.data[i], atol=3.0e-3)
    print("HiFiGAN up1 full stage fp32 — max abs:", max_abs,
