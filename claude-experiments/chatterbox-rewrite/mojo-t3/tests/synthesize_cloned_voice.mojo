"""
Pure-Mojo synthesize binary.

End-to-end (where ported): reads inputs from disk, runs Mojo kernels, writes a
WAV via Mojo file I/O. No Python in the runtime path.

Currently it loads:
  tests/fixtures/real/real_mel.bin             (1, 80, 262)  — from S3Gen (still torch upstream)
  tests/fixtures/real/real_s_stft_cat.bin      (1, 18, 31441) — STFT of source signal
  tests/fixtures/hifigan/weights/*.bin         — HiFiGAN weights

Runs the full Mojo HiFiGAN forward and writes:
  out.wav   (24kHz mono PCM-16)

Components that still load pre-dumped intermediates (vs. running in Mojo here):
  - reference-voice xvector (CAMPPlus)
  - S3Gen UpsampleConformerEncoder
  - CFM/ConditionalDecoder mel synthesis
  - f0_predictor + SineGen source signal

Each port-in-progress replaces a `load_fp32(<intermediate>)` with a real
Mojo function call. The runtime path stays Mojo-only throughout.
"""
...
from fixture import Tensor, load_fp32, save_wav
from conv import (
...
def copy_device(mut ctx: DeviceContext,
...
def main() raises:
    ...
    print("[synthesize] loading inputs...")
    var fix = "tests/fixtures/hifigan/weights/"
    var mel = load_fp32("tests/fixtures/real/cloned_voice_mel_mojo.bin")
    var s_stft = load_fp32("tests/fixtures/real/e2e_s_stft_cat.bin")
    var window = load_fp32(fix + "stft_window.bin")
    print("[synthesize] mel: n=", len(mel.data))
    print("[synthesize] s_stft:", s_stft.shape[0], "x", s_stft.shape[1], "x", s_stft.shape[2])
    ...
    print("[synthesize] loading HiFiGAN weights...")
    ...
    var cp_pre_w = load_fp32(fix + "conv_pre__weight.bin")
    var cp_pre_b = load_fp32(fix + "conv_pre__bias.bin")
    ...
    var up0_w = load_fp32(fix + "ups__0__weight.bin")
    var up0_b = load_fp32(fix + "ups__0__bias.bin")
    ...
    var s0_src_down_w = load_fp32(fix + "source_downs__0__weight.bin")
    var s0_src_down_b = load_fp32(fix + "source_downs__0__bias.bin")
    ...
    var up1_w = load_fp32(fix + "ups__1__weight.bin")
    var up1_b = load_fp32(fix + "ups__1__bias.bin")
    ...
    var s1_src_down_w = load_fp32(fix + "source_downs__1__weight.bin")
    var s1_src_down_b = load_fp32(fix + "source_downs__1__bias.bin")
    ...
    var up2_w = load_fp32(fix + "ups__2__weight.bin")
    var up2_b = load_fp32(fix + "ups__2__bias.bin")
    ...
    var s2_src_down_w = load_fp32(fix + "source_downs__2__weight.bin")
    var s2_src_down_b = load_fp32(fix + "source_downs__2__bias.bin")
    ...
    var cp_post_w = load_fp32(fix + "conv_post__weight.bin")
    var cp_post_b = load_fp32(fix + "conv_post__bias.bin")
    ...
    print("[synthesize] running HiFiGAN forward...")
    ...
    copy_device(ctx, s0_si_after_down_buf, s0_rb_x_buf, n_s0)
    _run_resblock_chain[S0_C, S0_T, 7, SNAKE_BLOCK](
        ctx, s0_rb_x_buf, s0_rb_next_buf, s0_rb_xt_buf, s0_rb_xt2_buf,
        fix + "source_resblocks__0__")
    copy_device(ctx, s0_rb_x_buf, s0_si_buf, n_s0)
    ...
    copy_device(ctx, s0_x_plus_si_buf, s0_rb_x_buf, n_s0)
    _run_resblock_chain[S0_C, S0_T, 3, SNAKE_BLOCK](
        ctx, s0_rb_x_buf, s0_rb_next_buf, s0_rb_xt_buf, s0_rb_xt2_buf,
        fix + "resblocks__0__")
    copy_device(ctx, s0_rb_x_buf, s0_acc_buf, n_s0)
    copy_device(ctx, s0_x_plus_si_buf, s0_rb_x_buf, n_s0)
    _run_resblock_chain[S0_C, S0_T, 7, SNAKE_BLOCK](
        ctx, s0_rb_x_buf, s0_rb_next_buf, s0_rb_xt_buf, s0_rb_xt2_buf,
        fix + "resblocks__1__")
    ...
    copy_device(ctx, s0_acc2_buf, s0_acc_buf, n_s0)
    copy_device(ctx, s0_x_plus_si_buf, s0_rb_x_buf, n_s0)
    ...
    with s0_acc2_buf.map_to_host() as src:
        with s0_acc_buf.map_to_host() as dst:
            for i in range(n_s0): dst[i] = src[i] * inv
    ...
    copy_device(ctx, s1_si_after_down_buf, s1_rb_x_buf, n_s1)
    _run_resblock_chain[S1_C, S1_T, 7, SNAKE_BLOCK](
        ctx, s1_rb_x_buf, s1_rb_next_buf, s1_rb_xt_buf, s1_rb_xt2_buf,
        fix + "source_resblocks__1__")
    copy_device(ctx, s1_rb_x_buf, s1_si_buf, n_s1)
    ...
    copy_device(ctx, s1_x_plus_si_buf, s1_rb_x_buf, n_s1)
    _run_resblock_chain[S1_C, S1_T, 3, SNAKE_BLOCK](
        ctx, s1_rb_x_buf, s1_rb_next_buf, s1_rb_xt_buf, s1_rb_xt2_buf,
        fix + "resblocks__3__")
    copy_device(ctx, s1_rb_x_buf, s1_acc_buf, n_s1)
    copy_device(ctx, s1_x_plus_si_buf, s1_rb_x_buf, n_s1)
    _run_resblock_chain[S1_C, S1_T, 7, SNAKE_BLOCK](
        ctx, s1_rb_x_buf, s1_rb_next_buf, s1_rb_xt_buf, s1_rb_xt2_buf,
        fix + "resblocks__4__")
    ...
    copy_device(ctx, s1_acc2_buf, s1_acc_buf, n_s1)
    copy_device(ctx, s1_x_plus_si_buf, s1_rb_x_buf, n_s1)
    ...
    with s1_acc2_buf.map_to_host() as src:
        with s1_acc_buf.map_to_host() as dst:
            for i in range(n_s1): dst[i] = src[i] * inv
    ...
    copy_device(ctx, s2_si_after_down_buf, s2_rb_x_buf, n_s2)
    _run_resblock_chain[S2_C, S2_T, 11, SNAKE_BLOCK](
        ctx, s2_rb_x_buf, s2_rb_next_buf, s2_rb_xt_buf, s2_rb_xt2_buf,
        fix + "source_resblocks__2__")
    copy_device(ctx, s2_rb_x_buf, s2_si_buf, n_s2)
    ...
    copy_device(ctx, s2_x_plus_si_buf, s2_rb_x_buf, n_s2)
    _run_resblock_chain[S2_C, S2_T, 3, SNAKE_BLOCK](
        ctx, s2_rb_x_buf, s2_rb_next_buf, s2_rb_xt_buf, s2_rb_xt2_buf,
        fix + "resblocks__6__")
    copy_device(ctx, s2_rb_x_buf, s2_acc_buf, n_s2)
    copy_device(ctx, s2_x_plus_si_buf, s2_rb_x_buf, n_s2)
    ...
    copy_device(ctx, s2_acc2_buf, s2_acc_buf, n_s2)
    copy_device(ctx, s2_x_plus_si_buf, s2_rb_x_buf, n_s2)
    _run_resblock_chain[S2_C, S2_T, 11, SNAKE_BLOCK](
        ctx, s2_rb_x_buf, s2_rb_next_buf, s2_rb_xt_buf, s2_rb_xt2_buf,
        fix + "resblocks__8__")
    ...
    with s2_acc2_buf.map_to_host() as src:
        with s2_acc_buf.map_to_host() as dst:
            for i in range(n_s2): dst[i] = src[i] * inv
    ...
    ctx.synchronize()
    var elapsed_ns = monotonic() - t0_ns
    var elapsed_s: Float32 = Float32(elapsed_ns) / 1.0e9
    print("[synthesize] HiFiGAN forward+sync took", elapsed_s, "s (", elapsed_s * 1000.0, "ms)")

    print("[synthesize] writing out.wav...")
    ...
    with audio_buf.map_to_host() as h:
        for i in range(n_audio):
            ...
            if v < -0.99: v = -0.99
            samples.append(v)
    save_wav("cloned_voice_out.wav", samples, 24000)
    print("[synthesize] wrote out.wav (", Float64(n_audio) / 24000.0, "s @ 24kHz)")
