...
def test_cfm_solve_full() raises:
    ...
    # ---- Inputs (B=1).
    var z_init = load_fp32(fix + "cfm_z_init.bin")
    var t_span = load_fp32(fix + "cfm_t_span.bin")
    var mu = load_fp32(fix + "cfm_mu.bin")
    var mask = load_fp32(fix + "cfm_mask.bin")
    var spks = load_fp32(fix + "cfm_spks.bin")
    var cond = load_fp32(fix + "cfm_cond.bin")
    var exp = load_fp32(fix + "cfm_mel_out.bin")
    ...
    # ---- time_mlp weights.
    var tm_w1 = upload_w(ctx, fix, "weights/flow__decoder__estimator__time_mlp__linear_1__weight.bin")
    var tm_b1 = upload_w(ctx, fix, "weights/flow__decoder__estimator__time_mlp__linear_1__bias.bin")
    var tm_w2 = upload_w(ctx, fix, "weights/flow__decoder__estimator__time_mlp__linear_2__weight.bin")
    var tm_b2 = upload_w(ctx, fix, "weights/flow__decoder__estimator__time_mlp__linear_2__bias.bin")
    ...
    # ---- Compare against cfm_mel_out.bin.
    ...
    with x_buf.map_to_host() as h:
        for i in range(n_x):
            ...
            sum_abs += Float64(d)
            if i < 8:
                print("mel[", i, "]: mojo=", h[i], "  torch=", exp.data[i], "  diff=", d)
            assert_almost_equal(h[i], exp.data[i], atol=5.0e-1)
    print("FULL CFM solve (10 steps) — max abs:", max_abs,
