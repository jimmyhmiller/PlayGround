...
def main() raises:
    ...
    var s_stft = load_fp32("tests/fixtures/real/cloned_voice_s_stft_mojo.bin")
    var window = load_fp32(fix + "stft_window.bin")
    print("[synthesize] mel: n=", len(mel.data))
    print("[synthesize] s_stft: n=", len(s_stft.data))
    ...
    with audio_buf.map_to_host() as h:
        for i in range(n_audio):
            ...
            if v < -0.99: v = -0.99
            samples.append(v)
    save_wav("cloned_voice_out_v2.wav", samples, 24000)
