...
def main() raises:
    ...
    var mel = load_fp32("tests/fixtures/real/real_mel.bin")
    var s_stft = load_fp32("tests/fixtures/real/real_s_stft_cat.bin")
    var window = load_fp32(fix + "stft_window.bin")
    print("[synthesize] mel:", mel.shape[0], "x", mel.shape[1], "x", mel.shape[2])
    ...
    with audio_buf.map_to_host() as h:
        for i in range(n_audio):
            ...
            if v < -0.99: v = -0.99
            samples.append(v)
    save_wav("out.wav", samples, 24000)
