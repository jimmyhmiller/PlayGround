use cf_tokio::sync::{broadcast, Notify};
use cf_tokio::time::{self, Duration, Instant};
...
struct Shared {
    ...
    /// is considered a "blocking" operation and `cf_tokio::task::spawn_blocking`
    /// should be used.
    state: Mutex<State>,
...
impl Db {
    ...
    pub(crate) fn new() -> Db {
        ...
        cf_tokio::spawn(purge_expired_tasks(shared.clone()));
