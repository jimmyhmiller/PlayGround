...
// ---------------------------------------------------------------------------
// Autoref specialization for debug_any on ValueType
// ---------------------------------------------------------------------------
//
// Not all `#[turbo_tasks::value]` types implement `Debug`. We use the autoref
// specialization pattern so that the macro-generated code picks the inherent
// method (returning `Some(debug_fn)`) when `T: Debug`, and falls through via
// `Deref` to the fallback (returning `None`) otherwise.

/// Primary wrapper — has an inherent `debug_any_fn` when `T: Debug`.
pub struct DebugAnySpecialize<T: 'static>(pub std::marker::PhantomData<T>);

/// Fallback wrapper — always returns `None`.
pub struct DebugAnyFallback<T: 'static>(std::marker::PhantomData<T>);

impl<T: 'static> std::ops::Deref for DebugAnySpecialize<T> {
    type Target = DebugAnyFallback<T>;
    fn deref(&self) -> &Self::Target {
        // SAFETY: Both types are ZSTs (contain only PhantomData), so pointer
        // reinterpretation is safe and no data is accessed.
        unsafe { &*(self as *const Self as *const Self::Target) }
    }
}

// When T: Debug, this inherent method is preferred over the Deref fallback.
impl<T: std::fmt::Debug + 'static> DebugAnySpecialize<T> {
    pub fn debug_any_fn(&self) -> Option<fn(&dyn std::any::Any) -> String> {
        Some(|any: &dyn std::any::Any| -> String {
    ...
    /// Format a single field value using Debug.
    pub fn format_value(&self, val: &T) -> String {
        format!("{val:#?}")
    }
}

// Fallback: T doesn't implement Debug.
impl<T: 'static> DebugAnyFallback<T> {
    pub fn debug_any_fn(&self) -> Option<fn(&dyn std::any::Any) -> String> {
        None
    }

    /// Fallback field formatting — show type name when Debug is not available.
    pub fn format_value(&self, _val: &T) -> String {
