...
/// The self-test harness: smokes, event-loop round-trips against a real
/// external client, then Node's own tests. Lib sources *and* test sources are
/// registered here; the script runner registers only the lib sources.
fn run() -> std::result::Result<(), String> {
    let sources: Vec<(&str, &str)> = NODE_SOURCES
        .iter()
        .chain(NODE_TEST_SOURCES.iter())
        .copied()
        .collect();
    let engine = Engine::new(&sources)?;

    // Phase 1: setup + smokes (synchronous; scheduling for async work).
    let phase1 = engine.with(|ctx| -> Result<()> {
        {
    ...
    let port_from_js: u32 = engine.with(|ctx| -> Result<u32> {
        {
    ...
    let http_port: u32 = engine.with(|ctx| -> Result<u32> {
        {
    ...
    let express_port: u32 = engine.with(|ctx| -> Result<u32> {
        {
    ...
    if let Ok(serve) = std::env::var("PORTABLE_NODE_SERVE") {
        let port: u32 = engine.eval_or(r#"globalThis._httpPort || 0"#, 0);
        if port == 0 {
...
fn run_script(script_path: &str) -> std::result::Result<(), String> {
    ...
    // Test sources are skipped here: user code doesn't need them.
    let engine = Engine::new(NODE_SOURCES)?;

    // The resolver treats an absolute path as step 2 of the CJS algorithm.
    engine.require(&abs_str)?;

    // No deadline: the loop exits when no I/O or timer work is pending, and a
    // listening server is pending forever, so this blocks until the script
    // winds all its work down.
    engine.run_until_idle()?;
    Ok(())
}
