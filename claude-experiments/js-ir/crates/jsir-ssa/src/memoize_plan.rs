...
use crate::cfg::{
    BinOp, BlockId, BlockKind, Cfg, Const, Instr, MemberKey, Op, PropKey, Term, UnOp, Value,
};
...
pub fn build_layout(cfg: &Cfg, infos: &[ScopeInfo], ranges: &Ranges) -> Result<MemoLayout, String> {
    ...
    let emitted = emitted_scopes(infos);
    ...
    let value_set: Vec<HashSet<Value>> = emitted
        .iter()
        .map(|i| i.scope.values.iter().copied().collect())
        .collect();
    ...
    let all_value_sets: Vec<HashSet<Value>> = infos
        .iter()
        .map(|i| i.scope.values.iter().copied().collect())
        .collect();
    ...
    let mut let_names: HashSet<String> = scope_outputs.iter().map(|v| em.name(*v)).collect();
    for p in &phi_params {
    ...
    fn emit_node(&self, node: &Node, out: &mut Vec<IrOp>, slot: &mut usize) -> Result<(), String> {
        match node {
            ...
            Node::If {
                ...
                if_op.regions.push(Region::with_block(Block {
                    args: vec![],
                    ops: vec![consequent],
                }));
                if_op.regions.push(Region::with_block(Block {
    ...
    fn emit_block(&self, b: BlockId, out: &mut Vec<IrOp>, slot: &mut usize) -> Result<(), String> {
        ...
        for (ix, ins) in instrs.iter().enumerate() {
            let owner = self
                .value_set
                .iter()
                .position(|vs| match (&ins.op, ins.result) {
        ...
        let last_owned: Vec<usize> = owned
            .iter()
        ...
        for (ix, ins) in instrs.iter().enumerate() {
            match instr_owner[ix] {
                Some(s) if ix == last_owned[s] => {
                    out.push(
                        self.em
                            .emit_scope(instrs, &owned[s], self.emitted[s], slot)?,
                    );
                }
        ...
        match &block.term {
            Term::Ret(v) => {
                if stop.is_some() {
                    ...
                    return Err(
                        "memoize_plan: early return inside branch (deferred to later step)".into(),
                    );
                }
...
fn build_arm(
    ...
    if arm == join {
        ...
        if !edge_args.is_empty() {
            ...
            return Err(
                "memoize_plan: conditional edge args to a non-join arm (unsupported)".into(),
            );
        }
...
fn find_join(
    ...
    match join {
        Some(j) => Ok(j),
        None => {
            Err("memoize_plan: branch arms do not reconverge (early return / divergence)".into())
        }
    }
}
...
fn check_soundness(
    ...
    // `all_value_sets`.
    let mut all_value_scope: std::collections::HashMap<Value, usize> =
    ...
    if !emitted.is_empty() {
        for b in &cfg.blocks {
            if let crate::cfg::Term::CondBr { cond, .. } = &b.term {
                if reachable_values(*cond)
                    .iter()
                    .any(|v| ranges.is_mutable_after_def(*v))
                {
                    return Err(
...
impl<'a> Emitter<'a> {
    ...
    fn emit_op_value(&self, op: &Op, eb: &mut ExprBuilder) -> Result<ValueId, String> {
        match op {
            ...
            Op::Bin(b, x, y) => {
                ...
                e.operands.push(yv);
                e.attrs
                    .push(("operator_".into(), Attr::Str(bin_str(*b).into())));
                Ok(eb.push(e))
            }
            Op::Un(u, x) => {
                let xv = eb.push(ident_op(&self.name(*x)));
                ...
                e.operands.push(xv);
                e.attrs
                    .push(("operator_".into(), Attr::Str(un_str(*u).into())));
            ...
            Op::StoreMember { obj, prop, value } => {
                ...
                assign
                    .attrs
                    .push(("operator_".into(), Attr::Str("=".into())));
                Ok(eb.push(assign))
            }
            Op::Call { callee, args } => {
            ...
            Op::MakeObject(props) => {
                ...
                obj.regions.push(Region::with_block(Block {
                    args: vec![],
                    ops: region_ops,
                }));
                Ok(eb.push(obj))
    ...
    fn emit_real_expr(&self, v: Value, eb: &mut ExprBuilder) -> Result<ValueId, String> {
        ...
        if let Some(idx) = self.cfg.params.iter().position(|p| *p == v) {
            let name = self
                ...
                .get(idx)
                .cloned()
                .unwrap_or_else(|| format!("_p{}", v.0));
            return Ok(eb.push(ident_op(&name)));
        }
        let op = self
            .def_op_of(v)
            .ok_or_else(|| format!("inplace: dep %{} is not a param and has no def", v.0))?;
        match op {
            Op::Member { obj, prop } => {
                ...
                for (k, val) in props {
                    ...
                    match k {
                        PropKey::Ident(s) => {
                            pop.attrs.push((
                                "literal_key".into(),
                                Attr::Identifier(Box::new(ident_attr(s))),
                            ));
                            pop.attrs.push(("shorthand".into(), Attr::Bool(false)));
                        }
                        PropKey::Computed(c) => {
            ...
            other => Err(format!(
                "inplace: value %{} is {} (cannot reconstruct expression)",
                v.0,
                op_kind(other)
            )),
        }
    ...
    fn convert_decl_to_assign(&self, decl: IrOp) -> Result<IrOp, String> {
        ...
        let init_id = *init
            .results
            .first()
            .ok_or("inplace: initializer op has no result value")?;
        let mut eb = self.eb();
        let lhs = eb.push(ident_ref_op(&name));
        eb.ops.push(init);
        let mut assign = IrOp::new("jsir.assignment_expression");
    ...
    fn rewrite_run(
        ...
        while let Some(group) = iter.next() {
            let root = group
                .last()
                .cloned()
                .unwrap_or_else(|| IrOp::new("jsir.__empty"));
...
fn op_kind(op: &Op) -> &'static str {
    match op {
        Op::Const(_) => "Const",
        Op::ReadVar(_) => "ReadVar",
        Op::WriteVar(_, _) => "WriteVar",
        Op::Bin(_, _, _) => "Bin",
        Op::Un(_, _) => "Un",
        Op::Global(_) => "Global",
        Op::Call { .. } => "Call",
        Op::Member { .. } => "Member",
        Op::StoreMember { .. } => "StoreMember",
        Op::MakeObject(_) => "MakeObject",
...
fn wrap_stmt_run(ops: Vec<IrOp>) -> IrOp {
    ...
    holder
        .regions
        .push(Region::with_block(Block { args: vec![], ops }));
    holder
}
...
fn var_decl(kind: &str, region_ops: Vec<IrOp>) -> IrOp {
    ...
    decl.attrs
        .push(("kind".into(), Attr::Str(kind.to_string())));
    decl.regions.push(Region::with_block(Block {
        args: vec![],
        ops: region_ops,
    }));
...
fn const_op(c: &Const) -> Result<IrOp, String> {
    Ok(match c {
        ...
        Const::Num(bits) => {
            ...
            op.attrs
                .push(("extra".into(), Attr::NumericLiteralExtra { raw, value: n }));
            op
        }
...
pub fn memoize_inplace(
    ...
    for info in &emitted {
        // Multi-output scopes (React caches several values in one guard, e.g.
        // `$[3] = a; $[4] = b`) are emitted when ALL outputs are NAMED: each gets
        // a slot, is hoisted to a `let`, assigned in the guard, restored in the
        // else. The anon-output extraction path only handles ONE temp, so a
        // multi-output scope WITH an anonymous output still bails (below).
        let dep_base = slot;
        let out_base = dep_base + info.deps.len();
        slot = out_base + info.outputs.len();
    ...
    let scope_outputs: HashSet<Value> = emitted
        .iter()
    ...
    for p in &plans {
        if let Some(expr_node) = p.anon_expr {
            if !patch_expr_to_ident(&mut new_file, expr_node, &p.outs[0].1) {
                return Err(
                    "inplace: could not locate anonymous output expression to patch".into(),
                );
            }
        }
    }
...
fn emitted_scopes(infos: &[ScopeInfo]) -> Vec<&ScopeInfo> {
    let mut emitted: Vec<&ScopeInfo> = infos.iter().filter(|i| !i.outputs.is_empty()).collect();
    emitted.sort_by_key(|i| scope_info_sort_key(i));
    emitted
}

fn scope_info_sort_key(i: &ScopeInfo) -> (u32, u32, Vec<u32>, Vec<u32>, Vec<u32>) {
...
fn flatten_run_into(op: IrOp, out: &mut Vec<IrOp>) {
    if op.name == "jsir.__stmt_run" {
        if let Some(block) = op
            ...
            .into_iter()
            .next()
            .and_then(|r| r.blocks.into_iter().next())
        {
    ...
    if let Some(prog) = first_op_mut(file, "jsir.program") {
        if let Some((_, a)) = prog.attrs.iter_mut().find(|(k, _)| k == "source_type") {
            *a = Attr::Str("module".into());
            ...
            prog.attrs
                .push(("source_type".into(), Attr::Str("module".into())));
        }
    }
}
...
/// Locate the SAME function `crate::lower::select_function` chose, by stable
/// `node_id` (the analysis and the in-place edit must target one function).
fn find_function_by_node_mut(op: &mut IrOp, target: u32) -> Option<&mut IrOp> {
    if op.name == "jsir.function_declaration" && op.node_id == Some(target) {
        return Some(op);
    }
    for r in &mut op.regions {
        for b in &mut r.blocks {
            for o in &mut b.ops {
                if let Some(f) = find_function_by_node_mut(o, target) {
                    return Some(f);
                }
            }
        }
...
fn function_body_stmts_mut(file: &mut IrOp) -> Result<&mut Vec<IrOp>, String> {
    // `selected_function_node_id` derives the id from this same tree, so the
    // by-node lookup always resolves when it is `Some`; `None` (no node_id at
    // all) falls back to the first function, which both sides agree on.
    let func = match crate::lower::selected_function_node_id(file) {
        Some(t) => find_function_by_node_mut(file, t),
        None => find_function_mut(file),
    }
    .ok_or("inplace: no function declaration")?;
    let body_block = func
        .regions
        .get_mut(1)
        .and_then(|r| r.blocks.first_mut())
    ...
    if body_block.name != "jshir.block_statement" {
        return Err(format!(
            "inplace: expected body block_statement, got {}",
            body_block.name
        ));
    }
    body_block
...
fn contains_node(op: &IrOp, node: u32) -> bool {
    ...
    op.regions.iter().any(|r| {
        r.blocks
            .iter()
            .any(|b| b.ops.iter().any(|o| contains_node(o, node)))
    })
}
...
fn decl_name(stmt: &IrOp) -> Result<String, String> {
    if stmt.name != "jsir.variable_declaration" {
        return Err(format!(
            "inplace: scope statement is {}, expected variable_declaration",
            stmt.name
        ));
    }
    let ops = stmt
...
fn function_body_stmts(file: &IrOp) -> Result<&Vec<IrOp>, String> {
    fn find_function(op: &IrOp) -> Option<&IrOp> {
        ...
        if op.name == "jsir.export_named_declaration"
            || op.name == "jsir.export_default_declaration"
        {
            return op
                .regions
                .first()
    ...
    // Locate the SAME function the analysis lowered (`crate::lower::select_function`),
    // by its stable `node_id`; fall back to the first function when no id.
    fn find_by_node(op: &IrOp, target: u32) -> Option<&IrOp> {
        if op.name == "jsir.function_declaration" && op.node_id == Some(target) {
            return Some(op);
        }
        for r in &op.regions {
            for b in &r.blocks {
                for o in &b.ops {
                    if let Some(f) = find_by_node(o, target) {
                        return Some(f);
                    }
                }
            }
    ...
    let func = crate::lower::selected_function_node_id(file)
        .and_then(|t| find_by_node(file, t))
        .or_else(|| find_function(file))
        .ok_or("inplace: no function declaration")?;
    let body_block = func
        .regions
        .get(1)
