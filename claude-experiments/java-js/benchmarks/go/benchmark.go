...
const (
	smallFunction = `function add(a, b) {
    return a + b;
}`

	smallClass = `function Calculator() {
    this.result = 0;
}
...
Calculator.prototype.subtract = function(a, b) {
	...
	mediumAsyncModule = `function UserDataFetcher() {
    this.cache = {};
}
...
function debounce(func, wait) {
	...
	largeModule = `function DataProcessor(options) {
    options = options || {};
...
func benchmark(name string, code string, iterations int) BenchmarkResult {
	// Warmup
	for i := 0; i < 1000; i++ {
		api.Transform(code, api.TransformOptions{
			Loader: api.LoaderJS,
		})
	}
	...
	avgMicros := float64(elapsed.Microseconds()) / float64(iterations)
...
func runBenchmarkSuite(suiteName string, code string, iterations int) {
	fmt.Println("\n============================================================")
	fmt.Printf("Benchmark: %s\n", suiteName)
	fmt.Printf("Code size: %d characters\n", len(code))
	fmt.Println("============================================================\n")

	result := benchmark("esbuild (Go)", code, iterations)

	fmt.Println("Results:")
	fmt.Println("------------------------------------------------------------")
	fmt.Printf("%-20s | %15s\n", "Parser", "Avg Time (µs)")
	fmt.Println("------------------------------------------------------------")
	fmt.Printf("🥇 %-18s | %15.3f\n", result.Name, result.AvgMicros)
}
...
func main() {
	...
	fmt.Println("\n════════════════════════════════════════════════════════════")
	fmt.Println("JavaScript Parser Benchmarks (Go-based parsers)")
	fmt.Println("════════════════════════════════════════════════════════════")
	fmt.Println("Warmup iterations: 1000")
	fmt.Printf("Measurement iterations: %d\n", iterations)
	fmt.Println("════════════════════════════════════════════════════════════")

	runBenchmarkSuite("Small Function", smallFunction, iterations)
	runBenchmarkSuite("Small Class", smallClass, iterations)
	runBenchmarkSuite("Medium Async Module", mediumAsyncModule, iterations)
	runBenchmarkSuite("Large Module", largeModule, iterations)
