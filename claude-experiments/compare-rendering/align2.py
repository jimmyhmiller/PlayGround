...
def main():
    print("Loading...")
    img1 = load_normalized("new.png")
    img2 = load_normalized("new2.png")
    ...
    cv2.imwrite("new_overlay.png", overlay)
    cv2.imwrite("new_img1.png", img1)
    cv2.imwrite("new_img2_aligned.png", aligned)

    # Difference
    diff = cv2.absdiff(img1, aligned)
    print(f"\nDiff stats: mean={np.mean(diff):.2f}, max={np.max(diff)}")

    cv2.imwrite("new_diff.png", np.clip(diff * 10, 0, 255).astype(np.uint8))
    ...
    cv2.imwrite("new_signed.png", signed_viz)

    print("\nSaved: new_overlay.png, new_diff.png, new_signed.png")
