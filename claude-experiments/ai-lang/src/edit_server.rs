...
/// Run the JSONL edit server on stdin/stdout against `cb_path`.
pub fn serve(cb_path: &PathBuf) {
    let mut state = match ServerState::open(cb_path) {
        Ok(s) => s,
        Err(e) => {
            eprintln!("edit server: {}", e);
            std::process::exit(1);
        }
    };
    ...
    for line in reader.lines() {
        let line = match line {
            Ok(l) => l,
            Err(e) => {
                eprintln!("edit server: read error: {}", e);
                break;
            }
        };
        ...
        let req = match jsonl::parse(&line) {
            ...
            Err(e) => {
                let resp = Json::obj([
                    ...
                    ("ok".to_string(), Json::Bool(false)),
                    ("error".to_string(), Json::obj([
                        ("kind".to_string(), Json::Str("ParseError".to_string())),
                        ("message".to_string(), Json::Str(format!("{}", e))),
                    ])),
                ]);
                let _ = writeln!(stdout, "{}", resp.to_string());
                let _ = stdout.flush();
                continue;
            }
        };
