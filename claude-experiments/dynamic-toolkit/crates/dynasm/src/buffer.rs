...
impl ExecutableBuffer {
    /// Create an executable buffer from raw code bytes.
    ///
    /// # Safety
    /// The caller must ensure `code` contains valid machine code for the
    /// current architecture.
    pub unsafe fn new(code: &[u8]) -> Self {
        use std::ptr;

        let len = code.len();
        assert!(len > 0, "cannot create empty executable buffer");

        // Round up to page size
        let page_size = unsafe { libc::sysconf(libc::_SC_PAGESIZE) } as usize;
        let alloc_len = (len + page_size - 1) & !(page_size - 1);
...
// ExecutableBuffer is Send+Sync because once created the memory is read-only executable.
