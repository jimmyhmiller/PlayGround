...
;; ========================================================================
;; Agent system: stubbed out for clojure-jvm fork.
;;
;; Upstream's agent definitions reference clojure.lang.Agent$Action/
;; pooledExecutor and friends, which we don't host. The agent semantics
;; (background actors, error handling, await) require real thread-pool
;; infrastructure that's out of scope. We define each Var as a no-op fn
;; so dependent forms (await, await-for, etc.) analyze cleanly. Calling
;; any of these returns nil — NOT safe for concurrent use.
;; ========================================================================
(def agent (fn* [& _] nil))
(def set-agent-send-executor! (fn* [& _] nil))
(def set-agent-send-off-executor! (fn* [& _] nil))
(def send-via (fn* [& _] nil))
(def send (fn* [& _] nil))
(def send-off (fn* [& _] nil))
(def release-pending-sends (fn* [& _] nil))
(def add-watch (fn* [& _] nil))
(def remove-watch (fn* [& _] nil))
(def agent-error (fn* [& _] nil))
(def restart-agent (fn* [& _] nil))
(def set-error-handler! (fn* [& _] nil))
(def error-handler (fn* [& _] nil))
(def set-error-mode! (fn* [& _] nil))
(def error-mode (fn* [& _] nil))
(def agent-errors (fn* [& _] nil))
(def clear-agent-errors (fn* [& _] nil))
(def shutdown-agents (fn* [& _] nil))
...
;; clojure-jvm stub: java.util.concurrent.Future not hosted.
;; Original upstream:
...
(def deref-future
  (fn* [& _]
    (throw "deref-future: java.util.concurrent.Future not supported in clojure-jvm fork")))
     
;; clojure-jvm: arity-3 form removed (depends on clojure.lang.IBlockingDeref
;; which we don't host). Arity-1 stays — used for @atom / @var / @delay.
;; Original upstream:
;; (defn deref
;;   "..."
...
;; clojure-jvm: removed (import). Original: (import '(java.lang.reflect Array))
...
;; clojure-jvm: removed. Original: (import clojure.lang.ExceptionInfo clojure.lang.IExceptionInfo)
...
;; clojure-jvm: removed. Original: (import '(java.util.concurrent BlockingQueue LinkedBlockingQueue))
