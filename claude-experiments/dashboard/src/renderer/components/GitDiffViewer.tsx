...
function parseDiffLines(diff: string | null): DiffLine[] {
  if (!diff) return [];
...
const lineStyleVars: Record<DiffLine['type'], { bg: string; color: string }> = {
  addition: {
...
const GitDiffViewer = memo(function GitDiffViewer({
  ...
  const diffEvent = useLatestEvent('git.diff.updated') as DashboardEvent<GitDiffPayload> | null;
  const statusEvent = useLatestEvent('git.status.changed') as DashboardEvent<GitStatusPayload> | null;
