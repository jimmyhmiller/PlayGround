 ...
 */
export function ComponentControls(): ReactElement {
  const { addInstance } = useComponentInstances();
  const [filePath, setFilePath] = useState('');

  const handleAddEditor = (): void => {
  ...
  const handleWatchFile = async (): Promise<void> => {
    if (!filePath) return;
    await window.fileAPI.watch(filePath);
  };

  const handleRefreshGit = async (): Promise<void> => {
    await window.gitAPI.refresh();
  };

  const handleStartGitPolling = async (): Promise<void> => {
    await window.gitAPI.startPolling(2000);
  };

  const handleStopGitPolling = async (): Promise<void> => {
