import { createContext, useContext, useCallback, memo, ReactNode, ComponentType } from 'react';
import { useComponentsState } from '../hooks/useBackendState';
import type { ComponentInstance } from '../../types/state';
...
interface ComponentTypeInfo<T = any> {
  Component: ComponentType<T>;
  defaultProps: Record<string, unknown>;
}

// Registry of available component types (in-memory, for component lookups)
const componentTypes = new Map<string, ComponentTypeInfo>();

/**
 * Register a component type
 * @param type - Component type name (e.g., 'codemirror', 'git-diff')
 * @param Component - The React component
 * @param defaultProps - Default props for this component type
 */
 ...
 */
export function getComponentType(type: string): ComponentTypeInfo | undefined {
...
interface ComponentInstancesContextValue {
  ...
  addInstance: (type: string, props?: Record<string, unknown>) => Promise<string | null>;
  removeInstance: (id: string) => void;
  updateInstanceProps: (id: string, newProps: Record<string, unknown>) => void;
 ...
 */
export interface ComponentInstancesProviderProps {
  children: ReactNode;
}

export function ComponentInstancesProvider({ children }: ComponentInstancesProviderProps) {
  const {
    instances,
  ...
  const addInstance = useCallback(async (type: string, props: Record<string, unknown> = {}): Promise<string | null> => {
  ...
  const updateInstanceProps = useCallback((id: string, newProps: Record<string, unknown>) => {
    backendUpdateInstanceProps(id, newProps);
  }, [backendUpdateInstanceProps]);

  const value: ComponentInstancesContextValue = {
    instances,
    addInstance,
    removeInstance,
    updateInstanceProps,
 ...
 */
interface DynamicComponentInstanceProps {
  instance: ComponentInstance;
  onRemove: (id: string) => void;
}

interface DynamicComponentInnerProps {
  instanceId: string;
  subscribePattern?: string;
  [key: string]: any;
}

const DynamicComponentInstance = memo(function DynamicComponentInstance({ instance, onRemove }: DynamicComponentInstanceProps) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <span>
          ...
          <span style={{ color: '#888', marginLeft: '8px' }}>
            {instance.props.subscribePattern ? `[${String(instance.props.subscribePattern)}]` : null}
          </span>
        </span>
        <button
      ...
      <Component {...instance.props as DynamicComponentInnerProps} instanceId={instance.id} />
    </div>
  );
});
