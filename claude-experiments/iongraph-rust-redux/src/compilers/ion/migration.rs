...
#[cfg(feature = "serde")]
pub fn migrate(mut ion_json: Value) -> IonJSON {
...
#[cfg(feature = "serde")]
fn migrate_block_v0_to_v1(block: &mut Value) {
        ...
        if let Some(preds) = obj.get("predecessors") {
            if let Some(preds_array) = preds.as_array() {
                let migrated: Vec<_> = preds_array
                    ...
                    .collect();
                obj.insert("predecessors".to_string(), json!(migrated));
            }
        }
        ...
        if let Some(succs) = obj.get("successors") {
            if let Some(succs_array) = succs.as_array() {
                let migrated: Vec<_> = succs_array
                    ...
                    .filter_map(|v| v.as_u64().map(|n| n as u32))
                    .collect();
                obj.insert("successors".to_string(), json!(migrated));
            }
        }
    }
}

/// Migrate Ion JSON from version 0 to version 1 (non-serde version)
#[cfg(not(feature = "serde"))]
pub fn migrate(mut ion_json: Value) -> IonJSON {
    // Check if we need to migrate from version 0
    if ion_json.get("version").is_none() {
        // This is version 0, migrate to version 1
        ion_json["version"] = Value::from(1);
    ...
    IonJSON::from_json(&ion_json).expect("Failed to deserialize migrated IonJSON")
}
...
fn migrate_block_v0_to_v1(block: &mut Value) {
    ...
    if let Some(obj) = block.as_object_mut() {
        ...
        if let Some(preds) = obj.get("predecessors") {
            if let Some(preds_array) = preds.as_array() {
                let migrated: Vec<Value> = preds_array
                    .iter()
                    .filter_map(|v| v.as_u64().map(|n| Value::from(n as u32)))
                    .collect();
                obj.insert("predecessors".to_string(), Value::Array(migrated));
            }
        }
        ...
        if let Some(succs) = obj.get("successors") {
            if let Some(succs_array) = succs.as_array() {
                let migrated: Vec<Value> = succs_array
                    .iter()
                    .filter_map(|v| v.as_u64().map(|n| Value::from(n as u32)))
                    .collect();
                obj.insert("successors".to_string(), Value::Array(migrated));
            }
        }
    }
}
