...
def export():
    ...
    for i in range(n_layer):
        ...
        # Attention QKV weight: nn.Linear stores [out, in], we need [in, out]
        attn_w = sd[f"{prefix}.attn.c_attn.weight"]  # [n_embd, 3*n_embd] (GPT2 uses Conv1D)
        attn_b = sd[f"{prefix}.attn.c_attn.bias"]    # [3*n_embd]
        # GPT2 Conv1D weight is already [in, out], no transpose needed!
        add_tensor(f"attn_qkv_w_{i}", attn_w)
        add_tensor(f"attn_qkv_b_{i}", attn_b)

        # Attention output projection
        proj_w = sd[f"{prefix}.attn.c_proj.weight"]  # [n_embd, n_embd] (Conv1D: already [in, out])
