...
fn non_exported_items_are_not_importable() {
    ...
    let err2 = vm.eval("import { secret } from \"math\"").unwrap_err().to_string();
...
fn qualified_record_only_contains_exports() {
    let mut vm = vm_with(&[("math", MATH_MOD)]);
    let err = vm.eval("import \"math\" as m\nm.private_helper(1)").unwrap_err().to_string();
...
fn missing_module_fails_loudly() {
    let mut vm = vm_with(&[]);
    let err = vm.eval("import { x } from \"nope\"").unwrap_err().to_string();
...
"#;
    let mut vm = vm_with(&[
        ("counter", counter_mod),
        ("user_a", "import { bump } from \"counter\"\nexport fn poke_a() = bump()"),
        ("user_b", "import { bump } from \"counter\"\nexport fn poke_b() = bump()"),
    ]);
    let v = vm
        .eval(
...
fn module_cannot_see_main_globals() {
    ...
    let err = vm.eval("import { peek } from \"leaky\"").unwrap_err().to_string();
...
fn host_module_missing_export_fails_loudly() {
    ...
    let err = vm.eval("import { nope } from \"m\"").unwrap_err().to_string();
...
fn import_path_traversal_is_rejected() {
    let mut vm = vm_with(&[]);
    let err = vm.eval("import { x } from \"../evil\"").unwrap_err().to_string();
...
fn module_top_level_faults_surface() {
    ...
    let err = vm.eval("import { f } from \"bad\"").unwrap_err().to_string();
...
fn load_module_api_lists_exports() {
    ...
    exports.sort();
    assert_eq!(exports, vec!["answer".to_string(), "double".to_string(), "triple".to_string()]);
}
