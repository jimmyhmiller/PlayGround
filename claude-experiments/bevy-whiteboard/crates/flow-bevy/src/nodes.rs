...
/// Opt-in plugin that drops the starter graph into a fresh canvas on
/// Startup. `build_app` installs it so the binary opens with a demo;
/// tests deliberately don't install it so they start empty and build
/// whatever topology they actually need. Keeping the seed out of the
/// general plugin set means the demo can change shape without test
/// churn.
pub struct DemoSeedPlugin;
impl Plugin for DemoSeedPlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(Startup, seed_demo_graph);
    }
}

/// Seed the canvas with a three-colour routing + multi-worker demo:
///
/// ```text
///   Gen_R ─┐                      ┌── Queue_R ─── Worker_R1 ─────────────── Sink_R
///   Gen_Y ─┼─── Router ──────────┼── Queue_Y ─┬─ Worker_Y1 ────────────┐
///   Gen_B ─┘                      │            └─ Worker_Y2 ────────────┴─ Sink_Y
///                                 │            ┌─ Worker_B1 ───────────┐
///                                 └── Queue_B ─┼─ Worker_B2            ├─ Sink_B
///                                              └─ Worker_B3 ───────────┘
/// ```
///
/// What it shows at a glance:
///   * A router filtering by `slot_of(n, "color")` — packet colour is
///     preserved end-to-end from each generator to its matching sink.
///   * Workers per queue: 1 / 2 / 3 for red / yellow / blue. All
///     generators emit at the same rate, so the red lane is the most
///     back-pressured (lowest throughput), and the blue lane has the
///     most slack.
fn seed_demo_graph(
    ...
    mut hidden_edges: ResMut<crate::edges::HiddenEdges>,
    theme: Res<Theme>,
) {
    // Three colour lanes, each with a different number of workers so
    // the throughput difference is immediately visible on the sinks.
    // The `key` field disambiguates workers inside a lane (B1/B2/B3).
    struct Seed {
        kind: Kind,
        pos: Vec2,
        slot: usize,
    ...
    let layout: [Seed; 15] = [
        ...
        Seed { kind: Kind::Generator, pos: Vec2::new(-600.0,  260.0), slot: 0, key: "gen"    },
        Seed { kind: Kind::Generator, pos: Vec2::new(-600.0,    0.0), slot: 1, key: "gen"    },
        Seed { kind: Kind::Generator, pos: Vec2::new(-600.0, -260.0), slot: 2, key: "gen"    },
        // One router fans the three streams out to colour-matched queues.
        Seed { kind: Kind::Router,    pos: Vec2::new(-300.0,    0.0), slot: 0, key: "router" },
        // Queues — one per colour, slotted to match.
        Seed { kind: Kind::Queue,     pos: Vec2::new(-50.0,   260.0), slot: 0, key: "queue"  },
        Seed { kind: Kind::Queue,     pos: Vec2::new(-50.0,     0.0), slot: 1, key: "queue"  },
        Seed { kind: Kind::Queue,     pos: Vec2::new(-50.0,  -260.0), slot: 2, key: "queue"  },
        // Red lane: 1 worker.
        Seed { kind: Kind::Worker,    pos: Vec2::new( 230.0,  260.0), slot: 0, key: "w1"     },
        // Yellow lane: 2 workers stacked tight around y=0.
        Seed { kind: Kind::Worker,    pos: Vec2::new( 230.0,   40.0), slot: 1, key: "w1"     },
        Seed { kind: Kind::Worker,    pos: Vec2::new( 230.0,  -40.0), slot: 1, key: "w2"     },
        // Blue lane: 3 workers stacked around y=-260.
        Seed { kind: Kind::Worker,    pos: Vec2::new( 230.0, -200.0), slot: 2, key: "w1"     },
        Seed { kind: Kind::Worker,    pos: Vec2::new( 230.0, -260.0), slot: 2, key: "w2"     },
        Seed { kind: Kind::Worker,    pos: Vec2::new( 230.0, -320.0), slot: 2, key: "w3"     },
        // One sink per colour — workers in the same lane fan-in.
        Seed { kind: Kind::Sink,      pos: Vec2::new( 520.0,  260.0), slot: 0, key: "sink"   },
        Seed { kind: Kind::Sink,      pos: Vec2::new( 520.0,    0.0), slot: 1, key: "sink"   },
    ];
    // Trailing entry below because the array-init above caps at 15. The
    // blue sink comes from the spare element we've yet to add.
    let blue_sink_seed = Seed { kind: Kind::Sink, pos: Vec2::new(520.0, -260.0), slot: 2, key: "sink" };

    let mut ids: Vec<(NodeId, usize, &'static str)> = Vec::new();
    let place = |seed: &Seed,
                     ids: &mut Vec<(NodeId, usize, &'static str)>,
                     ...
                     theme: &Theme| {
        counter.0 += 1;
        let name = format!("{}_{}", seed.kind.label(), counter.0);
        let fid = spawn_gadget(&mut flow.sim, seed.kind, &name, seed.slot);
        spawn_node_entity(commands, meshes, materials, maps, theme, fid, seed.kind, name, seed.pos);
        if !matches!(seed.kind, Kind::Router) {
            node_colors.0.insert(fid, theme.data[seed.slot]);
        }
        ids.push((fid, seed.slot, seed.key));
    };

    for seed in &layout {
        place(seed, &mut ids, &mut counter, &mut flow,
              &mut commands, &mut meshes, &mut materials,
              &mut maps, &mut node_colors, &theme);
    }
    place(&blue_sink_seed, &mut ids, &mut counter, &mut flow,
          &mut commands, &mut meshes, &mut materials,
          &mut maps, &mut node_colors, &theme);

    let find = |slot: usize, key: &str| -> NodeId {
        ids.iter()
            .find(|(_, s, k)| *s == slot && *k == key)
            .map(|(id, _, _)| *id)
            .unwrap_or_else(|| panic!("seed missing: slot={} key={}", slot, key))
    };
    let router = ids.iter().find(|(_, _, k)| *k == "router").unwrap().0;

    // Tune rates. Each generator emits at 30/s. Each worker serves at
    // 50ms (20/s). So per lane:
    //   red    (1 worker): 20/s capacity vs 30/s demand → backs up ~10/s.
    //   yellow (2 workers): 40/s capacity → headroom.
    //   blue   (3 workers): 60/s capacity → lots of headroom.
    for slot in 0..3 {
        flow.sim.nodes.get_mut(&find(slot, "gen")).unwrap()
            .slots.insert("period_ns".into(), flow::Value::Int(33_000_000));
    }
    let worker_keys_per_slot: [&[&str]; 3] = [&["w1"], &["w1", "w2"], &["w1", "w2", "w3"]];
    for (slot, keys) in worker_keys_per_slot.iter().enumerate() {
        for key in *keys {
            flow.sim.nodes.get_mut(&find(slot, key)).unwrap()
                .slots.insert("service_ns".into(), flow::Value::Int(50_000_000));
        }
    }

    // Wiring. Every edge goes through the shared `wire_flow_edge` so
    // pull semantics (Worker→Queue reverse edges, upstream/downstream
    // slot wiring) happen the same way the Connect tool does them.
    let mut wire_pair = |from_nid: NodeId, from_kind: Kind, to_nid: NodeId, to_kind: Kind| {
        crate::edges::wire_flow_edge(
            &mut flow.sim,
            &mut maps,
            &mut hidden_edges,
            &mut commands,
            from_nid, Some(from_kind),
            to_nid,   Some(to_kind),
        );
    };

    // Generators → Router (one per colour).
    for slot in 0..3 {
        wire_pair(find(slot, "gen"), Kind::Generator, router, Kind::Router);
    }
    // Router → Queues (one per colour — colour match picks the right
    // branch at runtime).
    for slot in 0..3 {
        wire_pair(router, Kind::Router, find(slot, "queue"), Kind::Queue);
    }
    // Workers → Queues (pull edges) and Workers → Sinks (push edges).
    for (slot, keys) in worker_keys_per_slot.iter().enumerate() {
        let queue = find(slot, "queue");
        let sink  = find(slot, "sink");
        for key in *keys {
            let worker = find(slot, key);
            wire_pair(worker, Kind::Worker, queue, Kind::Queue);
            wire_pair(worker, Kind::Worker, sink,  Kind::Sink);
        }
    }
}
