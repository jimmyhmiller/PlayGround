...
fn baseline_load_example_only_worker_mode() {
    ...
    let n = app.world().resource::<flow_bevy::edges::VisualTimelineRes>().0.packets.len();
...
fn repro_then_example_worker_mode_packets_visible() {
    ...
    let repro_packets = app.world().resource::<flow_bevy::edges::VisualTimelineRes>().0.packets.len();
