...
// src/rainbow-brackets.js
var colors = [
  "#c0caf5",
  // level 0 — soft white (blends with base text)
  "#7aa2f7",
  // level 1 — blue
  "#bb9af7",
  // level 2 — purple
  ...
  // level 5 — green
  "#f7768e"
  // level 6 — pink (not error-red)
];
var classNames = colors.map((_, i) => `rb-depth-${i}`);
var plugin = ViewPlugin.fromClass(class {
  ...
  build(view) {
    ...
    const openers = { "(": ")", "[": "]", "{": "}" };
    const closers = { ")": "(", "]": "[", "}": "{" };
    for (let pos = 0; pos < doc2.length; pos++) {
      const ch = doc2.sliceString(pos, pos + 1);
      if (openers[ch]) {
    ...
    decorations2.sort((a, b) => a.from - b.from || a.startSide - b.startSide);
    return Decoration.set(decorations2);
  }
}, {
  decorations: (v) => v.decorations
});
var theme2 = EditorView.baseTheme(
