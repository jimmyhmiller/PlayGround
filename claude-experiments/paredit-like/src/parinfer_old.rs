...
impl Parinfer {
    ...
    pub fn balance(&self) -> Result<String> {
        // Strategy: Ignore all closing parens and regenerate from indentation
        let mut result = Vec::new();
        let mut paren_stack: Vec<(ParenType, usize)> = Vec::new(); // (paren type, line index)
    ...
    fn remove_extra_parens(&self) -> Result<Vec<String>> {
        let mut result = Vec::new();
        ...
        for (line_idx, line) in self.lines.iter().enumerate() {
            ...
            for ch in line.text.chars() {
                ...
                if !in_string && !in_comment {
                    match ch {
                        '(' | '[' | '{' => {
                            paren_stack.push(ParenType::Open(ch));
                            new_line.push(ch);
                        }
                        ')' | ']' | '}' => {
                            let paren_type = ParenType::Close(ch);
                            if let Some(open_paren) = paren_stack.last() {
                                if open_paren.matches(&paren_type) {
                                    paren_stack.pop();
                                    new_line.push(ch);
                                    eprintln!("Line {}: matched {} with stack top, adding", line_idx, ch);
                                } else {
                                    eprintln!("Line {}: {} doesn't match stack top {:?}, skipping", line_idx, ch, open_paren);
                                }
                                // If doesn't match, skip (remove mismatched paren)
                            } else {
                                eprintln!("Line {}: {} but stack empty, skipping", line_idx, ch);
                            }
                            // If stack empty, skip (remove extra closing paren)
                        }
                        _ => new_line.push(ch),
                    }
                } else {
            ...
            eprintln!("Line {}: '{}' -> '{}'", line_idx, line.text, new_line);
    ...
    fn add_missing_parens(&self, cleaned_lines: &[String]) -> Result<String> {
        let mut result = Vec::new();
        let mut paren_stack: Vec<(ParenType, usize)> = Vec::new(); // (paren type, line index)
        let mut indent_stack: Vec<usize> = Vec::new();

        for (line_idx, line_text) in cleaned_lines.iter().enumerate() {
            ...
            // Process the cleaned line to track opens/closes and update paren_stack
                ...
                if !in_string && !in_comment {
                    match ch {
                        '(' | '[' | '{' => {
                            ...
                            paren_stack.push((paren_type, line_idx));
                            indent_stack.push(new_line.len());
                            new_line.push(ch);
                        }
                        ')' | ']' | '}' => {
                            let paren_type = ParenType::Close(ch);
                            if let Some((open_paren, _)) = paren_stack.last() {
                                if open_paren.matches(&paren_type) {
                                    paren_stack.pop();
                                    indent_stack.pop();
                                    new_line.push(ch);
            ...
            // Check if we need to close parens at end of line based on indentation
            if !in_string && !in_comment {
                ...
                eprintln!("Line {}: '{}', indent={}, next_indent={}, stack_empty={}", line_idx, line.text.chars().take(20).collect::<String>(), line.indent, next_indent, paren_stack.is_empty());

                if next_indent <= line.indent && !paren_stack.is_empty() {
                    eprintln!("Line {}: indent={}, next_indent={}, stack_len={}", line_idx, line.indent, next_indent, paren_stack.len());
                    // Close parens that should end at this indentation level
                    let mut to_close = Vec::new();
                    while let Some((paren, _)) = paren_stack.last() {
                        if let Some(indent) = indent_stack.last() {
                            eprintln!("  Checking paren at indent {}", indent);
                            // Close parens opened at positions >= next_indent
                    ...
                    eprintln!("  Adding {} closing parens", to_close.len());
                    // Add closing parens
        ...
        // Close any remaining open parens at the end
        while let Some((paren, _)) = paren_stack.pop() {
            if let Some(close_char) = paren.closing_char() {
...
mod tests {
    ...
    fn test_balance_mismatched_parens() {
        ...
        // Should remove mismatched parens
        assert!(!result.contains(")["));
