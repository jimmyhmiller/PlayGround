"""Benchmark with hipBLASLt, TunableOp, and no CUDAGraphs."""
...
def bench(name, d_model, n_heads, n_layers, do_compile, device, board, scalars, target):
    print(f"\n{'='*60}", flush=True)
    print(f"Testing: {name} (d={d_model}, h={n_heads}, L={n_layers})", flush=True)
    ...
    if do_compile:
        print("  Compiling (no cudagraphs)...", flush=True)
    ...
    # Warmup / tuning
    print("  Warmup (TunableOp will auto-tune on first pass)...", flush=True)
    for i in range(10):
...
if __name__ == "__main__":
    ...
    configs = [
        ("d128 compile+tuned fp16",     128, 4, 4, True),
        ("d256 compile+tuned fp16",     256, 8, 4, True),
        ("d256 6L compile+tuned fp16",  256, 8, 6, True),
    ]

    for name, d_model, n_heads, n_layers, do_compile in configs:
        bench(name, d_model, n_heads, n_layers, do_compile, device, board, scalars, target)
