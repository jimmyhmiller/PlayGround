"""Quick benchmark: native gfx1151 + compile + flash attention."""
...
def bench(name, d_model, n_heads, n_layers, device, board, scalars, target):
    print(f"\n{'='*60}", flush=True)
...
if __name__ == "__main__":
    ...
    print(f"Flash attention: EXPERIMENTAL ENABLED", flush=True)

    board = torch.randn(8192, 12, 8, 8, device=device)
    scalars = torch.randn(8192, 6, device=device)
    target = torch.randn(8192, device=device)

    bench("d128 compile+flash fp16", 128, 4, 4, device, board, scalars, target)
    bench("d256 compile+flash fp16", 256, 8, 4, device, board, scalars, target)
    bench("d256 compile+flash fp16 6L", 256, 8, 6, device, board, scalars, target)
