...
`functions`, `trace`, `generation`. Two **mutating** reflection ops: `gc()` (full major) and
`minorGc()` (young-only minor) force a collection and return `{kind, liveBefore, liveAfter, freed,
byType[...]}`. They run on the full-STW eval path (they free memory), not the concurrent read-only
handoff. The viewer exposes them on the browser console as **`tools.gc()`** / **`tools.minorGc()`**
(log the reclaim summary + re-poll every pane so the drop in live counts is visible immediately) —
debugging affordances kept in the console, not as UI chrome, since the viewer is itself a live REPL.
The two make the generational split observable: on floating garbage, `minorGc()` reclaims the young
objects and *leaves the ones already promoted to OLD*, then `gc()` sweeps those too.
