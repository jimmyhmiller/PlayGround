...
(time
 (for [playlist-id (intersect (get-in indexes [:playlist/name "Heavy Metal Classic"])
                              (into [] (keys (get-in indexes [:playlist-track/playlist]))))]
   (for [track-id (intersect (get-in indexes [:playlist-track/playlist playlist-id])
                             (into [] (keys (get-in indexes [:playlist-track/track]))))]
     [playlist-id track-id (get-in indexes [:playlist-track/track track-id])])))
