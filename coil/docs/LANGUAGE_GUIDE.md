...
- **Clean prelude operators**: `+ - * / %`, `= != < <= > >=`, `& | ^ << >>`.
  ...
  For `u8`/`u32` and other widths, use the metal ops. Prefer clean operators on
  `i64`/`bool`/pointers; drop to metal for other widths.
...
**Trait objects:** `(defdyn Trait)` after the trait reflects its methods into a vtable.
A `(dyn Trait)` param then accepts a pointer to *any* implementer — the coercion is
automatic at call/return sites, or explicit with `(make-dyn Trait p)`. The trait's
methods must take `(self (ptr Self))`. ⚠ `defdyn` lives in the `dyn` module, which —
unlike every other library listed at the end — is **not** bundled into the compiler;
you need its source alongside your program.
