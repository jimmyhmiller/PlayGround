...
`src/compiler/reader.coil` + `tests/compiler/oracle/` — study it before doing a new pass.
...
2. **Corpus + gate.** Snapshot the Rust dump over the whole corpus into
   `tests/compiler/oracle/<pass>/reference/`, plus curated error/edge fixtures. Write a
...
3. **Scaffold.** Write `src/compiler/<pass>.coil`: the data types (mirror the Rust ones),
...
- Low-level Lisp. Study `src/stdlib/sexp.coil`, `src/stdlib/str.coil`, `src/stdlib/slice.coil`,
  `src/stdlib/arraylist.coil`, `src/stdlib/result.coil`, `src/stdlib/hashmap.coil`, `src/examples/json.coil`,
  `src/examples/calc.coil`, and `src/compiler/reader.coil` (the worked example).
...
- Error propagation: `src/stdlib/try.coil` — `(try …)`, `(try! e)` unwrap-or-Err, `(try? e)`.
...
- Build a self-host pass binary: `build/bin/coil build src/compiler/<pass>.coil -o /tmp/coil-self-<pass>`.
- Run all .coil work from the REPO ROOT (imports are `"src/stdlib/…"` CWD-relative).
