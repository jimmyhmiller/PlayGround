...
#
#   scripts/benchmarks/run.sh            # run every benchmark
#   scripts/benchmarks/run.sh fib loop   # run a subset
#
...
for b in "${BENCHES[@]}"; do
  echo "=== $b ==="
  "$COIL" build "src/benchmarks/$b.coil" -o "$T/$b.coil" >/dev/null
  "$CC" -O3 "src/benchmarks/$b.c" -o "$T/$b.c3"
  ...
  zig_args=()
  if [ "$HAVE_ZIG" = 1 ] && [ -f "src/benchmarks/$b.zig" ] && build_zig "src/benchmarks/$b.zig" "$T/$b.zo" "$T/$b.zig"; then
    oz=$("$T/$b.zig")
    if [ "$oz" != "$oc" ]; then
