...
#   * bootstrap/seeds/native/coil-seed         — full compiler (LLVM + arm64), from main.coil
#   * bootstrap/seeds/native/coil-seed-nollvm  — LLVM-free compiler (arm64 only), from main_a64.coil
#
...
if [ "$WHICH" = both ] || [ "$WHICH" = full ]; then
  ...
  } > bootstrap/seeds/native/SEED_VERSION
  updated+=("bootstrap/seeds/native/coil-seed" "bootstrap/seeds/native/SEED_VERSION")
fi

if [ "$WHICH" = both ] || [ "$WHICH" = nollvm ]; then
  echo "=== [nollvm] verifying before touching the seed ==="
  ./scripts/compiler/rebootstrap-nollvm.sh /tmp/coil-newseed-nollvm || { echo "[nollvm] VERIFY FAILED — seed NOT updated"; exit 1; }
  ...
  } > bootstrap/seeds/native/SEED_VERSION_NOLLVM
  updated+=("bootstrap/seeds/native/coil-seed-nollvm" "bootstrap/seeds/native/SEED_VERSION_NOLLVM")
fi
