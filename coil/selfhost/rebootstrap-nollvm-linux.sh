...
#
# The produced compiler (from selfhost/src/main_x64.coil) omits the LLVM backend
...
#   1. $STAGE0 if set
#   2. selfhost/seed/coil-seed-nollvm-linux-x86_64  (the committed LLVM-free seed)
#   3. ./coil-linux                                 (the LLVM-backed local build)
#
...
#
# Usage: selfhost/rebootstrap-nollvm-linux.sh [install-dest]  (default: ./coil-nollvm-linux)
#        STAGE0=/path/to/coil selfhost/rebootstrap-nollvm-linux.sh
set -uo pipefail
cd "$(dirname "$0")/.."                 # repo root
SRC=selfhost/src/main_x64.coil
SEED=selfhost/seed/coil-seed-nollvm-linux-x86_64

if   [ -n "${STAGE0:-}" ];   then :
elif [ -x "$SEED" ];         then STAGE0="$SEED"
elif [ -x ./coil-linux ];    then STAGE0=./coil-linux
else echo "no stage0: need $SEED or ./coil-linux (or set STAGE0=/path/to/coil)"; exit 1
fi
echo "stage0 = $STAGE0"
...
./selfhost/oracle/x64/gate-run.sh /tmp/coil-nlx2 >/dev/null 2>&1 \
