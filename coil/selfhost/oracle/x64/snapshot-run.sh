#!/usr/bin/env bash
# Behavioral reference snapshot for the x64 backend gate.
#
...
# x64 gate (gate-run.sh) then builds the SAME programs with the self-host
# x64 backend and diffs runtime behavior byte-for-byte.  Runtime equality —
...
while IFS= read -r line; do
  ...
  f="$1"; shift
  id=$(echo "$f" | tr '/.' '__')
  exe="/tmp/coil-x64-ref-$id"
  if ! "$BIN" build "$f" -o "$exe" >/dev/null 2>"$REF/$id.buildlog"; then
    echo "BUILD FAIL: $f (see $REF/$id.buildlog)"; fail=1; continue
  fi
  cp "$exe" /tmp/coil-x64-fixed-$id && out=$(/tmp/coil-x64-fixed-$id "$@" </dev/null 2>"$REF/$id.stderr"; echo "EXIT:$?")
  code="${out##*EXIT:}"
