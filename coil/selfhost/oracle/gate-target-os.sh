...
#
# The bug it exists for: lib/fs.coil's open() flags are plain integers that
...
#
# Usage: selfhost/oracle/gate-target-os.sh <coil-binary>
set -uo pipefail
cd "$(dirname "$0")/../.."
...
# ---- and the same must hold for the real user of this: lib/fs.coil --------
...
fsflags() {
  if [ -z "$1" ]; then "$BIN" emit-ir lib/fs.coil 2>/dev/null
  else "$BIN" emit-ir lib/fs.coil --target "$1" 2>/dev/null; fi \
    | awk '/define .*@fs.write-file/,/^}/' \
    | grep -o 'BitOr\$i64\$|"(i64 [0-9]*, i64 [0-9]*' | head -1 \
    | sed 's/.*(i64 \([0-9]*\), i64 \([0-9]*\)/\1 \2/'
}
check "lib/fs.coil flags follow the host"    "$(fsflags '')"                         "64 512"
check "lib/fs.coil flags follow --target"    "$(fsflags aarch64-apple-darwin24.0.0)" "512 1024"
