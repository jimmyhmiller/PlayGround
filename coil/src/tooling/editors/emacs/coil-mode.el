...
Coil resolves a file's own `(import \"src/stdlib/…\")' paths relative to the process's
working directory, and the src/examples/stdlib expect that to be the PROJECT ROOT
\(the directory holding `src/stdlib/'). Walk up for the nearest `Coil.toml', then a
`src/stdlib/' directory, then `.git'; fall back to FILE's own directory. This is why
`run-coil' starts the REPL here rather than in the buffer's directory — so
`(import \"src/stdlib/…\")' resolves without you cd'ing anywhere."
...
file's `(import \"src/stdlib/…\")' paths resolve. Everything `inferior-lisp' offers
