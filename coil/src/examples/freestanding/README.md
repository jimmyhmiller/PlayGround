...
```
python3 scripts/dev.py example freestanding              # build + run hello.coil
python3 scripts/dev.py example freestanding uart         # typed-register PL011 driver
python3 scripts/dev.py example freestanding arena        # stdlib arena, bare-metal
python3 scripts/dev.py example freestanding hello --build-only
```
...
via the `defmmio-reg` macro (`src/stdlib/mmio.coil`):
...
freestanding-ness lives entirely in the `dev.py example freestanding` recipe plus the program:
...
- **Coil's codegen is freestanding-clean.** The linked image has ZERO undefined
  symbols (the build check asserts this) — no implicit malloc/abort/panic/runtime.
...
- **The explicit alloc/IO design works — and is FORCED.** With no ambient runtime
  ...
  using the capability-as-a-VALUE pattern (the same shape as `src/stdlib/alloc`/`src/stdlib/io`):
...
1. **No dead-code elimination → the stdlib dragged libc (the headline) — FIXED at the
   ...
   "src/stdlib/alloc.coil" …)` pulled in `malloc`/`free`/`realloc`/`abort` as undefined
   ...
   Now `import src/stdlib/alloc` + `--gc-sections` links with ZERO undefined symbols (see the
...
2. **`arena-allocator` malloced its buffer** (`src/stdlib/alloc.coil`) — the convenience
