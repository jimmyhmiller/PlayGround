...
struct Ctx<'a> {
    ...
    /// The expanding macro's module's imports + the program's exports, so hygiene
    /// resolves a template symbol in the macro's namespace (own defs AND `:use`d
    /// modules' exports) — Clojure-style: template names resolve where they're
    /// written, not at the use site.
    qualify_imports: Option<&'a crate::macros::ModImports>,
    qualify_exports: Option<&'a crate::macros::ExportMap>,
}

const FUEL: u64 = 50_000_000;
/// Max comptime call-stack depth. Bounds NATIVE recursion (each comptime call is
/// several Rust frames) so runaway recursion errors cleanly rather than overflowing
/// even the large dedicated expansion stack. Real macros recurse ~O(input size)
/// (fields, clauses, format chars), far below this.
const COMPTIME_MAX_DEPTH: u64 = 2_000;
...
pub fn fold_program(
    ...
    let ctx = Ctx {
        ...
        qualify_module: None,
        module_fns: None,
        qualify_imports: None,
        qualify_exports: None,
    };
...
pub fn expand_macro(
    ...
    extra_structs: &[StructDef],
    extra_sums: &[SumDef],
    imports: &crate::macros::ImportMap,
    exports: &crate::macros::ExportMap,
) -> Result<crate::reader::Sexp, String> {
    let table: HashMap<String, Func> = program.funcs.iter().map(|f| (f.name.clone(), f.clone())).collect();
    // Reflection also sees declarations generated by earlier macros in this pass.
    // These only FILL GAPS (genuinely macro-generated types) — never overwrite an
    // original, whose field types are already resolved (the extras are parsed raw).
    let mut smap: HashMap<String, StructDef> = program.structs.iter().map(|s| (s.name.clone(), s.clone())).collect();
    for s in extra_structs {
        smap.entry(s.name.clone()).or_insert_with(|| s.clone());
    }
    let mut smap_sums: HashMap<String, SumDef> = program.sums.iter().map(|s| (s.name.clone(), s.clone())).collect();
    for s in extra_sums {
    ...
    let ctx = Ctx {
        ...
        depth: std::cell::Cell::new(0),
        gensym: std::cell::Cell::new(0),
        // the macro's module + its imports, for referential hygiene of template symbols
        qualify_module: fn_name.rsplit_once('.').map(|(m, _)| m.to_string()),
        module_fns: Some(all_fns),
        qualify_imports: fn_name.rsplit_once('.').and_then(|(m, _)| imports.get(m)),
        qualify_exports: Some(exports),
    };
    ...
    let v = match &e.kind {
        ...
        ExprKind::Call { func, type_args, args } => {
            ...
            if ctx.variants.contains(func) {
                ...
                let d = ctx.depth.get() + 1;
                if d > COMPTIME_MAX_DEPTH {
                    return Err(format!(
                        "comptime: call stack too deep (> {COMPTIME_MAX_DEPTH}) calling '{func}' — runaway recursion?"
                    ));
                }
                ctx.depth.set(d);
                let r = eval_seq(&callee.body, &mut scope, ctx);
                ctx.depth.set(d - 1);
                return r;
            }
        }
...
/// a function (incl. a macro — macros are fns) VISIBLE in the expanding macro's
/// module is qualified to its defining module, so the generated call resolves where
/// the macro was written (own defs and `:use`d imports), not at the use site —
/// Clojure-style hygiene. Only the head is qualified; a symbol used as data (e.g.
/// compared with `code-eq`) is left alone, as are core forms, locals, unquoted args.
    ...
    if let (Some(m), Some(fns), Some(h)) = (&ctx.qualify_module, ctx.module_fns, out.first_mut()) {
        if let SexpKind::Sym(name) = &h.kind {
            if !name.contains('/') && !name.contains('.') {
                if let Some(q) = resolve_in_module(name, m, fns, ctx.qualify_imports, ctx.qualify_exports) {
...
/// Resolve a bare name to its qualified form in module `m`'s scope: M's own defs
/// first, then any `:use`d module's exported defs. Mirrors the name resolver.
fn resolve_in_module(
    name: &str,
    m: &str,
    fns: &HashSet<String>,
    imports: Option<&crate::macros::ModImports>,
    exports: Option<&crate::macros::ExportMap>,
) -> Option<String> {
    ...
    for (target, spec) in &imp.uses {
        let used = match spec {
            ...
            crate::macros::UseSpec::Names(ns) => ns.iter().any(|n| n == name),
        };
        let exported = exports.is_none_or(|e| crate::macros::exports(e, target, name));
        if used && exported {
            let q = format!("{target}.{name}");
            if fns.contains(&q) {
                return Some(q);
            }
        }
...
fn code_op(op: CodeOp, args: &[CtVal], ctx: &Ctx) -> Result<CtVal, String> {
    ...
    if op == CodeOp::TargetArch {
        // Coil AOT-targets the host, so the compile-host arch is the target arch.
        return Ok(CtVal::Code(Sexp::new(K::Sym(std::env::consts::ARCH.to_string()), dummy)));
    }
    if op == CodeOp::Error {
    ...
    if matches!(op, CodeOp::CFieldCount | CodeOp::CFieldName | CodeOp::CFieldKind | CodeOp::CFieldType) {
        ...
        // (`app.P`) — match exactly, else by a UNIQUE `.`-suffix. A bare name that
        // matches two modules' types is a hard error (never silently pick one).
        let sd = if let Some(s) = ctx.structs.get(&tname) {
            s
        } else {
            let mut it = ctx.structs.iter().filter(|(k, _)| k.rsplit('.').next() == Some(tname.as_str()));
            match (it.next(), it.next()) {
                (Some((_, s)), None) => s,
                (None, _) => return Err(format!(
                    "comptime: '{tname}' is not a struct (field reflection needs a struct; sums unsupported)"
                )),
                (Some(_), Some(_)) => return Err(format!(
                    "comptime: ambiguous type '{tname}' — multiple modules define it; field reflection can't disambiguate (qualify the type)"
                )),
            }
        };
    ...
    if matches!(op, CodeOp::CVariantSum | CodeOp::CVariantCount | CodeOp::CVariantName | CodeOp::CVariantFields) {
        ...
        if op == CodeOp::CVariantSum {
            // the (unique) sum whose variants include `arg` — ambiguity is an error
            let mut it = ctx.sums.iter().filter(|(_, sd)| sd.variants.iter().any(|v| bare(&v.name) == want));
            let sum = match (it.next(), it.next()) {
                (Some((k, _)), None) => k.clone(),
                (None, _) => return Err(format!("comptime: '{arg}' is not a known variant")),
                (Some(_), Some(_)) => return Err(format!(
                    "comptime: ambiguous variant '{arg}' — declared in multiple sums; reflection can't disambiguate"
                )),
            };
            return Ok(CtVal::Code(Sexp::new(K::Sym(sum), dummy)));
        }
        let sd = if let Some(s) = ctx.sums.get(&arg) {
            s
        } else {
            let mut it = ctx.sums.iter().filter(|(k, _)| k.rsplit('.').next() == Some(arg.as_str()));
            match (it.next(), it.next()) {
                (Some((_, s)), None) => s,
                (None, _) => return Err(format!("comptime: '{arg}' is not a known sum")),
                (Some(_), Some(_)) => return Err(format!(
                    "comptime: ambiguous sum '{arg}' — multiple modules define it; reflection can't disambiguate (qualify it)"
                )),
            }
        };
