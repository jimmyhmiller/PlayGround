...
pub fn load_program(
    ...
    // auto-loaded into every program — so `eq`/`hash`/`case`/operators work unimported.
    const PRELUDE: &str = include_str!("prelude.coil");
    let prelude_forms = crate::reader::read_all_unspanned(PRELUDE)?;
    process_forms(&prelude_forms, &mut out, &base, &mut visited, None, &mut imports, &mut exports)?;
    process_forms(forms, &mut out, &base, &mut visited, None, &mut imports, &mut exports)?;
...
fn process_import(
    ...
    if visited.insert(canon.clone()) {
        ...
        process_forms(&inc_forms, out, &inc_dir, visited, None, imports, exports)
