...
build/bootstrap/c/coil-bootstrap build src/examples/fib.coil -o /tmp/fib && /tmp/fib; echo $?   # -> 55
```

## How it works

`coilc.wasm` is the entire Coil compiler compiled to a single static
**wasm64** module (from `src/compiler/main_wasm.coil`), in which comptime runs
...
`python3 scripts/dev.py bootstrap c` does three steps, all with plain `cc`:
...
| `scripts/dev.py` | ours | the consolidated developer runner containing the 3-step build |
| `bootstrap/seeds/wasm/coilc.wasm` | generated seed | the compiler, as a wasm64 module |
...
build/bin/coil build src/compiler/main.coil -o /tmp/coil-i $(./scripts/compiler/llvm-link-flags.sh dynamic)
...
/tmp/coil-i build src/compiler/main_wasm.coil --target wasm64-unknown-unknown --wasm-stack-size=64 -o bootstrap/seeds/wasm/coilc.wasm
```

Then re-run `python3 scripts/dev.py bootstrap c`.

## What is verified

* `build/bootstrap/c/coil-bootstrap build src/examples/fib.coil -o /tmp/fib && /tmp/fib` exits **55**.
* **Self-build reproduction**: the C-bootstrapped compiler builds a compiler
  from `src/compiler/main_a64.coil --backend arm64`, and that binary's
