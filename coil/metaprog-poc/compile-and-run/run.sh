...
[ $rc -eq 0 ] || { echo "own_ok FAILED (exit $rc, want 0)"; exit 1; }
$COIL run $D/own_bad.coil > "$OUT/own.txt" 2>&1; rc=$?
[ $rc -ne 0 ] || { echo "own_bad FAILED to veto"; exit 1; }
ua=$(grep -c "error: use after release" "$OUT/own.txt"); dr=$(grep -c "error: double release" "$OUT/own.txt")
...
echo "=== 12. A GC DIALECT: per-type tracers GENERATED by a whole-program transform ==="
echo "       gcgen-demo.coil marks two types, writes ZERO tracer code; the transform"
echo "       emits Node-trace/Pair-trace + gc-managed-count; the collector reclaims"
( cd gc-dialect-poc && "$OLDPWD/$COIL" run gcgen-demo.coil > "$OUT/gc.txt" 2>&1 ) ||   ( cd gc-dialect-poc && ../coil run gcgen-demo.coil > "$OUT/gc.txt" 2>&1 ); rc=$?
grep -q "total_sum=2450000" "$OUT/gc.txt" || { cat "$OUT/gc.txt"; echo "gc demo wrong result"; exit 1; }
grep -q "managed-types=2" "$OUT/gc.txt" || { cat "$OUT/gc.txt"; echo "transform did not generate the count"; exit 1; }
cat "$OUT/gc.txt"
echo "gc dialect: OK (tracers + count generated by transform; graph collected)"
