...
and ARM64 `build` are regression-gated in `scripts/compiler/oracle/gate-cli.sh`.

Reproduced with the `coil` on PATH, macOS arm64, 2026-07-29. Deterministic: 5 out of 5 runs.

```sh
coil check    tests/repro/sigbus-emit-ir-singleton-cycle/crash.coil   # exit 0
coil dump-mono tests/repro/sigbus-emit-ir-singleton-cycle/crash.coil  # exit 0
coil emit-ir  tests/repro/sigbus-emit-ir-singleton-cycle/crash.coil   # Bus error: 10
coil build    tests/repro/sigbus-emit-ir-singleton-cycle/crash.coil -o /tmp/x                  # Bus error: 10
coil build    tests/repro/sigbus-emit-ir-singleton-cycle/crash.coil -o /tmp/x --backend arm64  # Bus error: 10
```
