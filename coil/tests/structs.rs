...
fn array_walk_with_recursion() {
    ...
    let src = r#"
        (defn sum [(a (ptr heap (array i64 4))) (i :i64) (n :i64)] (-> :i64)
          (if (icmp-ge i n) 0
...
fn rejects_unknown_field() {
    let src = r#"
        (defstruct P [(x :i64)])
        (defn main [] (-> :i64)
          (let [p (alloc frame P)] (load (field p y))))
    "#;
...
fn rejects_field_on_non_struct() {
    let src = "(defn main [] (-> :i64) (let [p (alloc heap)] (load (field p x))))";
