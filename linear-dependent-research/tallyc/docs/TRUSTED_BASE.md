...
| mutual-recursion certification (Phase B3) | an SCC is `%total` when EVERY call into the cycle passes a strict subterm of the caller's scrutinee into the CALLEE's matched position (size-change v1) | any infinite chain through the SCC would strictly descend a structural measure forever. Like wf-recursion this is analyzer-backed (the lowering is forward-call-unrolled `Fix`, not an eliminator); the same honest trusted-base delta. |
...
| `ajoin` | `… (1 lo : Slice llo a k) -> (1 hi : Slice lhi a m) -> (1 rj : Rejoin llo lhi a k m) -> Arr a (k + m)` | identity on the base. PROVENANCE IS TYPED: each split packs two fresh erased `Loc`s existentially (`MkASplit {0 llo}{0 lhi} …`, openable BY NAME with a full-arity pattern), and the Rejoin token demands exactly that pair in that order — a swapped join (which at `k = m` type-checked and made `afree` free an INTERIOR pointer: heap corruption, found re-auditing this row) and a cross-split join are now type errors. |
