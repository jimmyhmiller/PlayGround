...
| `docs/07-implementation-guide.md` | How to actually build it: the rig, the usage-as-output bidirectional checker, the soundness obligation, the staging order, and the pitfalls. |
| `docs/08-prior-art-vale.md` | Prior art: Vale's "Higher RAII"; where λ-Tally independently converges, the one deep divergence (what linearity is *for*), and features worth stealing. |
| `docs/09-totality-and-partial-eval.md` | Totality (eliminators ⇒ total by construction; strict positivity; the `partial` quarantine) and partial evaluation (multiplicities = staging; NbE = the partial evaluator). |
| `docs/10-surface-syntax.md` | The v1.0 surface language: ML-flavored types/signatures, Rust-flavored terms (`fn`/`enum`/`struct`/`match`); grammar, elaboration to the kernel, and the implementation roadmap. |
...
| `prototype/`               | **Runnable** semi-formal models: `qtt_checker.py` (QTT type checker, Stage A), `memory-model.rkt` (PLT Redex memory semantics + linear safety, Stage B), `lambda_tally_memory.py` (the unification — memory primitives as QTT functions, Stage C), `memory-safety-rosette.rkt` (Rosette: unbounded memory safety via an inductive invariant + erasure soundness, Stage D), `resource-soundness-rosette.rkt` (Rosette: a multiplicity checker + evaluator proving *accepts ⟹ no leak / no use-after-free*), and `interloc-dependency-rosette.rkt` (Rosette: inter-location dependency safe under freezing). See `prototype/README.md`. |
| `agda/`                    | **Machine-checked metatheory** (the proof-assistant phase): `Rig.agda` (the multiplicity semiring + laws + usage order, with linearity = exactly-once proven) and `Context.agda` (contexts as a module over the rig). The unbounded counterpart to the bounded Rosette evidence. See `agda/README.md`. |
