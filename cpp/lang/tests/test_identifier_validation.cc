...
void test_semicolon_should_not_be_identifier() {
  std::cout << "=== Testing Semicolon Should Not Be Identifier ===" << std::endl;
  ...
  try {
    ...
    std::function<void(const ASTNode*)> search_for_semicolon = [&](const ASTNode* node) {
      if (!node) return;
      
      if (node->type == ASTNodeType::Identifier && node->value == ";") {
        found_semicolon_identifier = true;
        std::cout << "❌ FOUND SEMICOLON IDENTIFIER at token line " << node->token.line 
                  << ", column " << node->token.column << std::endl;
      }
    ...
    if (found_semicolon_identifier) {
      std::cout << "❌ FAILED: Found semicolon being treated as identifier" << std::endl;
    } else {
      std::cout << "✅ SUCCESS: No semicolon identifiers found - semicolons properly handled as separators" << std::endl;
    }
...
void test_invalid_identifier_characters() {
  ...
  for (const auto& test_case : invalid_cases) {
    ...
    try {
      ...
      std::function<void(const ASTNode*)> check_identifiers = [&](const ASTNode* node) {
        if (!node) return;
        ...
        if (node->type == ASTNodeType::Identifier) {
          ...
          if (!value.empty()) {
            ...
            if (!std::isalpha(value[0]) && value[0] != '_') {
              found_invalid = true;
              std::cout << "  ❌ Invalid identifier: '" << value << "' (starts with non-letter)" << std::endl;
            }
            ...
            for (char c : value) {
              if (!std::isalnum(c) && c != '_') {
                found_invalid = true;
                std::cout << "  ❌ Invalid identifier: '" << value << "' (contains '" << c << "')" << std::endl;
                break;
              }
            }
          }
      ...
      if (!found_invalid) {
        std::cout << "  ⚠️  No invalid identifiers detected (this might indicate parsing didn't create invalid identifiers)" << std::endl;
      }
