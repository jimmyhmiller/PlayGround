...
std::string generate_struct_declaration(const ASTNode *node, int indent) {
  ...
  for (size_t i = 0; i < node->child_count(); ++i) {
    ...
    if (child->type != ASTNodeType::FieldDeclaration) {
      ...
      if (child->type == ASTNodeType::BinaryExpression && child->value == ":" &&
          child->child_count() >= 2) {
        result << generate_expression(child->child(0), 0) << ": "
               << generate_expression(child->child(1), 0);
      } else {
...
std::string generate_expression(const ASTNode *node, int /* indent */) {
  ...
  case ASTNodeType::BinaryExpression:
    if (node->child_count() >= 2) {
      return generate_expression(node->child(0), 0) + " " + node->value + " " +
...
std::string generate_block(const ASTNode *node, int indent) {
  ...
  for (size_t i = 0; i < node->child_count(); ++i) {
    ...
    if (!stmt.empty()) {
      ...
      // If it starts with spaces, it's already indented, otherwise add
      // indentation
      if (stmt[0] != ' ') {
      ...
      // - The last expression in a block should NOT have a semicolon (it's the
      // return value)
      bool is_last_statement = (i == node->child_count() - 1);
      bool always_needs_semicolon =
          (node->child(i)->type == ASTNodeType::LetStatement ||
      ...
      bool is_expression_statement =
          (node->child(i)->type == ASTNodeType::ExpressionStatement);

      if (always_needs_semicolon ||
          (is_expression_statement && !is_last_statement)) {
        result << ";";
      }
...
std::string generate_type(const ASTNode *type_node) {
  ...
  case ASTNodeType::FunctionType: {
    ...
    bool needs_parens =
        !type_node->name.empty() && type_node->name == "parenthesized";
