...
impl Compiler {
    pub fn new<M: ValueModel>(rt: &mut Runtime<M>) -> Self {
        ...
        for (name, p) in [
            ...
            ("%var-flags", VarFlags), ("%ns-interns", NsInterns), ("%ns-aliases", NsAliases), ("%resolve-in-ns", ResolveInNs), ("%rand", Rand), ("%cpu-count", CpuCount), ("%amap-get", AmapGet), ("%proto-has-type?", ProtoHasType), ("%scalar-type?", ScalarType), ("%eq2", Eq2), ("%all-ns", AllNs),
